...
详见 `docs/new-module-checklist.md`。摘要：

| 步骤 | 内容 |
|------|------|
| 1. 数据库 | `prisma/models/<domain>.prisma` + migration + seed |
| 2. RBAC | seed 注册资源树 + `module-nav.tsx` 配 `resourceKey` |
| 3. 页面 | facade server component + 子目录 `layout.tsx` 路由门禁 |
| 4. API | 认证 → 参数校验 → 调 service → 返回 DTO |
| 5. Service | `server/services/<domain>/` 业务逻辑 |
| 6. 文档 | `ARCHITECTURE.md` + 更新 CLAUDE.md 路由表 |
| 7. 硬约束 | `tsc --noEmit` / `lint --max-warnings=0` / `build` / `arch:check` / `size:check` |
