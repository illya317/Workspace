...
import { loadOrCreateStructureReport } from "./structure-report-cache";
...
type StructureReport = ReturnType<typeof buildStructureReport>;

function isStructureReport(value: unknown): value is StructureReport {
  if (!value || typeof value !== "object" || Array.isArray(value)) return false;
  const report = value as Partial<StructureReport>;
  return report.kind === "structure"
    && report.mode === "structure-intelligence"
    && !!report.summary
    && typeof report.summary === "object"
    && !!report.registries
    && typeof report.registries === "object"
    && !!report.patterns
    && typeof report.patterns === "object"
    && !!report.drift
    && typeof report.drift === "object";
}

export function createStructureReport(): StructureReport {
  return loadOrCreateStructureReport({
    createReport: buildStructureReport,
    validateReport: isStructureReport,
  });
}
