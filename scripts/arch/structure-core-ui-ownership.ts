...
export function findDomainSharedLayoutShells(files: SourceInfo[]) {
  ...
  for (const file of files) {
    ...
    if (!file.hasJsx && !/PageSurface|PageSurfaceTabBarSpec|PageSurfaceToolbar|Toolbar|TabBar|NavigationSurface/.test(file.text)) continue;
    candidates.push({
      file: file.relPath,
      packageName,
...
export function findSurfaceOwnsPageChrome(files: SourceInfo[]) {
  ...
  const surfaceTypeFiles = files.filter((file) => /^packages\/core\/ui\/(BodySurface|DataSurface|FormSurface)\.types\.ts$/.test(file.relPath));

  for (const file of surfaceTypeFiles) {
    const checks: Array<{ componentName: string; propName: string; pattern: RegExp; detail: string }> = [
      {
        componentName: "BodySurface",
        propName: "toolbar",
        pattern: /toolbarItems\??:|toolbar\??:/,
        detail: "BodySurface still declares page toolbar ownership; split controls must be derived into PageSurface.toolbar",
      },
      {
  ...
  const surfaceRuntimeFiles = files.filter((file) => /^packages\/core\/ui\/(BodySurface|CreateSurface|DataSurface|DocumentSurface|FormSurface|NavigationSurface|SelectorSurface|VisualizationSurface)\.tsx$/.test(file.relPath));
  for (const file of surfaceRuntimeFiles) {
    if (!/<Toolbar(?:\s|>)/.test(file.text)) continue;
    const componentName = path.basename(file.relPath, ".tsx");
    warnings.push({
      file: file.relPath,
      componentName,
      propName: "renderer",
      detail: `${componentName} renders Toolbar chrome; only PageSurface may render the page toolbar`,
    });
  }
