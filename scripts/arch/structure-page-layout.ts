...
function isBusinessToolbarCompositionScanFile(file: SourceInfo) {
  ...
  return /^packages\/(administration|capital-securities|external|finance|hr|library|production|work)\/ui\//.test(file.relPath);
}
    ...
    const visit = (node: ts.Node) => {
      ...
      if (ts.isJsxOpeningElement(node) || ts.isJsxSelfClosingElement(node)) {
        ...
        if (tagName === "PageSurface") {
          ...
          if (jsxHasAttribute(node.attributes, "embedded")) {
            addLayoutWarning(warnings, file.relPath, "embedded-page-surface", "PageSurface cannot be nested; use BodySurface for local content");
          }
        }
