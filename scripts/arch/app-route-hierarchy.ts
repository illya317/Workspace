...
import type { PageRouteRegistration } from "../../packages/core";
...
function checkRegisteredHierarchy(violations: Violation[]) {
  for (const registration of registeredModuleDefinitions) {
    ...
    for (const routeRegistration of registration.routes ?? []) {
      const route = pageRoutePath(routeRegistration);
  ...
  const registeredL2Routes = new Set(
    registeredModuleDefinitions.flatMap((registration) =>
      [
        ...
        ...(registration.routes ?? [])
          .map((routeRegistration) => normalizeRoute(pageRoutePath(routeRegistration)))
          .filter((route) => routeSegments(route).length === 2),
      ],
    ),
  );
