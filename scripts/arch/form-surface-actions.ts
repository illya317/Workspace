...
function commandObjects(
  ...
  if (ts.isParenthesizedExpression(node)) return commandObjects(node.expression, resolve, seen);
  return [];
}
  ...
  const visit = (node: ts.Node) => {
    if (ts.isCallExpression(node)) {
      ...
      if ((name === "createSectionSection" || name === "createPanelSection") && node.arguments[1] && ts.isObjectLiteralExpression(node.arguments[1])) {
        ...
        if (actionsProperty && sectionsNode && /create(?:Form|Fields|InlineFields)Section/.test(resolvedText(sectionsNode, sourceFile, resolve)) && parentActions.some((action) => isLifecycleCommand(action, sourceFile))) {
