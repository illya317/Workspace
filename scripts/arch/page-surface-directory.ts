...
const BODY_PAGE_INTEGRATION_FILE = "packages/core/ui/internal/body/BodySurfacePageIntegration.tsx";
...
export function checkPageSurfaceDirectoryRenderer() {
  ...
  const bodyIntegrationSource = readFileSync(BODY_PAGE_INTEGRATION_FILE, "utf8");
  const directoryRenderer = rendererSource(source, "renderDirectorySurface");
  const directorySectionRenderer = rendererSource(bodyIntegrationSource, "renderDirectorySection");
  const directoryBodyRenderer = rendererSource(bodyIntegrationSource, "renderBodySurfaceDirectory");
  ...
  if (!loginRenderer) {
    ...
    if (!loginRenderer.includes("renderBodySurfaceLoginForm") || !loginRenderer.includes("place-items-center")) {
