...
const SURFACE_PUBLIC_CONTRACT_RULES: Array<{
  ...
  {
    file: "packages/core/ui/PageSurface.types.ts",
    pattern: /\bembedded\??\s*:/,
    label: "embedded-page-surface",
    reason: "PageSurface is the page root and must not expose embedded mode; local content belongs to BodySurface.",
  },
  {
    file: "packages/core/ui/BodySurface.types.ts",
    pattern: /\bBodySurfaceSectioningSpec\b|\bsectioning\??\s*:/,
    label: "body-page-tabs",
    reason: "Page tabs belong to PageSurface.tabbar; BodySurface must not expose tab sectioning.",
  },
  {
    file: "packages/core/ui/BodySurface.types.ts",
    pattern: /\bBodySurfacePaginationProps\b|\bkind:\s*["'](?:navigation|pagination)["']/,
    label: "body-navigation-or-pagination",
    reason: "BodySurface must not expose standalone tabs or pagination; page tabs/footer belong to PageSurface and modal pagination belongs to BodySurfaceModalSpec.",
  },
  {
    file: "packages/core/ui/index.ts",
    pattern: /export\s*\{\s*default\s+as\s+NavigationSurface\s*\}/,
    label: "public-navigation-renderer",
    reason: "NavigationSurface is an internal renderer; callers declare PageSurface tabbar/footer, BodySurfaceModalSpec pagination, or NavigationContextSelector.",
  },
  {
    file: "scripts/generate-core-ui-surface-contracts.ts",
    pattern: /componentName:\s*["']NavigationSurface["']/,
    label: "generated-navigation-contract",
    reason: "Internal NavigationSurface must not receive an Agent-facing generated contract.",
  },
  {
    file: "packages/core/ui/internal/page/PageFrames.tsx",
    pattern: /\bWorkspaceSplitPage\b|\bAnalysisPageFrame\b|\bPageFrameTabs\b|\btoolbarItems\??\s*:/,
    label: "parallel-page-frame",
    reason: "PageSurface is the only page composition contract; internal frames must not expose parallel tabs or toolbar APIs.",
  },
  {
...
const SURFACE_DECLARE_RULES: Record<string, {
  ...
  maxTopLevelDeclares?: number;
  maxTotalDeclares?: number;
}> = {
  BodySurface: {
    topLevel: ["kind", "layout", "gridColumns", "mobilePresentation", "sections", "section.disclosure", "commands", "empty", "modals", "split"],
    maxTopLevelDeclares: 8,
    maxTotalDeclares: 32,
  },
  CreateSurface: {
    topLevel: ["trigger", "presentation", "anchor", "content", "submission", "feedback", "state"],
  },
  DataSurface: {
    topLevel: ["kind", "actions"],
  },
  DocumentSurface: {
    topLevel: ["kind", "pages", "viewer"],
  },
  FormSurface: {
    topLevel: ["kind", "content", "header", "actions", "commands", "submit"],
  },
  InputSurface: {
    topLevel: ["control", "valueType", "options", "presentation", "format", "mask", "state", "validation", "autoGrow", "usage", "dependencies"],
    maxTopLevelDeclares: 9,
  },
  NavigationSurface: {
    topLevel: [],
    maxTotalDeclares: 48,
  ...
  SelectorSurface: {
    topLevel: ["kind", "commands", "status", "card.actions", "card.inlineEdit", "loading", "emptyText"],
  },
  ...
  if (existsSync(pageSurfaceTypesFile)) {
    ...
    for (const statement of sourceFile.statements) {
      ...
      warnings.push({
        ...
        reason: "PageSurface public contract only exposes tabbar/toolbar/body/footer page chrome; header belongs to AppShell.",
      });
    }
  }
...
export function findSurfaceDeclareBoundaryWarnings(
  registry: readonly CoreUiComponentRegistration[] = coreUiComponentRegistry,
) {
  ...
  for (const component of registry) {
    ...
    const maxTopLevelDeclares = rule.maxTopLevelDeclares ?? MAX_SURFACE_TOP_LEVEL_DECLARES;
    if (topLevelNames.length > maxTopLevelDeclares) {
      warnings.push({
        surface: component.name,
        declarePath: "*",
        reason: `too many top-level declares (${topLevelNames.length}/${maxTopLevelDeclares})`,
      });
    }
...
export function checkSurfaceDeclareBoundaries() {
  const warnings = findSurfaceDeclareBoundaryWarnings();
  if (warnings.length === 0) {
    console.log("✓ Surface declare boundary gate passed.");
    return true;
  }

  console.error(`✗ Surface declare boundary gate: ${warnings.length} violation(s) detected.`);
  console.error("  Public declarations must stay on their owning Page/Content/Common Surface contract.");
  for (const warning of warnings) {
    console.error(`  - ${warning.surface}.${warning.declarePath}: ${warning.reason}`);
  }
  return false;
}
