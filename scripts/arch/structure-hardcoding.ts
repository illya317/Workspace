...
const CORE_BUSINESS_FACT_SIGNALS: Array<{ signal: string; regex: RegExp }> = [
  ...
  { signal: "capital-securities-domain", regex: /\b(CapitalSecurities|GovernanceOrganization|Investor)\b|资本证券|治理架构|投资人/ },
];
...
function typeReferenceName(node: ts.TypeReferenceNode) {
  const name = node.typeName;
  return ts.isIdentifier(name) ? name.text : name.right.text;
}

function isDataSurfaceColumnSpecType(node: ts.TypeNode): boolean {
  if (ts.isParenthesizedTypeNode(node)) return isDataSurfaceColumnSpecType(node.type);
  if (ts.isTypeOperatorNode(node)) return isDataSurfaceColumnSpecType(node.type);
  if (ts.isArrayTypeNode(node)) return isDataSurfaceColumnSpecType(node.elementType);
  if (!ts.isTypeReferenceNode(node)) return false;

  const name = typeReferenceName(node);
  if (name === "DataSurfaceColumnSpec") return true;
  if ((name === "Array" || name === "ReadonlyArray") && node.typeArguments?.length === 1) {
    return isDataSurfaceColumnSpecType(node.typeArguments[0]);
  }
  return false;
}

function isTypedDataSurfaceColumnDeclaration(declaration: ts.VariableDeclaration) {
  if (declaration.type && isDataSurfaceColumnSpecType(declaration.type)) return true;
  return Boolean(
    declaration.initializer
    && ts.isSatisfiesExpression(declaration.initializer)
    && isDataSurfaceColumnSpecType(declaration.initializer.type),
  );
}
  ...
  for (const file of files) {
    ...
    for (const statement of file.sourceFile.statements) {
      ...
      for (const declaration of statement.declarationList.declarations) {
        ...
        if (!kind) continue;
        // Typed DataSurface tables are approved structural declarations; the UI gate separately blocks raw/custom cells.
        if (kind === "columns" && isTypedDataSurfaceColumnDeclaration(declaration)) continue;
