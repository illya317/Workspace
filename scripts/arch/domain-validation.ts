...
import { createCompletionDatePolicyIssues } from "./completion-date-policy-gate";
...
export function checkDomainValidation() {
  try {
    const completionDatePolicyIssues = createCompletionDatePolicyIssues();
    if (completionDatePolicyIssues.length > 0) {
      console.error("✗ Global completion/date policy gate failed.");
      for (const issue of completionDatePolicyIssues) {
