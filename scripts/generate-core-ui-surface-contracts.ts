...
const OUTPUT_PATH = path.join(ROOT, "docs/generated/core-ui-surface-contracts.json");
const MAX_DEPTH = 12;
const REFERENCE_ONLY_NESTED_TYPE_NAMES = new Set([
...
const TARGETS: ContractTarget[] = [
  { componentName: "BodySurface", sourcePath: "packages/core/ui/BodySurface.types.ts", typeName: "BodySurfaceProps" },
  { componentName: "CreateSurface", sourcePath: "packages/core/ui/CreateSurface.types.ts", typeName: "CreateSurfaceProps" },
  ...
  { componentName: "PaperInputSurface", sourcePath: "packages/core/ui/PaperInputSurface.types.ts", typeName: "PaperInputSurfaceProps" },
  { componentName: "PageSurface", sourcePath: "packages/core/ui/PageSurface.types.ts", typeName: "PageSurfaceProps" },
  { componentName: "SelectorSurface", sourcePath: "packages/core/ui/SelectorSurface.types.ts", typeName: "SelectorSurfaceProps" },
...
function readTsConfig() {
  ...
  if (!configPath) throw new Error("Cannot find tsconfig.base.json");
...
function childrenFor(
  ...
  if (REFERENCE_ONLY_NESTED_TYPE_NAMES.has(typeKey)) return undefined;
...
function descriptorForPropertyGroup(
  ...
  const declaredType = ts.isPropertySignature(declaration) && declaration.type
    ? declaration.type.getText(declaration.getSourceFile())
    : undefined;
  const referenceOnlyType = declaredType
    ? [...REFERENCE_ONLY_NESTED_TYPE_NAMES].find((typeName) => declaredType.includes(typeName))
    : undefined;
  if (referenceOnlyType) {
    return {
      name: property.name,
      description: `引用 ${referenceOnlyType} 声明。`,
    };
  }
...
function descriptorJsonLines(descriptor: ContractDescriptor): string[] {
  const fields = `\"name\":${JSON.stringify(descriptor.name)},\"description\":${JSON.stringify(descriptor.description)}`;
  if (!descriptor.children?.length) return [`{${fields}}`];
  return [
    `{${fields},\"children\":[`,
    ...descriptorListJsonLines(descriptor.children),
    "]}",
  ];
}

function descriptorListJsonLines(descriptors: ContractDescriptor[]) {
  return descriptors.flatMap((descriptor, index) => {
    const lines = descriptorJsonLines(descriptor);
    if (index < descriptors.length - 1) lines[lines.length - 1] += ",";
    return lines;
  });
}
...
function formatOutput(contracts: Record<string, ContractDescriptor[]>) {
  ...
  for (const [index, [name, descriptors]] of entries.entries()) {
    lines.push(`${JSON.stringify(name)}:[`);
    lines.push(...descriptorListJsonLines(descriptors));
