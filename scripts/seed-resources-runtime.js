...
try {
  ...
  // Deploy sources .env before running this script; local/manual runs use dotenv when available.
}
...
function seedResources(db, resources) {
  ...
  const updateResource = db.prepare(`
    UPDATE Resource
    SET name = ?, parentId = ?, scopeTypes = ?, scopeInheritanceMode = ?, sortOrder = ?
    WHERE key = ?
  `);
  ...
  for (const resource of resources) {
    ...
    if (existing) {
      updateResource.run(resource.name, parentId, resource.scopeTypes, resource.scopeInheritanceMode, resource.sortOrder, resource.key);
    } else {
      insertResource.run(resource.key, resource.name, parentId, resource.scopeTypes, resource.scopeInheritanceMode, resource.sortOrder);
    }
  }
  ...
  if (staleIds.length > 0) {
    ...
    db.prepare(`DELETE FROM UserResourceActionGrant WHERE resourceId IN (${staleIdSql})`).run(...staleIds);
    db.prepare(`DELETE FROM PositionResourceActionGrant WHERE resourceId IN (${staleIdSql})`).run(...staleIds);
    db.prepare(`DELETE FROM DepartmentResourceActionGrant WHERE resourceId IN (${staleIdSql})`).run(...staleIds);
