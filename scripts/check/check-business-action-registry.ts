...
const EXPLICIT_NON_BUSINESS_WRITE_ROUTES = new Map<string, NonBusinessWriteRouteClassification>([
  ...
  ["POST /api/modules/finance/agent/rpc", { kind: "internal_rpc", reason: "Signed internal Agent transport; the selected Finance tool owns business authorization and mutation semantics." }],
  ["POST /api/modules/hr/agent/rpc", { kind: "internal_rpc", reason: "Signed internal Agent transport; the selected HR tool owns business authorization and mutation semantics." }],
  ["POST /api/modules/library/agent/rpc", { kind: "internal_rpc", reason: "Signed internal Agent transport; the selected Library tool owns business authorization and mutation semantics." }],
  ["POST /api/modules/work/agent/rpc", { kind: "internal_rpc", reason: "Signed internal Agent transport; the selected Work tool owns business authorization and mutation semantics." }],
