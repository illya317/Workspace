...
const HISTORY_FK_FIELDS = [
  ...
  ["Company", "partyId"],
  ["OwnershipInterest", "ownerPartyId"],
  ["OwnershipInterest", "issuerCompanyId"],
  ["ExternalPartyRole", "partyId"],
  ["Department", "parentId"],
    ...
    function visit(node) {
      if (ts.isVariableDeclaration(node) && ts.isIdentifier(node.name) && node.name.text.endsWith("_RELATION_REGISTRATIONS") && node.initializer) {
        ...
        if (!ts.isArrayLiteralExpression(initializer)) {
          fail(`${path.relative(ROOT, registryPath)}:${lineOf(sourceFile, node)} relation registrations must be an array literal`);
          return;
        }
