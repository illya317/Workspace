...
assertApiActions("POST", "/api/modules/finance/import/preview", ["read"]);
assertApiActions("POST", "/api/modules/finance/import/confirm", ["import"]);
...
for (const scopeType of ["department", "committee", "company"] as const) {
  const parentKey = `space.${scopeType}`;
  const childKeys = ["tasks", "projects", "templates"].map((kind) => `${parentKey}.${kind}`);
  assert.deepEqual(
...
assert.equal(actionPolicy("space.department").supportedActions.includes("configure"), false, "space parents should not expose retired workflow configuration");
...
const systemPriorityRecord = buildPermissionRecords({
  ...
  subjectType: "user",
  selectedResource: "finance.import",
  ancestorResourceKeys: [],
  directActionGrants: [],
  positionActionGrants: [{ subjectId: 10, resourceKey: "finance.import", actionKey: "export", resourceId: 1, scopeId: null }],
  departmentActionGrants: [],
  implicitActionGrants: [{ subjectId: 1, resourceKey: "finance.import", actionKey: "import", resourceId: 0, scopeId: null, source: "system" }],
})[1];
