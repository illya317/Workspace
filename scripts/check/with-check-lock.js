...
function createCheckCacheDescriptor() {
  ...
  try {
    ...
    addHashPart(hash, "kind", `check-result-v2:${kind}`);
    ...
    for (const key of cacheableEnvKeys) {
      addHashPart(hash, `env.${key}`, process.env[key] ?? "");
    }
...
(async () => {
  const cacheDescriptor = createCheckCacheDescriptor();
