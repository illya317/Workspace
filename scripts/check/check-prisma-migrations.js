...
if (fs.existsSync(legacyDir)) {
  fail("SQLite legacy migration 归档不得进入源码；请放入 WORKSPACE_CONFIG_DIR/audit");
}
ok("Pre-genesis migration history is absent from source and retained only in the private audit workspace");
...
if (foreignKeyCount === 0 || deferredForeignKeyCount === 0 || deferredForeignKeyCount > foreignKeyCount) {
  fail(`PostgreSQL baseline must preserve its explicit deferrable foreign-key subset (${deferredForeignKeyCount}/${foreignKeyCount})`);
}
...
ok(`PostgreSQL baseline preserves ${foreignKeyCount} foreign keys (${deferredForeignKeyCount} explicitly deferred) and four custom invariants`);
