...
test("suite manifest reuses one validated changed-file calculation", (t) => {
  const { cwd } = fixture();
  t.after(() => fs.rmSync(cwd, { recursive: true, force: true }));
  const snapshotKey = "a".repeat(64);
  const manifestFile = path.join(cwd, ".git", "changed-files.json");
  fs.writeFileSync(manifestFile, JSON.stringify({
    ...
    diffFilter: "ACMR",
    staged: [],
    unstaged: ["manifest-only.ts"],
    untracked: [],
    files: ["manifest-only.ts"],
    hasStagedChanges: false,
    source: "worktree",
  }));

  assert.deepEqual(changedFileSets({
    cwd,
    env: {
      CHECK_WORKSPACE_SNAPSHOT_KEY: snapshotKey,
      WORKSPACE_CHANGED_FILES_MANIFEST: manifestFile,
    },
  }).files, ["manifest-only.ts"]);
  ...
  assert.deepEqual(changedFileSets({
    cwd,
    env: {
      CHECK_WORKSPACE_SNAPSHOT_KEY: "b".repeat(64),
      WORKSPACE_CHANGED_FILES_MANIFEST: manifestFile,
    },
  }).files, []);
});
