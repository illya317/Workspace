...
async function main() {
  await Promise.all(["01", "02", "03"].map((code, index) => prisma.company.upsert({
    where: { code },
    update: { name: `E2E公司${code}`, isActive: true, sortOrder: index },
    create: { code, name: `E2E公司${code}`, isActive: true, sortOrder: index },
  })));
