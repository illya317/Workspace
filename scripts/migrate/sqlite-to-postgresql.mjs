...
const BASELINE_MIGRATION = "00000000000000_sanitized_baseline";
...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (["--sqlite", "--target", "--manifest", "--expected-source-sha256", "--legacy-migrations-dir"].includes(arg)) {
...
function usage() {
  return [
    ...
    "  --legacy-migrations-dir <dir>     Absolute private pre-genesis SQLite migration archive",
...
function sourceMigrationState(sqlite, legacyMigrationsDir) {
  const exists = sqlite
  ...
  const expected = readdirSync(legacyMigrationsDir, { withFileTypes: true })
    .filter((entry) => entry.isDirectory())
    .map((entry) => entry.name)
    .sort();
  const expectedChecksums = new Map(expected.map((name) => {
    const sqlPath = join(legacyMigrationsDir, name, "migration.sql");
  ...
  if (
    ...
    || blankChecksumRows.length > 0
  ) {
    throw new Error(
      `Source SQLite migration set mismatch; missing=${missing.join(",") || "none"}; unknown=${unknown.join(",") || "none"}; duplicates=${[...new Set(duplicates)].join(",") || "none"}; checksumMismatches=${checksumMismatches.join(",") || "none"}; blankChecksums=${blankChecksumRows.map((row) => row.migrationName).join(",") || "none"}`,
    );
  }
  return {
    ...
    pinnedLegacyChecksumVerifiedCount: 0,
...
async function main() {
  ...
  const legacyMigrationsInput = options.legacy_migrations_dir ?? process.env.SQLITE_LEGACY_MIGRATIONS_DIR;
  if (!legacyMigrationsInput || !isAbsolute(legacyMigrationsInput)) {
    throw new Error("An absolute private --legacy-migrations-dir or SQLITE_LEGACY_MIGRATIONS_DIR is required");
  }

  const sourcePath = await realpath(resolve(options.sqlite));
  const legacyMigrationsDir = await realpath(legacyMigrationsInput);
  ...
  try {
    ...
    const sourceMigrations = sourceMigrationState(sqlite, legacyMigrationsDir);
