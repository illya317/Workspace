...
async function main() {
  ...
  // Sort by organization hierarchy (G/M 1 -> 2 -> 3)
  allDepts.sort((a, b) => String(a.hierarchyKind || 'M').localeCompare(String(b.hierarchyKind || 'M')) || a.level - b.level);
  ...
  for (const d of allDepts) {
    ...
    const created = await prisma.department.create({
      data: {
        ...
        hierarchyKind: d.hierarchyKind || 'M',
        level: d.level,
        parentId,
      }
    });
