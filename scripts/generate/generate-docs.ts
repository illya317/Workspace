...
function permLabel(resource: string, action: string): string {
  return `${resource}.${action}`;
}

function extractContractAuth(method: ApiMethod, apiPath: string): string | null {
  const contract = findApiContract(method, apiPath);
  if (!contract) return null;
  if (contract.access === "disabled") return "已禁用";
  if (contract.access === "internal") return "内部服务鉴权";
  if (contract.access === "dev") return "仅开发环境";
  if (contract.access === "public") return "公开（API contract）";

  const checks = ["登录"];
  if (contract.resourceKey) {
    const actions = contract.requiredActions.length > 0
      ? contract.requiredActions.map((action) => `${contract.resourceKey}.${action}`).join("/")
      : contract.resourceKey;
    checks.push(actions);
  }
  if (contract.runtimeEnforcement === "serviceDelegated") checks.push("service 授权");
  return checks.join(" + ");
}

function extractAuth(content: string, method: ApiMethod, apiPath: string): string {
  ...
  if (contractAuth) {
    return content.includes("requireAdminApiAccess(")
      ? `${contractAuth} + 管理权限`
      : contractAuth;
  }

  const hasAuth = content.includes("authenticate(")
    || content.includes("getTokenFromCookie(")
    || content.includes("requireAdmin(")
    || content.includes("requireApiAccess(")
    || content.includes("requireAdminApiAccess(")
    || content.includes("createApiRouteHandler(")
    || content.includes("createCommandRoute(")
  ...
  // 只有文件中存在 403/requireAdmin 时才统计 checkPermission/checkHRRead
  // （auth/me 等文件也调用 checkPermission 但只是查询用户权限，不控制访问）
  if (has403) {
    if (content.includes("checkHRRead(")) checks.push("HR权限");
  ...
  // Never infer public access from the absence of a recognized wrapper. Public access
  // is only reported when the authoritative API contract explicitly declares it.
  if (checks.length === 0) return "未识别（查 API contract）";
      ...
      if (stat.isDirectory()) {
        ...
        for (const method of methods) {
          routes.push({
            ...
            displayPath,
            method,
            auth: extractAuth(content, method as ApiMethod, pathKey),
...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Workspace API Reference</title>
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
...
<body>
<div class="container">
<h1>Workspace API Reference</h1>
<p class="subtitle">${endpoints.length} endpoints across ${groups.size} groups；访问标记来自 API contract 与 route wrapper</p>
<div class="summary">
  <div class="summary-grid">
    ...
    <div class="summary-item"><div class="num">${endpoints.filter((e) => e.method === "PATCH").length}</div><div class="label">PATCH</div></div>
