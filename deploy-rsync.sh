#!/bin/bash
# =============================================================================
# 备份版本：本地构建 + rsync 上传（旧版部署方式）
# =============================================================================
# 说明：这是服务器端构建（deploy.sh）之前的旧版部署脚本。
# 适用于网络环境良好、本地构建后上传的场景。
# 当前服务器端构建版本见 deploy.sh。
#
# 注意：
#   - 此脚本基于 Prisma 5 时代的逻辑编写，若恢复使用需根据 Prisma 7 调整
#   - 当前 REMOTE_DIR 指向 /home/ubuntu/weekly-new（新版目录）
# =============================================================================
...
ssh -i "$KEY" "$SERVER" "sed -i 's|file:/Users/koito/Desktop/Project/[^/]*/prisma/dev.db|file:/home/ubuntu/weekly-new/prisma/dev.db|' $REMOTE_DIR/.next/standalone/.env 2>/dev/null || true"

echo "==> 重启服务..."
ssh -i "$KEY" "$SERVER" "cd $REMOTE_DIR/.next/standalone && export DATABASE_URL='file:/home/ubuntu/weekly-new/prisma/dev.db' && (pm2 restart $PM2_NAME --update-env 2>/dev/null || pm2 start server.js --name $PM2_NAME --cwd $REMOTE_DIR/.next/standalone --env production)"
