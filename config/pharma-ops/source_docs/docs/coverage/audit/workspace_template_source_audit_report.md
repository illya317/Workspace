...
### 中间体
- 2.3 含量 [parents/hplc_content_full]：计算字段未见 JSON 回填：结论-结果

### 待包装品
- 2.4 含量 [parents/hplc_content_full]：MD 子标题不连续：2.4.4→6；JSON 缺 MD 标题：2.4.9 结论 (heading)；计算字段未见 JSON 回填：20片总净重，结论-结果
...
- 2.7 含量 [parents/hplc_content_full]：计算字段未见 JSON 回填：20片总净重，结论-结果
