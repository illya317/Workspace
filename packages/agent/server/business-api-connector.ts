...
import { createAgentApiDelegationToken, AGENT_API_DELEGATION_HEADER } from "@workspace/platform/server/agent-api-delegation";
import {
  discoveryBusinessApiInputSchema,
  mutationBusinessApiInputSchema,
  readBusinessApiInputSchema,
} from "./business-api-input";
...
const discoverBusinessApisTool: AgentTool = {
  ...
  async execute(params) {
    const parsed = discoveryBusinessApiInputSchema.safeParse(params);
...
const readBusinessApiTool: AgentTool = {
  ...
  async execute(params, execution) {
    const parsed = readBusinessApiInputSchema.safeParse(params);
...
const proposeBusinessApiMutationTool: AgentTool = {
  ...
  writeMode: "proposal",
  async execute(params, execution) {
    const parsed = mutationBusinessApiInputSchema.safeParse(params);
...
export function validateAgentBusinessApiMutationProposalPayload(payload: Record<string, unknown>) {
  const parsed = mutationBusinessApiInputSchema.safeParse(payload);
