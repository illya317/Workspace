...
const createContractAdapter = defineBusinessActionCommandAdapter({
  ...
  validate: (input: CreateContractInput) => {
    const command = buildContractCreateCommand(input.body, input.userId);
...
const updateContractAdapter = defineBusinessActionCommandAdapter({
  ...
  validate: (input: UpdateContractInput) => {
    const command = buildContractUpdateCommand(input.id, input.body, input.userId);
    return command.ok ? serviceOk(command.data) : validationError(command.issue);
  },
