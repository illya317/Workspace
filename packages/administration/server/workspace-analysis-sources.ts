...
export const ADMINISTRATION_CONTRACTS_ANALYSIS_SOURCE = defineWorkspaceAnalysisReadModel<AdministrationContractListRow>()({
  ...
  fields: {
    ...
    approvalSourceKey: internal("text", "审批来源", "合同关联审批记录的来源系统键。"),
    approvalRecordId: confidential("text", "审批记录标识", "来源系统中的审批记录稳定标识。"),
    approvalRecordUrl: field({
      label: "审批记录链接",
      description: "来源系统审批记录链接，仅供在线导航，不允许分析导出。",
      valueKind: "text",
      sensitivity: "restricted",
      exportPolicy: "forbidden",
    }),
    approvalStatusSnapshot: internal("text", "审批状态快照", "最近一次同步的来源审批状态。"),
    approvedOn: confidential("date", "审批通过日期", "来源审批记录确认的通过日期。"),
    approvalSyncedAt: internal("date", "审批同步时间", "审批关联信息最近同步到合同台账的时间。"),
    currentRevisionId: internal("integer", "当前修订 ID", "合同当前正式修订的稳定内部 ID。"),
