...
type OfficialTemplateSyncResult = {
  qcSpace: Awaited<ReturnType<typeof ensureQcOfficialTemplates>>;
  hrSpace: Awaited<ReturnType<typeof ensureHrPositionDescriptionOfficialTemplate>>;
};

let officialTemplateSyncInFlight: Promise<OfficialTemplateSyncResult> | null = null;
...
export async function ensureOfficialTemplates(db: DocsEditorDb = docsEditorDb()) {
  if (officialTemplateSyncInFlight) return officialTemplateSyncInFlight;

  officialTemplateSyncInFlight = Promise.all([
    ensureQcOfficialTemplates(db),
    ensureHrPositionDescriptionOfficialTemplate(db),
  ]).then(([qcSpace, hrSpace]) => {
    return { qcSpace, hrSpace };
  }).finally(() => {
    officialTemplateSyncInFlight = null;
  });
  return officialTemplateSyncInFlight;
}
