...
import type { ServiceResult } from "@workspace/platform/server/api";
...
export async function publishDocumentTemplateDraft(userId: number, templateId: number | string, version: number) {
  return unwrapServiceResult(await publishDraft({
    userId,
...
export async function deleteDocumentTemplateDraft(userId: number, templateId: number | string, version: number) {
  return unwrapServiceResult(await deleteDraft({ userId, templateId: numberFromString(templateId) ?? 0, version }));
}

export async function archiveDocumentTemplate(userId: number, templateId: number | string, version: number) {
  return unwrapServiceResult(await archiveTemplate({
    userId,
    templateId: templateIdFromString(templateId),
    version,
  }));
