...
export function actionAccessLabel(actions: EditorSpaceActionPermissions) {
  if (actions.canManagePermissions) return "可授权";
  if (actions.canPublish || actions.canApprove) return "可审批";
  if (actions.canDelete || actions.canArchive) return "可删除";
  if (actions.canCreate || actions.canUpdate || actions.canSubmit) return "可编辑";
  if (actions.canExport) return "可导出";
  return "可查看";
}
