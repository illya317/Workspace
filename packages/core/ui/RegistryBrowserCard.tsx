    ...
    <section className="rounded-lg border border-slate-200 bg-white shadow-sm">
      <div className="border-b border-slate-200 p-5">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {registryBrowserLayers.map((layer) => {
            const active = layer.key === activeLayer;
            const count = items.filter((item) => getRegistryBrowserLayer(item) === layer.key).length;
            return (
              <button
                key={layer.key}
                type="button"
              ...
              >
                {layer.label}
                <span className={active ? "ml-2 text-emerald-100" : "ml-2 text-slate-400"}>{count}</span>
              </button>
            );
          })}
        </div>
