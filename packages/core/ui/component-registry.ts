...
const coreUiComponentRegistryRaw = [
  { name: "ActionButton", tier: "primitive", kind: "toolbar", description: "工具栏图标动作按钮，只接受 ActionGlyph kind + label，统一新增、编辑、刷新、删除等动作入口。", example: "Toolbar、DataTable 行动作和新建确认都传 kind=\"save\" / kind=\"cancel\" 这类 glyph 动作。", composes: ["ActionGlyph"], foundations: ["getToolbarActionClassName"] },
  { name: "ActionGlyph", tier: "primitive", kind: "toolbar", description: "通用动作图标 glyph，按 toolbar 大组、语义小组和全局顺序管理新增、编辑、确认、删除、查看、搜索、刷新、传输、关联、权限、生成和更多操作。", example: "新增 glyph 时进入 ACTION_GLYPH_KINDS 和 ACTION_GLYPH_GROUPS，并在 ACTION_GLYPH_ORDER 写入 icon / group / subgroup / order。" },
  { name: "Toolbar", tier: "assembly", kind: "toolbar", description: "统一工具栏，通过 items 配置视图控制、筛选、操作、编辑和元信息区域，替代分散的 CommandToolbar / FilterToolbar / ActionToolbar 手写组合。", example: "列表页顶部统一配置搜索、状态筛选、新建按钮和分页信息。", composes: ["ActionButton", "SearchInput", "SelectField", "ToolbarOptionGroup"] },
  { name: "ActionToolbar", tier: "assembly", kind: "toolbar", description: "通用页面动作栏，承接主按钮、次按钮和左右插槽；内部复用 Toolbar 统一渲染。", example: "在资料库列表顶部显示“已选择 2 条记录 / 导出 / 新增”。", composes: ["Toolbar", "ActionButton"] },
  { name: "CommandToolbar", tier: "assembly", kind: "toolbar", description: "综合命令栏容器，按隐藏/显示、新建、搜索、互斥筛选、字段值筛选、刷新、页面动作、编辑动作和分页信息分区；一行优先，空间不足自动换行。", example: "列表页同时承载左右分栏控制、新建入口、搜索筛选、导出、编辑保存和分页信息。", composes: ["Toolbar", "SearchInput", "ToolbarOptionGroup", "FieldValueFilter", "ActionButton", "RefreshActionButton", "SelectField"] },
  { name: "FilterToolbar", tier: "assembly", kind: "toolbar", description: "列表筛选工具栏，统一搜索、下拉筛选和操作按钮；内部复用 Toolbar 统一渲染。", example: "在项目列表顶部统一展示搜索、状态下拉和每页条数。", composes: ["Toolbar", "SearchInput", "SelectField", "ActionButton", "ToolbarOptionGroup"] },
  ...
  { name: "PageToolbar", tier: "shell", kind: "toolbar", description: "页面级工具栏接口，通过 features 参数让智能体选择需要展示的区块（标题、视图、搜索、筛选、操作、编辑、元信息、列控制、分页），内部统一复用 Toolbar。", example: "列表页顶部只传 features 数组和业务回调，即可组合出视图切换、搜索、状态筛选、新建、导出、编辑和分页信息。", composes: ["Toolbar", "EditToolbar", "ActionButton", "SelectField", "ToolbarOptionGroup"] },
  { name: "RefreshActionButton", tier: "assembly", kind: "toolbar", description: "工具栏刷新动作按钮，使用无边框图标样式和统一可访问名称。", example: "Toolbar 的刷新入口统一显示刷新图标，而不是文字按钮。", composes: ["ActionButton"] },
  ...
  { name: "CreateConfirmActions", tier: "assembly", kind: "form", description: "新建模式的确认/取消图标动作 primitive，供 InlineCreatePanel、BlockCreatePanel 和工具条新建态共享。", example: "新建态标题旁展示取消和确认按钮，不在业务页手写 x/勾图标。", composes: ["ActionButton"] },
  { name: "CreateStartButton", tier: "assembly", kind: "form", description: "新建模式的 + 入口 primitive，统一普通态、激活态和禁用态。", example: "列表、阶段或任务区进入新建态时，标题旁 + 高亮。", composes: ["ActionButton"] },
  ...
  { name: "DataTableActionsCell", tier: "primitive", kind: "data", description: "表格操作列模板，统一查看、编辑、删除等行级动作图标。", example: "员工资料和批次记录表格都用查看/删除图标，不再手写操作列按钮。", composes: ["ActionButton"] },
  ...
  { name: "DropdownSurface", tier: "primitive", kind: "overlay", description: "下拉浮层行为 primitive，封装触发、展开、外部点击关闭、Escape 关闭和定位。", example: "供 SelectField、DropdownMenu 等组件复用，业务不应直接使用。" },
  { name: "DropdownMenu", tier: "primitive", kind: "overlay", description: "通用下拉菜单 primitive，统一触发按钮、浮层、分隔线和危险动作样式；内部封装 DropdownSurface 下拉浮层行为。", example: "平台用户菜单展示“设置 / 登出”，业务只提供动作列表。", composes: ["DropdownSurface"] },
  ...
  { name: "FieldValueFilter", tier: "assembly", kind: "picker", description: "字段和值组合筛选，触发器显示“字段：值”，点击后再选择字段和值；字段可声明 FK，并由业务域传入 reference-options endpoint。", example: "显示“员工：张文孝”，点击后先选字段，再用 HR reference-options 搜索选择员工。", composes: ["SelectField", "SearchInput", "FkFieldInput", "PickerOptionButton"] },
  ...
  { name: "CreatePanel", tier: "assembly", kind: "form", description: "新建入口薄封装，通过 variant 统一选择 inline / block / modal 三种新建形态，内部复用已有的 InlineCreatePanel / BlockCreatePanel / ModalCreatePanel。", example: "业务页只传 variant 和统一回调，即可在列表顶部、卡片区或弹窗中复用同一套新建逻辑。", composes: ["InlineCreatePanel", "BlockCreatePanel", "ModalCreatePanel"] },
  ...
  { name: "PageStyleShowcase", tier: "frame", kind: "layout", description: "页面样式预览中心，按八大业务板块展示页眉、Tab、Toolbar、主体、页脚、预览和弹出框模板。", example: "架构任务用它审阅财务、生产、人事、工作、行政、外部关系、文档中心和资料库页面样式。", composes: ["TabBar", "Toolbar", "DataTable", "Pagination", "SplitWorkspace", "AnalysisBlock", "StructuredTable", "DatabasePageFrame", "PanelCard"] },
  ...
  { name: "SelectField", tier: "primitive", kind: "form", description: "统一下拉选择字段，支持单选/多选、搜索和不同密度尺寸。", example: "从“现用 / 已归档 / 全部”中选择筛选范围，或用多选切换表格列显隐。", composes: ["SearchInput", "DropdownSurface", "CheckboxField"] },
  ...
  { name: "SplitWorkspace", tier: "assembly", kind: "layout", description: "左右分栏工作区，适合列表加详情的主从工作流。", example: "左侧项目列表，右侧保持当前项目详情编辑区。", composes: ["Toolbar", "ActionButton"] },
  { name: "SplitWorkspaceToolbar", tier: "assembly", kind: "toolbar", description: "分栏工作区工具条，承接折叠、模式和辅助操作；内部复用 Toolbar inline 变体统一渲染。", example: "在左右分栏顶部提供收起列表和保存详情按钮。", composes: ["Toolbar", "ActionButton"] },
