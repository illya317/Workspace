...
export interface SurfaceOptionSpec {
  label: string;
  value: string;
  description?: string;
}
...
export type SurfaceDataRowActionKind = "view" | "add" | "edit" | "save" | "cancel" | "delete" | "archive" | "restore";
...
export interface SurfaceToolbarLabelItem {
  kind: "label";
  key: string;
  label: ReactNode;
}
...
export interface SurfaceToolbarCreateItem {
  /** @deprecated Standard create flows are declared through CreateSurface. */
  kind: "create";
  key: string;
  label?: string;
  active?: boolean;
