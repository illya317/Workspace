...
export type FixedSidebarBodyOptions = {
  left: BodySurfaceProps;
  right: BodySurfaceProps;
};
...
export function createFixedSidebarBody(options: FixedSidebarBodyOptions): BodySurfaceProps {
  return {
    kind: "section",
    layout: "split",
    splitPresentation: "fixed-sidebar",
    left: options.left,
    right: options.right,
    sideOpen: true,
    drawerOpen: false,
    onSideOpenChange: ignoreFixedSidebarToggle,
    onDrawerOpenChange: ignoreFixedSidebarToggle,
    sideLabel: "",
    showSideControls: false,
  };
}

function ignoreFixedSidebarToggle(_open: boolean) {}
...
export function createMessageSection(
  ...
  return { key, chrome: "plain", body: { kind: "section", message } };
}
...
export function createPanelSection(
  ...
  const { actions, chrome, create, disclosure, framed, gridColumns, itemRef, layout = "stack", mobilePresentation, sections, title } = panel;
  return {
    key,
    label: title,
    chrome,
