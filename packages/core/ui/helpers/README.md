...
- Helper names should make the target declaration obvious, for example `createFormBlock` or `createVisualizationBlock`.

Current status: Surface block helpers live here. The root helper files are compatibility re-export shims while imports are migrated.
