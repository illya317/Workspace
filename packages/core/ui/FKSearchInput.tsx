...
export interface FKSearchOption {
  id: number;
  name: string;
  subtitle?: string;
...
export interface FKSearchInputProps {
  fkKey: string;
  value: string;
  displayValue?: string;
  onChange: (value: string, option?: FKSearchOption) => void;
  placeholder?: string;
  disabled?: boolean;
...
export default function FKSearchInput({
  fkKey,
  value,
  displayValue,
  onChange,
  ...
  const [options, setOptions] = useState<FKSearchOption[]>([]);
  ...
  const search = useCallback(
    async (q: string) => {
      ...
      try {
        ...
        const response = await fetch(workspacePath(`/api/hr/autocomplete?${params.toString()}`));
        if (response.ok) {
          const data = (await response.json()) as { items?: FKSearchOption[] };
  ...
  function handleSelect(option: FKSearchOption) {
