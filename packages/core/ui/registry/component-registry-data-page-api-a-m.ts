...
export const page_api_registry_entries = [
  ...
  },
  {
    ...
    declares: [
        ...
        children: [
          ...
          { name: "selector", description: "选择器正文，payload 为 SelectorSurface。" },
          {
            ...
            children: [
              ...
              { name: "master/detail", description: "split 布局只声明主列表与详情 BodySurface；master.label 统一驱动折叠按钮和移动端返回文案，master.presentation=compact 在桌面主栏压缩选择卡并保留一个辅助事实，移动端仍展示完整卡片。" },
            ],
          },
        ],
      },
      ...
      { name: "split", description: "split 专属：master/detail/desktop/mobile；master.presentation 可声明 default / compact，compact 把桌面选择卡收敛为身份、状态和一个辅助事实，并在未声明 desktop 布局时使用固定侧栏；PageSurface 自动持有桌面折叠状态并合并唯一 toolbar 控制。" },
    ],
