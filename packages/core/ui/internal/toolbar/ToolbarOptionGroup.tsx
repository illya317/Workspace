...
export interface ToolbarOptionGroupProps {
  ...
  className?: string;
  size?: ControlSize;
  /** Toolbar-only compactselected values show their label. */
...
export default function ToolbarOptionGroup({
  ...
  const activeTrigger = isAccordion && accordionTrigger === "active";
  const parentOption = activeTrigger
    ? activeOption ?? defaultOption
    : isAccordion && defaultOption
    ? {
        ...defaultOption,
        label: value === defaultOption.value ? (ariaLabel ?? defaultOption.label) : (activeOption?.label ?? ariaLabel ?? defaultOption.label),
      }
    : defaultOption;
  const childOptions = isAccordion
    ? activeTrigger
      ? options.filter((option) => option.value !== parentOption?.value)
      : options.slice(1)
    : options;
      ...
      {visibleOptions.map((option) => {
        ...
        return (
          <button
            ...
            onClick={() => {
              if (activeTrigger && option.value === parentOption?.value) {
            ...
            className={joinClassNames(
              ...
              isAccordion
                ...
                : itemClasses,
              itemText,
              active || (!activeTrigger && isAccordion && option.value === defaultOption?.value && value !== defaultOption.value)
                ? isAccordion
