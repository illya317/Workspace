...
export function sectionCardClassName(position?: BodySectionStackPosition, nested = false) {
  ...
  if (!position || position === "single") return "space-y-4 overflow-clip rounded-xl border border-slate-200 bg-white p-3 sm:rounded-md sm:p-4 sm:shadow-sm";
  const roundedClass =
    position === "first"
      ...
      : position === "last"
        ...
        : "-mt-px rounded-none";
  return joinClassNames("space-y-4 overflow-clip border border-slate-200 bg-white p-3 sm:p-4", roundedClass);
}
