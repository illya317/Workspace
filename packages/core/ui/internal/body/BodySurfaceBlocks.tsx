...
export function renderBodyMessage(message?: BodySurfaceMessageLike) {
  ...
  const content = <>{renderContent(message.content)}{message.link ? <a className="ml-2 font-medium underline" href={message.link.href}>{message.link.label}</a> : null}</>;
  if (message.presentation === "plain") return <div key="message">{content}</div>;
  const toneClass =
  ...
  return <div key="message" className={joinClassNames("rounded-md border px-3 py-2 text-sm", toneClass)}>{content}</div>;
}
