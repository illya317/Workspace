...
test("renders loading status content instead of replacing it with a silent skeleton", () => {
  const body: BodySurfaceComposedSectionProps = {
    kind: "section",
    status: { kind: "loading", content: "正在加载分析数据", compact: true },
  };
  const markup = renderClientSurface(<AntdComposedBody body={body} />);
  assert.match(markup, /正在加载分析数据/);
  assert.match(markup, /ant-skeleton/);
});
...
test("block section create derives a Core anchor before the section body", () => {
  const body: BodySurfaceComposedSectionProps = {
    kind: "section",
    sections: [{
      key: "employees",
  ...
  assert.ok(markup.indexOf('class="contents"') < markup.indexOf("名册内容"));
