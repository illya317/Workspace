...
function documentSlotWidth(part: PaperInputLayoutSpec): CSSProperties {
  return { width: `min(${cssSlotWidth(part.width)}, 100%)`, maxWidth: "100%" };
}

function inputAlignClass(part: PaperInputLayoutSpec) {
...
function inputWidth(part: PaperInputLayoutSpec): CSSProperties {
...
export function paperRangeError(part: PaperInputLayoutSpec, value?: string) {
  const range = part.recommendedRange;
  ...
  if ((range.min != null && num < range.min) || (range.max != null && num > range.max)) {
    return `超出推荐范围 ${paperRangeLabel(part)}`;
  }
  return undefined;
}

function underlineClass(part: PaperInputLayoutSpec, inTable?: boolean) {
  if (inTable) return "border-b-0";
  return part.underline === true ? "border-b border-slate-950" : "border-b-0";
}

function selectRootBorderClass(part: PaperInputLayoutSpec, inTable?: boolean) {
  if (inTable || part.underline !== true) return "!border-0";
  return "border-b border-slate-950";
}

export function PaperLineInput({
  part,
  readOnly,
  value,
  onChange,
  ...
  const error = paperRangeError(part, rawValue);
  ...
  if (part.multiline) {
    return (
      <textarea
        aria-label={part.fieldKey}
        data-field-key={part.fieldKey}
        value={currentValue}
  ...
  return (
    <input
      ...
      placeholder={part.placeholder}
      readOnly={isReadOnly}
      inputMode={part.valueType === "number" ? "decimal" : undefined}
      type="text"
      title={error}
...
function formattedNumberValue(value: string, part: PaperInputLayoutSpec) {
...
export function PaperSelectInput({
  part,
  options = [],
  readOnly,
  value,
  ...
  const error = paperRangeError(part, value ?? part.defaultValue);
...
export function PaperChoiceInput({
  fieldKey,
  options = ["是", "否"],
  type = "radio",
  disabled,
