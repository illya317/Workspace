...
import { Button, Skeleton, Space, Table, type TableColumnsType } from "antd";
import type {
...
import { ACTION_GLYPH_ACTION_BY_KEY, ActionGlyph } from "../action/ActionGlyphs";
...
function columnClassName<T>(column: DataSurfaceColumnSpec<T>, row: T) {
  const font = column.font ?? (column.numeric ? "mono" : undefined);
  return [
    ...
    <Space size={4} wrap>
      {actions.map((action) => (
        <Button
          ...
          danger={ACTION_GLYPH_ACTION_BY_KEY[action.kind].variant === "danger"}
          disabled={action.disabled}
          icon={<ActionGlyph kind={ACTION_GLYPH_ACTION_BY_KEY[action.kind].icon} className="size-4" />}
          key={action.key}
          onClick={(event) => {
          ...
          size="small"
          title={action.label}
          type={ACTION_GLYPH_ACTION_BY_KEY[action.kind].variant === "primary" ? "primary" : "default"}
        />
      ))}
    </Space>
  );
...
function AntdMobileTableList<T>({
  data,
  visible,
  presentation,
}: {
  ...
  const toMobileColumn = (column: DataSurfaceColumnSpec<T>): DataTableColumn<T> => ({
    ...column,
    render: (row: T) => <AntdDataCell value={column.cell(row)} />,
  });
  ...
  return (
    <div className="sm:hidden" role="list" data-mobile-table-presentation="list">
      <div className="divide-y divide-slate-100 bg-white">
        {data.loading ? <div className="px-4 py-6"><Skeleton active /></div> : null}
        {!data.loading && data.rows.length === 0 ? (
          <div className="px-4 py-12 text-center text-sm text-slate-400">
            {data.emptyText ?? data.empty ?? "暂无数据"}
          </div>
        ) : null}
        {!data.loading && data.rows.map((row, index) => {
          ...
          return (
            ...
            >
              ...
              {isExpanded && data.expandedRow ? (
                ...
                >
                  <AntdDataCell value={data.expandedRow(row) ?? null} />
                </div>
              ) : null}
            </article>
          );
...
export function AntdDataTable<T>({ data }: { data: DataSurfaceTableProps<T> }) {
  ...
  const columns: TableColumnsType<AntdRowRecord<T>> = visible.map((column, columnIndex) => ({
    ...
    width: (matrix
      ? matrixWidths[columnIndex] ?? resolveTableColumnWidthValue(column.width ?? (column.numeric ? "sm" : undefined))
      : resolveTableColumnWidthValue(column.width ?? (column.numeric ? "sm" : undefined))) ?? undefined,
    ...
    render: (_value, { row }) => {
      const content = <AntdDataCell value={column.cell(row)} />;
  ...
  const desktopTable = (
    <Table<AntdRowRecord<T>>
      ...
      expandable={data.expandedRow ? {
        ...
        expandedRowRender: ({ row }) => <AntdDataCell value={data.expandedRow?.(row) ?? null} />,
