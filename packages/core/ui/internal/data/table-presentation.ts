...
export function resolveStandardTableMinWidth(columnCount: number, scroll?: DataSurfaceScrollSpec) {
  return scroll?.x === true ? `${columnCount * 8}rem` : undefined;
}
  ...
  const maxHeight = scroll?.maxHeight === "sm"
    ...
    : scroll?.maxHeight === "md"
      ...
      : scroll?.maxHeight === "lg"
        ...
        : "";
  return joinClassNames(
    scroll?.x === true ? "overflow-x-auto" : scroll?.x === false ? "overflow-x-hidden" : "",
...
export function resolveDataTableScroll(
  ...
  if (scroll?.maxHeight) return { x: true, y: "auto", ...scroll };
  return { x: false, y: "hidden", ...scroll };
}
...
export function resolveTablePresentation(
  ...
  options: { rowHover?: DataTablePresentation["rowHover"]; stickyHeader?: boolean } = {},
  ...
  const fillRowControls = presentation?.controlHeight === "fillRow";
  ...
  return {
    ...
    head: joinClassNames(options.stickyHeader ? "sticky top-0 z-10" : "", headGridClassName, headerClassName),
    ...
    cell: joinClassNames(wrapClassName, cellPadding, "min-w-0 max-w-full", cellGridClassName, fillRowControls ? "h-px align-middle" : ""),
    cellContent: joinClassNames("min-w-0 max-w-full overflow-hidden text-ellipsis", fillRowControls ? "flex h-full min-h-[56px] items-stretch" : ""),
