...
function treeBranchLineStyle(level: number): CSSProperties {
  const color = TREE_BRANCH_LINE_COLORS[Math.min(level - 1, TREE_BRANCH_LINE_COLORS.length - 1)];
  return { boxShadow: `inset 3px 0 ${color}` };
}
...
function toTreeData<T>(
  ...
  return items.map((item) => {
    ...
    const branchLineLevel = selected || expandedBranch ? level : inheritedBranchLineLevel;
    ...
    return {
      ...
      style: branchLineLevel ? treeBranchLineStyle(branchLineLevel) : undefined,
