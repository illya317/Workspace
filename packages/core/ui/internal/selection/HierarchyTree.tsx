      ...
      >
        ...
        <span className="min-w-0 flex-1">
          <span className="flex min-w-0 items-baseline gap-2">
            <span className={joinClassNames("min-w-0 flex-1 truncate whitespace-nowrap text-sm font-semibold text-slate-900", titleClassName)} title={textOverflowTitle(title)}>
              {title}
            </span>
            {code && <span className="shrink-0 font-mono text-xs text-slate-400">{code}</span>}
          </span>
          {meta && <span className="mt-0.5 block truncate whitespace-nowrap text-xs text-slate-500" title={textOverflowTitle(meta)}>{meta}</span>}
        </span>
      </button>
