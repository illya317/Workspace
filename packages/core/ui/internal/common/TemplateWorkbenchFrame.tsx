...
import SelectorCard, { type SelectorCardMetaItem } from "../selection/SelectorCard";
    ...
    <div className={joinClassNames("grid gap-4 px-4 py-3 md:grid-cols-[minmax(0,1fr)_auto] md:items-center", row.inset && "pl-9")}>
      <div className="flex min-w-0 items-start gap-3">
        ...
        <div className="min-w-0">
          <div className="truncate font-semibold text-slate-900" title={textOverflowTitle(row.title)}>{row.title}</div>
          {row.description && <div className="mt-1 truncate text-xs text-slate-500" title={textOverflowTitle(row.description)}>{row.description}</div>}
        </div>
      </div>
...
function TemplateSection({ section, onToggle }: { section: SectionView; onToggle: () => void }) {
  const title = (
    <span className="flex min-w-0 items-center gap-3">
      <span className="truncate" title={textOverflowTitle(section.title)}>{section.title}</span>
