...
export default function SplitWorkspace({
  ...
  splitRatio,
  desktopPresentation,
  masterPresentation = "default",
  mobileDetailActive,
  onMobileNavigateToList,
}: SplitWorkspaceProps) {
  const [sideFr, contentFr] = splitRatio ?? [3, 7];
  const splitStyle = {
    "--split-side-fr": `${sideFr}fr`,
    "--split-content-fr": `${contentFr}fr`,
  } as CSSProperties;
  const resolvedDesktopPresentation = desktopPresentation
    ?? (splitRatio ? "ratio" : masterPresentation === "compact" ? "fixed-sidebar" : "ratio");
  const desktopColumns = resolvedDesktopPresentation === "fixed-sidebar"
