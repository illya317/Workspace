...
export function ReadOnlyField({
  ...
  return (
    <FieldShell readOnly disabled={disabled} size={resolvedSize} density={resolvedDensity} className={joinClassNames(fontClass, toneClass, className)} style={style}>
      {onClick ? (
        <button
          type="button"
          onClick={onClick}
        ...
        <div title={title ?? textOverflowTitle(body)} aria-label={ariaLabel} className={joinClassNames(INNER_CLASS_NAME, "truncate", alignClass)}>
          {body}
        </div>
      )}
    </FieldShell>
