...
function getDefaultVisibleCount(optionCount: number) {
  return optionCount > 0 && optionCount < 10 ? optionCount : 5;
}
...
export default function SearchableOptionInput({
  ...
  const resolvedVisibleCount = visibleCount ?? getDefaultVisibleCount(options.length);
  ...
  const filteredOptions = useMemo(() => {
    ...
    const limit = maxResults ?? resolvedVisibleCount;
  ...
  }, [filterQuery, maxResults, options, resolvedVisibleCount]);
