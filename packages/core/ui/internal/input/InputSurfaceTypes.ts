...
import type { SelectionOption, SelectionOptionGroup } from "../selection/SelectionOptionTypes";
...
export type InputOption = SelectionOption & { searchText?: string; subtitle?: string; disabled?: boolean };
export type InputOptionGroup = SelectionOptionGroup;

export type InputChoiceOptions = {
  visibleCount?: number;
  commonValues?: string[];
...
export type InputOptions =
  | { source: "none" }
  | ({ source: "static"; items: InputOption[] } & InputChoiceOptions)
  | ({ source: "grouped"; groups: InputOptionGroup[] } & InputChoiceOptions)
  | {
      source: "remote";
      fkKey: string;
      endpoint: string;
...
export function createInputOption(value: string, label: string, description?: string | null): InputOption {
  return description?.trim() ? { value, label, description } : { value, label };
}
