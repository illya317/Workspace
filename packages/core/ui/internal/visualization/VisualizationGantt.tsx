...
export function VisualizationGanttCanvas({ spec }: { spec: VisualizationGanttSpec }) {
  ...
  useLayoutEffect(() => {
    if (!dependencies.length) {
      setDependencyLines((current) => current.length ? [] : current);
      return undefined;
    }
    const compute = () => {
  ...
  }, [dependencies, periodStartTime, spec.zoom]);
...
function BaselineBar({ row, periodStart, periodEnd }: { row: VisualizationGanttRowSpec; periodStart: Date; periodEnd: Date }) {
  const start = parseGanttDate(row.plannedStartDate);
  const end = parseGanttDate(row.plannedEndDate);
...
function CurrentBar({
  ...
  if (!start || !end) {
    if (!single && (row.plannedStartDate || row.plannedEndDate)) return null;
...
function TimelineHint({ children }: { children: string }) {
  return <span className="absolute top-3 max-w-full truncate text-xs font-medium text-slate-400" title={children}>{children}</span>;
}
...
function isDelayed(row: VisualizationGanttRowSpec) {
  const baselineEnd = parseGanttDate(row.plannedEndDate);
