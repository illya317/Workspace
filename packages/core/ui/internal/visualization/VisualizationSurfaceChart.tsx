...
function renderVisualLegend(
  ...
  placement: "footer" | "header-center" | "header-end" = "footer",
) {
  if (!Array.isArray(legend) || legend.length === 0) return null;
  const placementClass = placement === "footer"
    ? "mt-3 flex flex-wrap items-center gap-4 border-t border-slate-100 pt-3 text-xs text-slate-400"
    : joinClassNames(
        "mb-3 flex flex-wrap items-center gap-4 text-xs text-slate-400",
        placement === "header-end" ? "justify-end" : "justify-center",
      );
  return (
...
function renderGroupedBarChart(visual: VisualizationGroupedBarChartSpec) {
  ...
  const barWidthClass = visual.groups.length <= 4 ? "w-8" : visual.groups.length <= 8 ? "w-4" : "w-2.5";
  const legendPlacement = visual.legendPlacement ?? "footer";
  return (
    <div>
      {visual.title ? <h4 className="mb-2 text-xs text-slate-500">{visual.title}</h4> : null}
      {legendPlacement !== "footer" ? renderVisualLegend(visual.legend, legendPlacement) : null}
      <div className="flex items-stretch gap-1" style={{ height: visual.height ?? 128 }}>
            ...
            <div className="flex min-h-0 flex-1 items-end gap-0.5">
              {group.bars.map((bar) => {
                ...
                return (
                  <div
                    key={bar.key}
                    className={joinClassNames(barWidthClass, "rounded-t", visualToneClass(bar.tone ?? "blue", "bar"))}
                    style={{ height: `${height}%` }}
      ...
      {legendPlacement === "footer" ? renderVisualLegend(visual.legend) : null}
    </div>
  );
}
    ...
    return (
      <div key={node.key} className={depth > 0 ? "ml-6 border-l-2 border-slate-200 pl-4" : ""}>
        ...
        >
          ...
          <span className="min-w-0 flex-1 truncate text-sm font-medium" title={node.label}>{node.label}</span>
