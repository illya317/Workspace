...
export async function searchLibraryDocumentSet(input: { query: string; userId: number; limit?: number }) {
  ...
  const maxConfidentialityLevel = await getMaxConfidentialityLevel(command.userId);
  if (maxConfidentialityLevel <= 0) throw new Error("No Library read permission");
  return queryLibraryDocumentSet({ query: command.query, limit: command.limit, maxConfidentialityLevel });
}
