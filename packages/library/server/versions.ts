...
export interface DocumentVersionInfo extends VersionInfo {
  documentId: number;
}
...
/**
 * Bounded batch reader for callers that have already resolved the visible
 * document set through the Library business read model. Storage locators and
 * file content deliberately remain outside the returned public version DTO.
 */
export async function getDocumentVersionsForDocuments(
  documentIds: readonly number[],
  take: number,
): Promise<DocumentVersionInfo[]> {
  if (!Number.isInteger(take) || take < 1) {
    throw new TypeError("Document version batch size must be a positive integer");
  }
  if (documentIds.some((id) => !Number.isInteger(id) || id < 1)) {
    throw new TypeError("Document version batch contains an invalid document id");
  }
  const uniqueDocumentIds = [...new Set(documentIds)];
  if (uniqueDocumentIds.length === 0) return [];

  return prisma.libraryDocumentVersion.findMany({
    where: { documentId: { in: uniqueDocumentIds } },
    orderBy: [
      { documentId: "asc" },
      { versionNo: "desc" },
      { id: "desc" },
