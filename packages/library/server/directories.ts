...
export async function listDirectories(
  ...
  const documentWhere: Prisma.LibraryDocumentWhereInput = {
    AND: [
      ...(accessWhere ? [accessWhere] : []),
      { rootKey: DEFAULT_ROOT_KEY, currentDirectoryId: { not: null } },
    ],
  };
...
export async function ensureLibraryDirectory(
  ...
  options: { scannedAt?: Date; displayNames?: Record<string, string> } = {},
) {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    for (const path of paths) {
      const name = options.displayNames?.[path] ?? path.split("/").filter(Boolean).at(-1) ?? "/";
