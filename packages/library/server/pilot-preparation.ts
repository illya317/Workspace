...
export type LibraryPilotSummary = {
  catalogCount: number;
  pilotCount: number;
  draftCaseCount: number;
  categories: Record<string, number>;
  fileTypes: Record<string, number>;
  needsOcrOrReview: number;
  formalTaxonomyTagCount: number;
  catalogTagsOutsideTaxonomy: string[];
  approvedCaseCount: number;
};

export type PreparedLibraryPilotArtifacts = {
  summary: LibraryPilotSummary;
  files: Readonly<Record<
    "pilot-manifest.v1.jsonl" | "gold-cases.draft.v1.jsonl" | "gate0-summary.json" | "REVIEW.md",
    string
  >>;
};

function parseCatalogJsonl(text: string): CatalogRecord[] {
  return text.split(/\r?\n/).filter(Boolean).map((line, index) => {
    try {
      return JSON.parse(line) as CatalogRecord;
    } catch (error) {
...
/**
 * Builds the complete deterministic Phase 0 pilot artifact set from Library-owned data.
 * Callers only provide in-memory source data and persist the returned files.
 */
export function prepareLibraryPilotArtifacts(input: {
  catalogJsonl: string;
  taxonomy: LibraryTaxonomy;
}): PreparedLibraryPilotArtifacts {
  const records = parseCatalogJsonl(input.catalogJsonl);
  if (records.length === 0) throw new Error("Catalog is empty");

  const formalTags = new Set(Object.values(input.taxonomy.dimensions).flat());
  ...
  const uniqueCatalogChecksums = new Set(records.map((record) => record.sha256)).size;

  if (pilot.length !== 30) {
    throw new Error(`Pilot must contain 30 unique checksums, got ${pilot.length} from ${uniqueCatalogChecksums} unique catalog records`);
  }
  if (new Set(pilot.map((record) => record.sha256)).size !== pilot.length) {
    throw new Error("Pilot contains duplicate checksums");
  }
  ...
  const summary: LibraryPilotSummary = {
