...
import type { LibraryDocumentAccessPolicy } from "./permissions";
...
export async function queryLibraryDocumentSet(input: {
  query: string;
  limit: number;
  accessPolicy: LibraryDocumentAccessPolicy;
}) {
  const terms = extractLibrarySearchTerms(input.query);
  const candidateRows = await readRankedDocumentCandidates({
    query: input.query,
    terms,
    maxConfidentialityLevel: input.accessPolicy.maxConfidentialityLevel,
    deniedGeneratorKeys: input.accessPolicy.deniedGeneratorKeys,
  });
