...
import type { LibraryDocument, Prisma } from "@workspace/platform/server/prisma";
import {
  addAndConditions,
  buildFilterWhere,
...
function buildWhere(filters: ListFilters, accessWhere?: Prisma.LibraryDocumentWhereInput) {
  ...
  if (accessWhere) andConditions.push(accessWhere as Record<string, unknown>);
...
export async function listDocuments(
  filters: ListFilters,
  accessWhere?: Prisma.LibraryDocumentWhereInput,
): Promise<DocumentListResult> {
  const where = buildWhere(filters, accessWhere);
