...
import { archiveDocument, deleteDocumentPermanently, reviewDocument, updateDocument, uploadDocumentVersion, useDocumentDetail, useLibraryDocumentVersions } from "../hooks/useLibraryDocuments";
import {
...
export default function LibraryDocumentReader({
  ...
  const pdfPreviewUrl = isPdf && activeVersionId
    ? workspacePath(`/api/modules/library/basic-info/documents/${documentId}/versions/${activeVersionId}/preview`)
    : null;
    ...
    : officeViewerUrl
      ...
      : isPdf && pdfPreviewUrl
        ? createDocumentSection("library-pdf-preview", {
            kind: "viewer",
            viewer: { src: pdfPreviewUrl, title: `${doc?.title || doc?.fileName || "资料"} PDF 预览` },
          })
  ...
  const body = doc
    ? createMasterDetailBody({
        master: { label: "资料信息", body: createPageBody([
