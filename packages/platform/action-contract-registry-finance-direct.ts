...
import { FINANCE_ASSET_ACTION_CONTRACT_METADATA } from "./business-action-finance-assets-interface";

const d = (validatorKey: string, commitKey: string): ActionMutationDomainBindingReference => ({ validatorKey, commitKey });
const financeSharedValidation = (name: string) => `packages/finance/server/domain/shared-validation.${name}`;
const financeLedgerValidation = (name: string) => `packages/finance/server/ledger/validation.${name}`;
const financeBudgetValidation = (name: string) => `packages/finance/server/budget/validation.${name}`;
...
export const FINANCE_DIRECT_ACTION_CONTRACT_METADATA = defineActionContractMetadataList([
  ...
  registeredWrite({ key: "finance.ledger.account.update", activeEntity: "FinanceAccount", domain: d(financeLedgerValidation("buildFinanceAccountUpdateCommand"), "packages/finance/server/ledger/accounts.updateFinanceAccount") }),
  registeredLifecycle({ key: "finance.ledger.account.delete", activeEntity: "FinanceAccount", operation: "delete", domain: d(financeSharedValidation("buildFinanceIdCommand"), "packages/finance/server/ledger/accounts.deleteFinanceAccount"), referencePolicy: "domain" }),
  registeredWrite({ key: "finance.ledger.voucher.create", activeEntity: "FinanceVoucher", domain: d(financeLedgerRouteCommand("buildCreateVoucherCommand"), financeLedgerRouteCommand("executeCreateVoucherCommand")), shape: "full_record", target: "new_record", commitMode: "activate" }),
  registeredWrite({ key: "finance.ledger.voucher.update", activeEntity: "FinanceVoucher", domain: d(financeLedgerRouteCommand("buildUpdateVoucherCommand"), financeLedgerRouteCommand("executeUpdateVoucherCommand")) }),
  registeredLifecycle({ key: "finance.ledger.voucher.delete", activeEntity: "FinanceVoucher", operation: "delete", domain: d(financeSharedValidation("buildFinanceIdCommand"), financeLedgerRouteCommand("executeDeleteVoucherCommand")), referencePolicy: "domain" }),
  registeredWrite({ key: "finance.ledger.period.create", activeEntity: "FinancePeriod", domain: d(financeLedgerValidation("buildFinancePeriodCreateCommand"), "packages/finance/server/ledger/periods.createFinancePeriod"), shape: "full_record", target: "new_record", commitMode: "activate" }),
  registeredWrite({ key: "finance.ledger.period.update", activeEntity: "FinancePeriod", domain: d(financeLedgerValidation("buildFinancePeriodUpdateCommand"), "packages/finance/server/ledger/periods.updateFinancePeriod") }),
  registeredLifecycle({ key: "finance.ledger.period.delete", activeEntity: "FinancePeriod", operation: "delete", domain: d(financeSharedValidation("buildFinanceIdCommand"), "packages/finance/server/ledger/periods.deleteFinancePeriod"), referencePolicy: "domain" }),
  registeredWrite({ key: "finance.ledger.defaultBook.create", activeEntity: "FinanceLedgerDefaultBook", domain: d(financeLedgerRouteCommand("buildInitializeFinanceDefaultsCommand"), financeLedgerRouteCommand("executeInitializeFinanceDefaultsCommand")), shape: "full_record", target: "new_record", commitMode: "activate" }),
  registeredWrite({ key: "finance.ledger.balance.revise", activeEntity: "FinanceAccountBalance", domain: d(financeLedgerValidation("buildBalanceComputeCommand"), financeLedgerRouteCommand("executeRecomputeFinanceBalancesCommand")), shape: "change_set", commitMode: "native_transition" }),
  {
  ...
  registeredWrite({ key: "finance.ledger.reclassRule.save", activeEntity: "FinanceReclassRule", domain: d(financeLedgerRouteCommand("buildSaveReclassRuleChangeSetRouteCommand"), financeLedgerRouteCommand("executeSaveReclassRuleChangeSetRouteCommand")), shape: "change_set", target: "mixed", commitMode: "native_transition" }),
  registeredWrite({ key: "finance.ledger.reclassAdjustment.save", activeEntity: "FinanceBalanceReclassAdjustment", domain: d(financeLedgerRouteCommand("buildSaveBalanceReclassAdjustmentChangeSetRouteCommand"), financeLedgerRouteCommand("executeSaveBalanceReclassAdjustmentChangeSetRouteCommand")), shape: "change_set", target: "existing_record", commitMode: "native_transition" }),
  registeredWrite({ key: "finance.ledger.reclassResult.generate", activeEntity: "ReclassResult", domain: d(financeLedgerRouteCommand("buildBuildReclassResultsCommand"), financeLedgerRouteCommand("executeBuildReclassResultsCommand")), shape: "change_set", target: "mixed", commitMode: "native_transition" }),
  registeredWrite({ key: "finance.ledger.reclassResult.adjust", activeEntity: "ReclassResult", domain: d(financeLedgerRouteCommand("buildReclassResultPatchCommand"), financeLedgerRouteCommand("executeReclassResultPatchCommand")), commitMode: "native_transition" }),
  registeredWrite({ key: "finance.ledger.close.open", activeEntity: "FinanceCloseRun", domain: d("packages/finance/server/close/route-commands.buildOpenFinanceCloseRouteCommand", "packages/finance/server/close/route-commands.executeOpenFinanceCloseRouteCommand"), shape: "full_record", target: "new_record", commitMode: "activate" }),
  ...
  registeredLifecycle({ key: "finance.ledger.close.complete", activeEntity: "FinanceCloseRun", operation: "approve", versionKey: "expectedVersion", domain: d("packages/finance/server/close/route-commands.buildCompleteFinanceCloseRouteCommand", "packages/finance/server/close/route-commands.executeCompleteFinanceCloseRouteCommand"), auditPolicy: "event" }),
  ...
  registeredWrite({ key: "finance.statements.consolidationScope.save", activeEntity: "FinanceConsolidationScopeSelection", domain: d("packages/finance/server/domain/consolidation-scope-selection-validation.buildSaveFinanceConsolidationScopeSelectionCommand", "packages/finance/server/statements/consolidation-scope-selections.saveFinanceConsolidationScopeSelection"), shape: "change_set", target: "mixed", commitMode: "native_transition" }),
  ...
  registeredLifecycle({ key: "finance.cost.import.delete", activeEntity: "FinanceCostImport", operation: "delete", domain: d(financeCostRouteCommand("buildFinanceActorRouteIdCommand"), financeCostRouteCommand("executeDeleteCostImportCommand")), referencePolicy: "domain" }),
