...
- 提供 `@workspace/platform/server/auth` 作为认证和平台权限检查契约；旧 `lib/auth.ts` 只保留兼容 re-export
- 提供 `@workspace/platform/server/crud-factory` 作为字段级 CRUD route helper 契约；旧 `lib/crud-factory.ts` 只保留兼容 re-export
...
Platform 可以读取业务包的注册信息，但不能直接 import 业务页面或业务 service。业务包需要认证和权限时依赖 `@workspace/platform/server/auth`，需要通用 CRUD helper 时依赖 `@workspace/platform/server/crud-factory` 并在本领域封装，需要数据库访问时依赖 `@workspace/platform/server/prisma`，需要审计快照时依赖 `@workspace/platform/server/history`，需要 FK 快照显示名时依赖 `@workspace/platform/server/resolve-fk`；不要直接依赖 app-root `@/lib/auth`、`@/lib/crud`、`@/lib/prisma`、`@/lib/history`、`@/lib/resolve-fk` 或 generated Prisma client。
