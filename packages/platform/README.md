...
- 提供 `@workspace/platform/server/workspace-analysis-definition-compiler`、`workspace-analysis-source-directory` 与 `workspace-analysis-source-rpc`，分别负责严格 v3 DSL 编译、显式 provider 组合和单体内部签名 discovery/execute；业务权限仍由 source owner 每次按 requester + target scope 判定
...
经营分析的“轻代码”是受控 JSON DSL，不是 JavaScript/SQL 运行环境。Platform 只拥有登记、验证和执行编排接口；各业务域拥有自己的数据源 registration、字段业务含义和 transport adapter。登记项必须固定版本；`sourceKey` 第一段等于规范化后的 owner module（camelCase 会转 kebab-case），并与真实 GET API contract 的 owner、resource、actions、runtime enforcement 和 projection 一致。
