...
export const OPERATIONAL_BUSINESS_ACTION_REGISTRATIONS = [
  ...
  { ...ADMINISTRATION_CONTRACTS, ...PERMISSION_ONLY, key: "administration.contract.revision.create", label: "创建合同修订", writeKind: "revise", targetKind: "ContractRevision", directPermissionAction: "update", apiRoutes: [route("POST", "/api/modules/administration/contracts/:id/revisions")] },
  { ...ADMINISTRATION_CONTRACTS, ...PERMISSION_ONLY, key: "administration.contract.revision.publish", label: "发布合同修订", writeKind: "publish", targetKind: "ContractRevision", directPermissionAction: "update", apiRoutes: [route("POST", "/api/modules/administration/contracts/:id/revisions/:revisionId/publish")] },
  { ...ADMINISTRATION_CONTRACTS, ...PERMISSION_ONLY, key: "administration.contract.state.transition", label: "变更合同状态", writeKind: "save", targetKind: "ContractStateEvent", directPermissionAction: "update", apiRoutes: [route("POST", "/api/modules/administration/contracts/:id/state-events")] },
  { ...ADMINISTRATION_CONTRACTS, ...PERMISSION_ONLY, key: "administration.contract.state.reverse", label: "冲销合同状态事件", writeKind: "revise", targetKind: "ContractStateEvent", directPermissionAction: "update", apiRoutes: [route("POST", "/api/modules/administration/contracts/:id/state-events/:eventId/reverse")] },
  ...
  { ...EXTERNAL_CUSTOMERS, ...PERMISSION_ONLY, key: "external.customers.party.delete", label: "停用客户角色", writeKind: "delete", targetKind: "ExternalPartyRole", directPermissionAction: "delete", apiRoutes: [route("DELETE", "/api/modules/external/customers/:id")], notes: "DELETE 只追加角色结束期间，不删除角色、来源映射或 Party。" },
  { ...EXTERNAL_CUSTOMERS, ...PERMISSION_ONLY, key: "external.customers.party.availability.change", label: "登记客户角色可用期间", writeKind: "save", targetKind: "ExternalPartyRole", directPermissionAction: "update", apiRoutes: [route("POST", "/api/modules/external/customers/:id/availability")] },
  ...
  { ...HR_ROSTER, ...PERMISSION_ONLY, key: "hr.roster.employmentAgreementAttachment.remove", label: "移除员工协议附件", writeKind: "archive", targetKind: "EmploymentAgreementAttachment", directPermissionAction: "update", apiRoutes: [route("POST", "/api/modules/hr/roster/employee-profiles/:id/agreements/:agreementUid/attachments/:attachmentUid/remove")] },
  { ...HR_ROSTER, ...PERMISSION_ONLY, key: "hr.roster.employeePeriod.correct", label: "修正人员任职资料", writeKind: "update", targetKind: "EmployeeTemporalPeriod", directPermissionAction: "update", apiRoutes: [route("PATCH", "/api/modules/hr/roster/employee-profiles/:id/periods/:periodId")] },
  { ...HR_ROSTER, ...PERMISSION_ONLY, key: "hr.roster.employeeAssignment.create", label: "新增员工任职记录", writeKind: "create", targetKind: "EDP", directPermissionAction: "update", apiRoutes: [route("POST", "/api/modules/hr/roster/employee-profiles/:id/assignments")] },
  ...
  { ...HR_ROSTER, ...PERMISSION_ONLY, key: "hr.roster.employment.update", label: "修正员工雇佣资料", writeKind: "update", targetKind: "Employment", directPermissionAction: "update", apiRoutes: [route("PUT", "/api/modules/hr/roster/employments")] },
  { ...HR_ROSTER, ...PERMISSION_ONLY, key: "hr.roster.employment.create", label: "新增员工雇佣期间", writeKind: "create", targetKind: "Employment", directPermissionAction: "update", apiRoutes: [route("POST", "/api/modules/hr/roster/employments")] },
