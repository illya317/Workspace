...
test("every registered L2 has an explicit mobile product strategy", () => {
  assert.equal(mobileExperienceEntries.length, 30);
  ...
  assert.equal(mobileExperienceEntries.filter((child) => child.mobileExperience.strategy === "unavailable").length, 2);
});
...
test("resolves L2 and deep-route mobile strategies by the longest matching path", () => {
  ...
  assert.equal(resolveMobileExperience("/finance/import").strategy, "unavailable");
