...
export function withOpenApiConsoleAccess(handler: AuthHandler) {
  return withOpenApiConsolePermission("read", handler);
}

export function withOpenApiConsoleManage(handler: AuthHandler, action: PermissionActionKey = "update") {
  return withAuth(
    handler,
    async (userId) => {
      const [canReadConsole, canManageClients] = await Promise.all([
        authorize({ user: userId, resourceKey: "settings.api", action: "read" }),
