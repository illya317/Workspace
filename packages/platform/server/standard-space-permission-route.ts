...
import { listNaturalSpacePermissions } from "./business-space-permissions";
...
export async function getStandardSpacePermissionGrantData<TGrantData extends object>(
  ...
  const isSystemAdmin = await isSuperAdmin(command.userId);
  const [grantData, natural] = await Promise.all([
    config.loadPermissionGrantData("user", command.resourceKey, command.scopeId, {
      ...
      projection: "space",
      canMutateGrantAction: isSystemAdmin,
    }),
  ...
  return serviceOk({
    ...
    canManageUserGrants: isSystemAdmin,
  });
}
