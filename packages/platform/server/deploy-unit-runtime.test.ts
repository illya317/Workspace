...
import { deployUnitHealthResponse } from "./deploy-unit-runtime";
...
test("deploy unit health reports ready when the runtime config path is traversable", async () => {
  const directory = mkdtempSync(join(tmpdir(), "workspace-health-ready-"));
  try {
    await withWorkspaceConfigDir(directory, async () => {
      const response = await deployUnitHealthResponse();
...
test("deploy unit health fails closed when the runtime config path cannot be traversed", async () => {
