...
export interface GuardedDeleteInput {
  ...
  lifecyclePolicy?: "protect" | "allow_protected";
  transactionIsolation?: "default" | "serializable";
...
function blockedByLifecycle(record: RecordData, mode: DeleteMode, actionLabel: string): DeleteGuardResult | null {
  const blockedFields = [
    ["deletedAt", "已删除"],
    ["isArchived", "已归档"],
    ["lockedAt", "已锁定"],
  ...
  if (mode !== "archive" && "status" in record && PROTECTED_STATUS_VALUES.has(String(record.status ?? "").trim())) {
...
function assertDeleteModeFields(
  record: RecordData,
  input: GuardedDeleteInput,
  mode: DeleteMode,
  actionLabel: string,
): DeleteGuardResult | null {
  if (mode === "soft" && !("deletedAt" in record)) {
    return { ok: false, error: `不能${actionLabel}，目标未定义软删字段 deletedAt`, status: 500 };
  }
  const archiveField = input.archiveField?.field ?? "isArchived";
  if (mode === "archive" && !(archiveField in record)) {
    return { ok: false, error: `不能${actionLabel}，目标未定义归档字段 ${archiveField}`, status: 500 };
  }
...
function assertDeleteContract(input: GuardedDeleteInput, mode: DeleteMode, actionLabel: string): DeleteGuardResult | null {
  ...
  if (mode === "archive" && input.referencePolicy !== "checked" && input.referencePolicy !== "none" && input.referencePolicy !== "retained") {
    return { ok: false, error: `不能${actionLabel}，归档必须声明引用检查、显式无引用或按聚合保留引用`, status: 500 };
  }
  if (mode === "archive" && input.referencePolicy === "checked" && !input.references?.length) {
    return { ok: false, error: `不能${actionLabel}，归档声明引用检查但未提供引用策略`, status: 500 };
  }
  if (mode === "archive" && input.referencePolicy === "none" && input.references?.length) {
    return { ok: false, error: `不能${actionLabel}，归档不能同时声明无引用和引用策略`, status: 500 };
  }
...
async function applyDelete(
  ...
  if (mode === "soft") {
    ...
    const archiveField = input.archiveField?.field ?? "isArchived";
    const archiveValue = input.archiveField && "value" in input.archiveField ? input.archiveField.value : true;
    await update({
      where: { id: input.id },
      data: {
...
export async function guardedDelete(input: GuardedDeleteInput): Promise<DeleteGuardResult> {
  ...
  const actionLabel = input.actionLabel ?? `${mode === "archive" ? "归档" : mode === "deactivate" ? "停用" : "删除"}${input.entityType}`;
  ...
  try {
    const operation = async (tx: PrismaTx): Promise<DeleteGuardResult> => {
      ...
      const lifecycleBlock = input.lifecyclePolicy === "allow_protected" ? null : blockedByLifecycle(record, mode, actionLabel);
      if (lifecycleBlock) return lifecycleBlock;
      const modeBlock = assertDeleteModeFields(record, input, mode, actionLabel);
    ...
    return input.transactionIsolation === "serializable"
      ? await runSerializableTransaction(operation)
