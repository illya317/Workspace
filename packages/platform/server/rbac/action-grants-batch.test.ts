...
const rootActorUserIds = new Set<number>([7]);
...
mock.module("../auth/root", {
  namedExports: {
    ...
    isRootAdminUser: async (_userId: number, client: { __txId?: number }) => {
      ...
      return rootActorUserIds.has(_userId);
    },
  },
} as never);
...
test("direct user grant creation requires a live root actor", async () => {
  resetState();
  rootActorUserIds.clear();
  await assert.rejects(
...
test("callback denial occurs after tx-only preflight and leaves target rows and ledger untouched", async () => {
  ...
  assert.equal(txOperations.some((operation) => operation.type === "root"), true);
