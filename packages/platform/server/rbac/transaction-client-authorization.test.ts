...
test("lock-time Agent authorization uses only the injected transaction client through the full RBAC chain", async () => {
  ...
  for (const value of [true, false]) {
    const directUserAuthorization = await authorizePermissionGrantRequest({
      actorUserId: 7,
      subjectType: "user",
      subjectId: 9,
      resourceKey: "agent.source",
    ...
    assert.equal(directUserAuthorization.ok, false);
    if (!directUserAuthorization.ok) assert.equal(directUserAuthorization.status, 403);
  }
});
