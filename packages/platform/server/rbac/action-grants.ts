...
import { runPermissionReviewAfterGrantMutation } from "../permission-review";
...
async function assertDirectUserGrantActor(
  changes: readonly SubjectPermissionActionGrantChange[],
  actorUserId: number | undefined,
  client: Prisma.TransactionClient,
) {
  if (!changes.some((change) => change.subjectType === "user" && change.value)) return;
  if (!actorUserId || !await isRootAdminUser(actorUserId, client)) {
    throw new PermissionGrantMutationError("个人直授仅限内置 admin 账号维护", 403);
  }
}
...
export async function setSubjectPermissionActionGrants(
  ...
  const results = await prisma.$transaction(async (tx) => {
    ...
    await assertDirectUserGrantActor(changes, opts.actorUserId, tx);
  ...
  const changed = changes.filter((_change, index) => results[index]?.changed);
  if (changed.length > 0) await runPermissionReviewAfterGrantMutation(changed);
  return results;
}
