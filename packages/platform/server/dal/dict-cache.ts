...
export const getCompanies = unstable_cache(
  async () => {
    const rows = await prisma.company.findMany({ include: { party: true }, orderBy: { code: "asc" } });
    return rows.map(({ party, ...company }) => ({
      ...company,
      name: party.name,
      fullName: party.fullName,
      unifiedCode: party.identityNumber,
      legalPerson: party.legalRepresentative,
    }));
  },
  ["companies"],
