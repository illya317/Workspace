...
export async function getAuditLogEntries(
  ...
  type VersionWithEditor = Prisma.EditHistoryGetPayload<{ include: { editor: { select: { employees: { select: { name: true }; take: 1 } } } } }>;
  ...
  const entries: AuditLogEntry[] = pageVersions
    ...
    .map((version) => {
      ...
      return {
        ...
        editorName: userEmployeeName(version.editor) || "未知人员",
