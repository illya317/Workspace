...
const KEYWORD_RULES: Array<{
  ...
  {
    patterns: [/源码|代码|架构|实现|怎么做|怎么接|CNB|GitHub|仓库|resource|RBAC|权限|API|路由|route|registry|module-registry/i],
    tool: "source.searchWorkspaceCode",
    extractParams: (input) => ({ query: input }),
  },
  {
    patterns: [
      /资料库/i,
      /^(?!.*(?:员工|人员|人事|工号|花名册)).*(?:找|搜|查|检索).*(?:文件|文档|报告|制度|资料)/,
      /^(?!.*(?:员工|人员|人事|工号|花名册)).*(?:文件|文档|报告|制度|资料).*(?:在哪|下载|给我)/,
    ],
    tool: "library.searchDocuments",
    extractParams: (input) => ({
      query: input
        .replace(/^(?:帮我|请|麻烦)?\s*(?:在\s*)?(?:资料库(?:中)?\s*)?(?:找|搜索|搜|查询|查找|检索)?(?:一下)?\s*/, "")
        .replace(/(?:并)?提供下载|(?:并)?下载(?:给我)?|给我$/g, "")
        .replace(/相关(?:的)?(?:文件|文档|报告|制度|资料)?$/g, "")
        .trim() || input.trim(),
    }),
  },
  // 写入必须在最前面，优先于查询
