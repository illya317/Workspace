...
test("runtime audit snapshot binds a run to immutable responsibility and capability evidence", () => {
  ...
  assert.match(snapshot.runtimeConfigJson ?? "", /"profileKey":"workspace\.business-assistant"/);
  assert.match(snapshot.runtimeConfigJson ?? "", /"displayName":"Workspace 提案助理"/);
  ...
  assert.match(snapshot.runtimeConfigJson ?? "", /"actorEmployeeId":"AI0004"/);
  assert.match(snapshot.runtimeConfigJson ?? "", /"actorEmployeeName":"Workspace 提案助理"/);
