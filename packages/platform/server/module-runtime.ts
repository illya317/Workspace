...
export function disabledApiResponseForRequest(request: Request) {
  ...
  const resourceKeys = [contract.resourceKey, contract.authorization.resourceKey, contract.ownerResourceKey].filter((key): key is string => Boolean(key));
  ...
  const resourceKey = contract.resourceKey ?? contract.authorization.resourceKey ?? contract.ownerResourceKey;
