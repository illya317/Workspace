...
export type DocsEditorTemplateStorageTemplate = Pick<
  DocsEditorTemplateRow,
  "id" | "title" | "type" | "status" | "ownerUserId" | "sourceKind" | "sourceProductKey" | "version"
>;
...
export async function readTemplateEditableContent(input: DocsEditorTemplateStorageContext & {
  row: DocsEditorTemplateContentRow;
}) {
  const draft = await readTemplateDraftContentJson(input);
  const content = draft ?? await readTemplateContentJson(input.row);
  return {
    document: parseJson(content.documentJson, {}),
    fieldModel: parseJson(content.fieldModelJson, {}),
  };
...
export async function readTemplateDraftContentJson(input: DocsEditorTemplateStorageContext) {
  const refs = await planTemplateContentRefs({ ...input, mode: "draft" });
  const [documentJson, fieldModelJson] = await Promise.all([
    readContentRef(refs.documentContentRef),
    readContentRef(refs.fieldModelContentRef),
  ]);
  if (documentJson === null && fieldModelJson === null) return null;
  return {
    documentJson: documentJson || EMPTY_JSON,
    fieldModelJson: fieldModelJson || EMPTY_JSON,
  };
...
export async function planTemplateContentRefs(input: DocsEditorTemplateStorageContext & {
  ...
  const targetRef = input.mode === "draft"
    ? `${baseRef}/drafts/v${padId(input.template.version)}`
...
export async function deleteTemplateDraftContentFiles(input: DocsEditorTemplateStorageContext) {
  await deleteTemplateContentFiles(await planTemplateContentRefs({ ...input, mode: "draft" }));
}
...
async function resolveSpaceLabel(targetType: "department" | "personal" | "company" | "committee", targetId: number, fallback: string) {
  ...
  if (targetType === "personal") {
    ...
    return slug(user?.username || user?.employees[0]?.name || fallback || "user");
  }
  const company = await prisma.company.findUnique({
    where: { id: targetId },
    select: { party: { select: { name: true } } },
  });
  return slug(company?.party.name || fallback || "company");
}
...
async function nextVersionLabel(baseRef: string, now: Date) {
  const datePrefix = businessYymmdd(now);
  const fs = await runtimeFs();
...
function businessYymmdd(now: Date) {
