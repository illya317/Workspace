...
export async function syncGeneratedQcTemplates(input: {
  ...
  const snapshotKey = entries.map((entry) => (
    `${entry.productKey}:${entry.generatedAt?.getTime() ?? ""}:${entry.fileMtimeMs}:${entry.fileSize}`
  )).join("|");
...
async function upgradeUserEditedGeneratedQcTemplate(
  ...
  const upgraded = upgradeGeneratedQcUserContent({
    ...
    resultSuffixUpgradeRules: getTenantProfile().docs.resultSuffixUpgradeRules,
  });
...
function generatedQcProductsRoot() {
  return path.join(resolveTenantConfigPath(getTenantProfile().directories.qcTemplateSnapshots), "products");
}

function generatedQcAuditPath() {
  return path.join(resolveTenantConfigPath(getTenantProfile().directories.qcTemplateSnapshots), "audit.json");
}
...
async function listGeneratedQcIndex(): Promise<GeneratedQcIndexEntry[]> {
  const files = (await readdir(generatedQcProductsRoot()).catch(() => []))
    ...
    .sort();
  const officialProductKeys = qcOfficialTemplateProductKeys();
  const expectedKeys = new Set<string>(officialProductKeys);
  const entries = (await Promise.all(files.map(async (file) => {
    const filePath = path.join(generatedQcProductsRoot(), file);
    const [header, fileStat] = await Promise.all([
      readGeneratedQcHeader(filePath),
  ...
  const missing = officialProductKeys.filter((key) => !actualKeys.has(key));
  const extra = entries.map((entry) => entry.productKey).filter((key) => !expectedKeys.has(key));
  if (missing.length > 0 || extra.length > 0) {
    throw new Error(`官方 QC 模板快照必须固定为 ${officialProductKeys.length} 个产品：missing=${missing.join(",") || "-"} extra=${extra.join(",") || "-"}`);
  }
