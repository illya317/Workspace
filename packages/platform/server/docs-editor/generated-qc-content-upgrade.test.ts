...
test("upgrades user-edited QC content without replacing unrelated edits", () => {
  const document = {
    ...
    blocks: [{ align: "left", parts: [
      { type: "text", text: "样 1=" },
      { type: "formulaSlot", fieldKey: "intermediate/content/pantoprazole_uv/yang_1_han_liang" },
      { type: "text", text: "×100%=" },
    ] }],
  };
  ...
  const result = upgradeGeneratedQcUserContent({ productKey: "pantoprazole", document, fieldModel, sourceFieldModel });
...
test("upgrade is idempotent and keeps formula-process multipliers", () => {
  ...
  const result = upgradeGeneratedQcUserContent({ productKey: "pantoprazole", document, fieldModel, sourceFieldModel: fieldModel });
...
test("centers summary results across QC products but leaves calculation rows aligned", () => {
  ...
  const result = upgradeGeneratedQcUserContent({ productKey: "atenolol", document, fieldModel: { fields: {} }, sourceFieldModel: { fields: {} } });
