...
export async function listDirectManagerUserIds(userId: number) {
  ...
  const reportToPositionIds = Array.from(new Set(
    employees.flatMap((employee) => employee.positions.flatMap((position) => (
      position.reportToPositionId ? [position.reportToPositionId] : []
    ))),
  ));
  if (reportToPositionIds.length === 0) return [];

  const managers = await prisma.employee.findMany({
    where: {
      userId: { not: null },
      employments: { some: currentEmploymentDateWhere() },
      positions: { some: currentOpenEndedDateWhere({ positionId: { in: reportToPositionIds } }) },
    },
    select: { userId: true },
    orderBy: { employeeId: "asc" },
  });
...
function getAdministrativeDepartments() {
  const departmentCodes = getTenantProfile().organization.administrativeDepartmentCodes;
