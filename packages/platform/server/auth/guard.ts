...
export async function requireResourceAccess(resourceKey: string): Promise<SessionUser> {
  ...
  if (
    ...
    !(await canEnterResource(user.id, resourceKey)) &&
    !(await authorize({ user, resourceKey, action: "entry" }))
  ) redirect("/portal");
  return user;
}
...
export async function requireAnyResourceAccess(resourceKeys: string[]): Promise<SessionUser> {
  ...
  for (const resourceKey of resourceKeys) {
    ...
    if (await canEnterResource(user.id, resourceKey)) return user;
    if (await authorize({ user, resourceKey, action: "entry" })) return user;
  }
  redirect("/portal");
}

/**
 * Require the user to manage authorization or configuration for at least one resource.
 * Resource grant/config admins can enter /settings/admin without broad system access.
 */
export async function requireAdminManageAccess(): Promise<SessionUser> {
  ...
  if (
    (user.manageableResourceKeys?.length ?? 0) === 0 &&
    (user.adminResourceKeys?.length ?? 0) === 0
  ) redirect("/portal");
  return user;
}
