...
export const ADDITIONAL_ACTION_CONTRACT_METADATA = defineActionContractMetadataList([
  ...
  ...PRODUCTION_PRODUCTS_ACTION_CONTRACT_METADATA,
  ...INVENTORY_RECEIPT_ACTION_CONTRACT_METADATA,
  ...
  },
  {
    ...
    workflow: {
      ...
      validateOn: ["draft", "submit", "commit"],
      notes: "默认流程为员工自评 -> 直属上级评分 -> HR 最终评分归档；WorkflowPolicy 可关闭新入口，但关闭后动作 unavailable、不得 direct，已有请求继续按创建时快照处理。",
    },
    display: {
