...
export const ADDITIONAL_ACTION_CONTRACT_METADATA = defineActionContractMetadataList([
  ...
  },
  {
    ...
    payload: {
      ...
      shape: "full_record",
      target: "new_record",
      notes: "工作节点创建流程提交完整节点表单；通过后重新走 WorkItem domain validator 写入正式节点。",
    },
    persistence: {
      strategy: "approval_payload",
      ...
      statusField: "status",
      commitMode: "copy_to_active",
      notes: "流程数据存在 ApprovalRequest.latestPayload；通过后调用 WorkItem service 创建正式节点。",
    },
    form: {
      ...
      notes: "工作节点创建审批复用工作节点表单 payload。",
    },
    domain: {
    ...
    workflow: {
      ...
      configuration: WORK_TASKS_WORKFLOW_CONFIGURATION,
      validateOn: ["draft", "submit", "commit"],
      notes: "组织工作空间工作节点创建使用基础 work.tasks 流程；通过后写入正式工作节点。",
    },
  ...
  },
  {
    ...
    payload: {
      ...
      target: "existing_record",
      targetIdKey: "workId",
      notes: "工作节点更新流程提交节点 ID 和完整表单 patch；通过后更新正式节点。",
    },
    persistence: {
      strategy: "approval_payload",
      activeEntity: "WorkItem",
      ...
      statusField: "status",
      commitMode: "apply_patch",
      notes: "流程数据存在 ApprovalRequest.latestPayload；通过后调用 WorkItem service 更新正式节点。",
    },
    form: {
      adapterKey: "work.tasks.item",
      payloadVersion: 1,
      supportedPersistenceModes: ["workflowDraft"],
      supportedModes: ["workflow"],
      notes: "工作节点更新审批复用工作节点表单 payload。",
    },
    domain: {
    ...
    workflow: {
      ...
      configuration: WORK_TASKS_WORKFLOW_CONFIGURATION,
      validateOn: ["draft", "submit", "commit"],
      notes: "组织工作空间工作节点更新使用基础 work.tasks 流程；通过后更新正式工作节点。",
    },
