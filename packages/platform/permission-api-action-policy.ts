...
export const PERMISSION_API_ACTION_POLICIES = [
  { method: "POST", pathPrefix: "/api/modules/finance/import/preview", requiredActions: ["read"] },
  ...
  { method: "POST", pathPrefix: "/api/modules/finance/statements/consolidation/source-packages", requiredActions: ["create"], pathPattern: /^\/api\/modules\/finance\/statements\/consolidation\/source-packages$/ },
  { method: "POST", pathPrefix: "/api/modules/finance/statements/consolidation/source-packages", requiredActions: ["submit"], pathPattern: /^\/api\/modules\/finance\/statements\/consolidation\/source-packages\/[^/]+\/submit$/ },
  ...
  { method: "GET", pathPrefix: "/api/modules/agent/config/permission-grants", requiredActions: ["read"], pathPattern: /^\/api\/modules\/agent\/config\/permission-grants$/, notes: "The gateway requires agent.config.read; the service repeats that check and independently requires real grant-management authority for the selected Agent capability resource." },
  { method: "PUT", pathPrefix: "/api/modules/agent/config/permission-grants", requiredActions: ["read"], pathPattern: /^\/api\/modules\/agent\/config\/permission-grants$/, notes: "The gateway requires agent.config.read; the service repeats that check and preauthorizes every target resource with its actual grant boundary before one atomic RBAC write." },
  { method: "PUT", pathPrefix: "/api/modules/agent/config/action-ceiling", requiredActions: ["configure"], pathPattern: /^\/api\/modules\/agent\/config\/action-ceiling$/, notes: "The global Agent action ceiling is Agent configuration policy and never grants organization RBAC permissions." },
  { method: "GET", pathPrefix: "/api/modules/agent/config", requiredActions: ["read"], notes: "The canonical Agent configuration read model refreshes profiles, runtime capability candidates, validity, ceiling, and focused grant-management availability." },
  { method: "PUT", pathPrefix: "/api/modules/agent/config", requiredActions: ["configure"], notes: "Agent profile/runtime configuration does not grant organization RBAC permissions." },
  { method: "GET", pathPrefix: "/api/agent/profiles", requiredActions: ["read"], notes: "Virtual-employee profile discovery uses agent.assistant and only returns profiles with at least one registered tool currently usable by both requester and actor; Agent management-center access is not required." },
