...
export const PERMISSION_API_ACTION_POLICIES: readonly PermissionApiActionPolicy[] = [
  ...
  { method: "POST", pathPrefix: "/api/modules/administration/contracts", requiredActions: ["archive"], pathPattern: /^\/api\/modules\/administration\/contracts\/[^/]+\/archive$/ },
  { method: "POST", pathPrefix: "/api/modules/administration/contracts", requiredActions: ["update"], pathPattern: /^\/api\/modules\/administration\/contracts\/[^/]+\/(?:attachments|records)$/ },
  { method: "PUT", pathPrefix: "/api/modules/administration/contracts", requiredActions: ["update"], pathPattern: /^\/api\/modules\/administration\/contracts\/[^/]+\/approval-reference$/ },
  { method: "POST", pathPrefix: "/api/modules/administration/contracts", requiredActions: ["update"], pathPattern: /^\/api\/modules\/administration\/contracts\/[^/]+\/attachments\/[^/]+\/remove$/ },
  { method: "GET", pathPrefix: "/api/modules/administration/contracts", requiredActions: ["read"], pathPattern: /^\/api\/modules\/administration\/contracts\/[^/]+\/(?:package|attachments\/[^/]+\/download)$/ },
  ...
  { method: "PATCH", pathPrefix: "/api/modules/hr/roster/employee-profiles", requiredActions: ["update"], pathPattern: /^\/api\/modules\/hr\/roster\/employee-profiles\/[^/]+\/periods\/[^/]+$/, notes: "Employment and EDP in-place corrections use ordinary update permission; the service preserves optimistic concurrency, temporal invariants, and audit evidence." },
  { method: "POST", pathPrefix: "/api/modules/hr/roster/employee-profiles", requiredActions: ["update"], pathPattern: /^\/api\/modules\/hr\/roster\/employee-profiles\/[^/]+\/assignments$/, notes: "Adding an assignment is an ordinary employee-profile update; the service derives persistence metadata and validates the full timeline." },
  { method: "POST", pathPrefix: "/api/modules/hr/roster/employments", requiredActions: ["update"], notes: "Adding an Employment period updates an existing employee profile; actor, current-state projection, audit, and idempotent replay are server-owned." },
  ...
  { method: "GET", pathPrefix: "/api/modules/finance/cost/operational-analytics/spaces", requiredActions: ["read"], runtimeEnforcement: "serviceDelegated", pathPattern: /^\/api\/modules\/finance\/cost\/operational-analytics\/spaces\/(?<targetType>personal|department|project)\/(?<targetId>[^/]+)\/(?:sources(?:\/discover)?|templates)$/, scopeExtractor: standardBusinessSpaceScopeFromPath, notes: "Source discovery and template catalogs require concrete-space read access; personal API keys additionally require apiUse in the Finance service." },
  { method: "GET", pathPrefix: "/api/modules/finance/cost/operational-analytics/spaces", requiredActions: ["configure"], runtimeEnforcement: "serviceDelegated", pathPattern: /^\/api\/modules\/finance\/cost\/operational-analytics\/spaces\/(?<targetType>personal|department|project)\/(?<targetId>[^/]+)\/templates\/[^/]+$/, scopeExtractor: standardBusinessSpaceScopeFromPath, notes: "Reading an editable draft requires concrete-space configure access; personal API keys additionally require apiUse." },
  ...
  { method: "POST", pathPrefix: "/api/modules/finance/cost/operational-analytics/spaces", requiredActions: ["configure"], runtimeEnforcement: "serviceDelegated", pathPattern: /^\/api\/modules\/finance\/cost\/operational-analytics\/spaces\/(?<targetType>personal|department|project)\/(?<targetId>[^/]+)\/templates$/, scopeExtractor: standardBusinessSpaceScopeFromPath, notes: "Draft creation requires concrete-space configure access, source reauthorization and personal API-key apiUse." },
  { method: "PUT", pathPrefix: "/api/modules/finance/cost/operational-analytics/spaces", requiredActions: ["configure"], runtimeEnforcement: "serviceDelegated", pathPattern: /^\/api\/modules\/finance\/cost\/operational-analytics\/spaces\/(?<targetType>personal|department|project)\/(?<targetId>[^/]+)\/templates\/[^/]+$/, scopeExtractor: standardBusinessSpaceScopeFromPath, notes: "Draft updates require concrete-space configure access, source reauthorization, revision CAS and personal API-key apiUse." },
  ...
  { method: "POST", pathPrefix: "/api/modules/finance/statements/comparisons", requiredActions: ["import", "create"], pathPattern: /^\/api\/modules\/finance\/statements\/comparisons$/, notes: "Comparison evidence workbook upload persists an immutable evidence package and therefore requires the explicit-only import action plus create; the broad POST=create default must never authorize upload. The upload creates no accounting fact." },
  { method: "PUT", pathPrefix: "/api/modules/finance/statements/comparisons", requiredActions: ["update"], pathPattern: /^\/api\/modules\/finance\/statements\/comparisons\/[^/]+\/mapping$/, notes: "Mapping confirmation/remap updates comparison evidence with (id, revision) CAS in the Finance service." },
  { method: "POST", pathPrefix: "/api/modules/finance/statements/comparisons", requiredActions: ["create"], pathPattern: /^\/api\/modules\/finance\/statements\/comparisons\/[^/]+\/runs$/, notes: "Creating an immutable comparison run is an ordinary create; rerun always appends a new run." },
  { method: "POST", pathPrefix: "/api/modules/finance/statements/comparisons", requiredActions: ["update"], pathPattern: /^\/api\/modules\/finance\/statements\/comparisons\/[^/]+\/archive$/, notes: "Archiving comparison evidence is an update; referenced evidence is archived, never deleted." },
  { method: "POST", pathPrefix: "/api/modules/finance/statements/amount-explanations/query", requiredActions: ["read"], pathPattern: /^\/api\/modules\/finance\/statements\/amount-explanations\/query$/, notes: "Explicit read-only POST exception: complex ad hoc amount-origin explanation with no persistence. The stable DTO always carries residual/ambiguity/truncation and accountingTreatment: not_evaluated; Agent narration must surface those fields and must never claim reconciliation or accounting treatment." },
  ...
  { method: "POST", pathPrefix: "/api/modules/work/projects", requiredActions: ["update"], runtimeEnforcement: "serviceDelegated", pathPattern: /^\/api\/modules\/work\/projects\/[^/]+\/notification-rules$/, notes: "Project notification-rule creation requires project update access; Work also requires the explicit settings.notifications.configure grant and project manage access." },
  { method: "PUT", pathPrefix: "/api/modules/work/projects", requiredActions: ["update"], runtimeEnforcement: "serviceDelegated", pathPattern: /^\/api\/modules\/work\/projects\/[^/]+\/notification-rules\/[^/]+$/, notes: "Project notification-rule revision is object-scoped in Work and additionally requires settings.notifications.configure." },
  { method: "POST", pathPrefix: "/api/modules/work/projects", requiredActions: ["read"], runtimeEnforcement: "serviceDelegated", pathPattern: /^\/api\/modules\/work\/projects\/[^/]+\/notification-rules\/[^/]+\/preview$/, notes: "Rule preview is non-mutating; Work rechecks project visibility and settings.notifications.read before exposing trusted snapshot facts." },
  { method: "POST", pathPrefix: "/api/modules/work/projects", requiredActions: ["update"], runtimeEnforcement: "serviceDelegated", pathPattern: /^\/api\/modules\/work\/projects\/[^/]+\/notification-rules\/[^/]+\/(?:publish|archive)$/, notes: "Rule lifecycle changes require project manage access plus the explicit settings.notifications.configure grant." },
  { method: "POST", pathPrefix: "/api/modules/work/projects", requiredActions: ["update"], runtimeEnforcement: "serviceDelegated", pathPattern: /^\/api\/modules\/work\/projects\/[^/]+\/notification-signals\/redrive$/, notes: "Dead-letter redrive requires project management and the explicit settings.notifications.configure grant; Work rechecks signal ownership, failed state and the expected attempt count before the CAS transition." },
  ...
  { method: "POST", pathPrefix: "/api/modules/capitalSecurities/investments/documents", requiredActions: ["import"], pathPattern: /^\/api\/modules\/capitalSecurities\/investments\/documents$/, notes: "Investee evidence ingestion is an import action; the Library owner creates the immutable version and derived OCR/vector artifacts." },
  { method: "POST", pathPrefix: "/api/modules/capitalSecurities/investments/search", requiredActions: ["read"], pathPattern: /^\/api\/modules\/capitalSecurities\/investments\/search$/, notes: "Semantic search is non-mutating and restricted to the exact Library document UIDs linked to the selected investee." },
  ...
  { method: "GET", pathPrefix: "/api/modules/settings/notifications/definitions", requiredActions: ["read"], pathPattern: /^\/api\/modules\/settings\/notifications\/definitions$/ },
  { method: "POST", pathPrefix: "/api/modules/settings/notifications/publications", requiredActions: ["create", "apiUse"], pathPattern: /^\/api\/modules\/settings\/notifications\/publications$/, notes: "Personal API notification publication requires the orthogonal create and apiUse grants." },
  { method: "GET", pathPrefix: "/api/settings/api/open/notification-definitions", requiredActions: ["read"], pathPattern: /^\/api\/settings\/api\/open\/notification-definitions$/ },
  { method: "POST", pathPrefix: "/api/settings/api/open/notification-definitions", requiredActions: ["configure"], notes: "Definition draft save, publish, and archive are configure-only; routes recheck the explicit capability before calling the service." },
  { method: "GET", pathPrefix: "/api/settings/api/open/group-notifications", requiredActions: ["read"], pathPattern: /^\/api\/settings\/api\/open\/group-notifications$/, notes: "Managed-group directory, policy and delivery summaries require notification read access; the service filters configure-only reference options." },
  { method: "POST", pathPrefix: "/api/settings/api/open/managed-groups", requiredActions: ["configure"], notes: "Managed-group claim and verification are configure-only and remain fail-closed in the service." },
  { method: "PATCH", pathPrefix: "/api/settings/api/open/managed-groups", requiredActions: ["configure"], notes: "Managed-group lifecycle changes are configure-only and version guarded." },
  { method: "POST", pathPrefix: "/api/settings/api/open/group-policies", requiredActions: ["configure"], notes: "Managed-group policy creation is configure-only; enabling rechecks group verification and published definitions." },
  { method: "PATCH", pathPrefix: "/api/settings/api/open/group-policies", requiredActions: ["configure"], notes: "Managed-group policy updates are configure-only and version guarded." },
  { method: "GET", pathPrefix: "/api/agent/profiles", requiredActions: ["read"], notes: "Virtual-employee profile discovery uses agent and only returns profiles with at least one registered tool currently usable by both requester and actor." },
  ...
  { method: "POST", pathPrefix: "/api/agent/proposals", requiredActions: ["submit"], pathPattern: /^\/api\/agent\/proposals\/[^/]+\/(?:confirm|cancel)$/, notes: "Agent proposal settlement is a submit-cycle action; business API confirmation rechecks the fixed requester/profile identities, Agent action ceiling and the live target API contract before dispatch." },
  ...
  { method: "GET", pathPrefix: "/api/modules/settings/governance/operations", requiredActions: ["audit"], pathPattern: /^\/api\/modules\/settings\/governance\/operations$/, notes: "Governed operations records require the explicit settings.governance audit action." },
  { method: "GET", pathPrefix: "/api/settings/governance/relation-policies", requiredActions: ["read"], runtimeEnforcement: "serviceDelegated", pathPattern: /^\/api\/settings\/governance\/relation-policies$/, notes: "Relation policy catalog reads remain restricted to root users in the route." },
  { method: "PATCH", pathPrefix: "/api/settings/governance/relation-policies", requiredActions: ["configure"], runtimeEnforcement: "serviceDelegated", pathPattern: /^\/api\/settings\/governance\/relation-policies$/, notes: "Relation policy changes require governance configure and remain restricted to root users in the route." },
  { method: "PATCH", pathPrefix: "/api/settings/governance/sql-settings", requiredActions: ["configure"], runtimeEnforcement: "serviceDelegated", pathPattern: /^\/api\/settings\/governance\/sql-settings$/, notes: "Creates a root-only PostgreSQL operation request; the web runtime never executes privileged SQL or receives generated credentials." },
  { method: "GET", pathPrefix: "/api/settings/governance/database-schema", requiredActions: ["read"], runtimeEnforcement: "serviceDelegated", notes: "Database relation reads remain restricted to root users in the route." },
  { method: "GET", pathPrefix: "/api/settings/governance/modules", requiredActions: ["read"], runtimeEnforcement: "serviceDelegated", notes: "Module and source analysis reads remain restricted to root users in the route." },
  { method: "PATCH", pathPrefix: "/api/settings/governance/modules", requiredActions: ["configure"], runtimeEnforcement: "serviceDelegated", notes: "Module runtime changes require governance configure and remain restricted to root users in the route." },
  { method: "GET", pathPrefix: "/api/settings/governance", requiredActions: ["read"], runtimeEnforcement: "serviceDelegated", notes: "Platform governance reads use the L2 resource; privileged sources remain restricted in their route handlers." },
  { method: "PATCH", pathPrefix: "/api/settings/governance", requiredActions: ["configure"], runtimeEnforcement: "serviceDelegated", notes: "Platform governance writes require configure and remain restricted by their route/service." },
