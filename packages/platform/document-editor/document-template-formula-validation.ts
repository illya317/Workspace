...
} from "@workspace/platform/formula/parser";
import { durationUnit, isFormulaAlias, validateDateDifferenceExpression } from "@workspace/platform/formula/date-difference";
import {
  failCommand,
  okCommand,
...
function collectFormulaCandidates(document: unknown, fieldModel: unknown) {
  ...
  walkJson(document, (node) => {
    ...
    candidates.push({
      ...
      inputType: stringField(node.inputType),
