...
export function collectFieldModelFormulaTokens(
  fieldModel: FieldModel | undefined,
  existing: Map<string, FormulaDisplayToken>,
...
function formulaTarget(attrs: EditorSlotInline, field?: FieldDefinition) {
  return {
    inputType: attrs.inputType ?? field?.inputType,
    valueType: attrs.valueType ?? field?.valueType ?? field?.type,
  };
}
...
function constantFieldReplacement(field: FieldDefinition, dryingWeightMultipliers: Record<string, number>) {
  const numeric = numericText(field.defaultValue) ?? numericText(field.recommendedValue);
  if (numeric) return numeric;
  const source = ((field as FieldDefinition & { source?: Record<string, unknown> }).source ?? field.metadata?.source ?? {}) as Record<string, unknown>;
  const multiplier = dryingWeightMultipliers[stringValue(source.productKey)];
