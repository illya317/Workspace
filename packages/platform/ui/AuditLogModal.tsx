...
export default function AuditLogModal({ open, onClose, entityType, onRestored }: AuditLogModalProps) {
  ...
  const loadDates = useCallback(async () => {
    try {
      const res = await fetch(workspacePath(`/api/settings/governance/audit-log?entityType=${entityType}&dates=1`));
      if (res.ok) {
  ...
  const load = useCallback(async (p: number, d: string) => {
    ...
    try {
      ...
      const res = await fetch(workspacePath(`/api/settings/governance/audit-log?${params}`));
      if (res.ok) {
  ...
  const restore = useCallback(async (historyId: number) => {
    ...
    try {
      const res = await fetch(workspacePath("/api/settings/governance/audit-log/restore"), {
        method: "POST",
