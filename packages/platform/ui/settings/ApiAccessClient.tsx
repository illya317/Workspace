...
function buildAgentAccessText({
  ...
  const lines = [
    ...
    "- Auth: /api/auth/*",
    "- Agent: /api/agent/*",
    "- Source of truth: module registry / API registry. Run arch:gate after API changes.",
    "",
    "L1/L2 modules:",
  ];
  ...
  for (const moduleRow of modules) {
    ...
    for (const child of moduleRow.children) {
      const api = child.apiPrefixes.length > 0
        ? child.apiPrefixes.join(", ")
        : child.noApiReason
          ? `no API: ${child.noApiReason}`
          : "no API";
...
export default function ApiAccessClient({
  ...
  async function rotateApiKey() {
    setLoading(true);
    const res = await fetch(workspacePath("/api/settings/account/api-key"), { method: "POST" });
    if (res.ok) {
      ...
      <SectionCard
        ...
        actions={
          <div className="flex flex-wrap gap-2">
            {apiKey ? (
              <ActionButton onClick={() => setConfirmOpen(true)} disabled={loading}>
                {loading ? "申请中..." : "重新申请"}
              </ActionButton>
            ) : (
              <ActionButton onClick={rotateApiKey} disabled={loading}>
                {loading ? "申请中..." : "申请 Key"}
              </ActionButton>
            )}
            <ActionButton onClick={copyConnectionBlock} variant="primary">
              {copied ? "已复制" : "复制接入信息"}
            </ActionButton>
          </div>
        }
