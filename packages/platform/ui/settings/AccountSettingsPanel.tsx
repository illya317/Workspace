...
import { ActionButton, ActionToolbar, FileField, FormField, PageContent, PanelCard, TextField } from "@workspace/core/ui";
...
export default function AccountSettingsPanel({ user, onUserRefresh, apiAccessModules }: AccountSettingsPanelProps) {
  ...
  async function saveProfile() {
    ...
    const res = await fetch(workspacePath("/api/settings/account/username"), {
      ...
      body: JSON.stringify({ username: username.trim(), nickname: nickname.trim() }),
    });
    const data = await res.json();
    if (!res.ok) {
      setUsernameMessage({ type: "error", text: data.error || "修改失败" });
      return;
    }
    setUsernameMessage({ type: "success", text: "账号资料已更新" });
    onUserRefresh();
  }
  ...
  async function saveAvatarUrl(nextAvatar: string) {
    ...
    const res = await fetch(workspacePath("/api/settings/account/avatar"), {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ avatar: nextAvatar.trim() || null }),
    });
    const data = await res.json();
    if (!res.ok) {
      setAvatarMessage({ type: "error", text: data.error || "修改失败" });
      return;
    }
    setAvatar(nextAvatar);
    setAvatarFile(null);
    setAvatarMessage({ type: "success", text: nextAvatar.trim() ? "头像已更新" : "头像已清除" });
    onUserRefresh();
  }
  ...
  async function saveAvatar() {
    if (!avatarFile) {
      setAvatarMessage({ type: "error", text: "请先选择头像文件" });
      return;
    }
    ...
    const data = await res.json();
    if (!res.ok) {
      setAvatarMessage({ type: "error", text: data.error || "上传失败" });
  ...
  return (
    <PageContent className="max-w-4xl py-10">
      <ActionToolbar
        leftSlot={(
          <div className="flex min-w-0 items-center gap-4">
            <span
              className="flex h-16 w-16 shrink-0 items-center justify-center overflow-hidden rounded-full bg-emerald-50 bg-cover bg-center text-xl font-semibold text-emerald-700"
      ...
      <div className="mt-6 grid gap-5 lg:grid-cols-3">
        <PanelCard title="修改账号" className="h-full" bodyClassName="space-y-3 p-4">
          <FormField label="姓名">
            <TextField
              ...
              readOnly
              className="cursor-not-allowed border-slate-200 bg-slate-100 text-slate-400 shadow-none"
            />
          </FormField>
          <FormField label="昵称">
            <TextField
        ...
        <PanelCard title="修改密码" className="h-full" bodyClassName="space-y-3 p-4">
          ...
          <ActionButton variant="secondary" className="w-full" onClick={() => void savePassword()}>
            保存密码
          </ActionButton>
        </PanelCard>
        ...
        <PanelCard title="修改头像" className="h-full" bodyClassName="p-4">
          <div className="flex flex-col items-center gap-4">
            <span
              className="flex h-36 w-36 shrink-0 items-center justify-center overflow-hidden rounded-full border border-emerald-100 bg-emerald-50 bg-cover bg-center text-3xl font-semibold text-emerald-700 shadow-inner"
            ...
            <div className="grid w-full grid-cols-2 gap-2">
              ...
              />
              <ActionButton variant="primary" className="w-full" disabled={!avatarFile || avatarSaving} onClick={() => void saveAvatar()}>
