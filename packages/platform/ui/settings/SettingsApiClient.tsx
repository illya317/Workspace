...
function StatusPill({ value }: { value: string }) {
  ...
  return (
    <span className={`rounded-full px-2 py-0.5 text-xs ${active ? "bg-emerald-50 text-emerald-700" : "bg-slate-100 text-slate-500"}`}>
      {active ? "启用" : value}
    </span>
  );
}

function MethodPill({ value }: { value: string }) {
  return (
    <span className="rounded bg-cyan-50 px-2 py-0.5 font-mono text-xs font-semibold text-cyan-700">
      {value}
    </span>
  );
}

export default function SettingsApiClient({ focusRegistrationKey }: { focusRegistrationKey?: string }) {
  ...
  const registrationColumns: DataTableColumn<OpenApiRegistrationRow>[] = [
    { key: "label", label: "能力", render: (row) => <div className="font-medium text-slate-900">{row.label}<div className="text-xs font-normal text-slate-500">{row.key}</div></div> },
    { key: "resource", label: "开放资源", render: (row) => row.resources.map((resource) => resource.key).join(", ") },
    { key: "scope", label: "Scope", render: (row) => row.scopes.map((scope) => scope.key).join(", ") },
    { key: "runtime", label: "运行归属", render: (row) => row.runtimeParentResourceKey },
    { key: "console", label: "页面", render: (row) => <a className="text-cyan-700 hover:underline" href={workspacePath(row.consoleHref)}>进入</a> },
  ];
  const endpointColumns: DataTableColumn<OpenApiEndpointRow>[] = [
    { key: "method", label: "Method", render: (row) => <MethodPill value={row.method} /> },
    { key: "path", label: "Path", render: (row) => <code className="text-xs text-slate-700">{row.pathPrefix}</code> },
    { key: "scope", label: "Scope", render: (row) => row.scopeKey },
  ];
  const clientColumns: DataTableColumn<OpenApiClientRow>[] = [
    { key: "name", label: "Client", render: (row) => <div className="font-medium text-slate-900">{row.name}<div className="text-xs font-normal text-slate-500">{row.description || "无说明"}</div></div> },
    { key: "status", label: "状态", render: (row) => <StatusPill value={row.status} /> },
    { key: "scopes", label: "Scope", render: (row) => row.scopeKeys.length ? row.scopeKeys.join(", ") : "未授权" },
    { key: "lastUsedAt", label: "最近调用", render: (row) => formatDate(row.lastUsedAt) },
    { key: "actions", label: "操作", render: (row) => <ActionButton disabled={busy === `rotate-${row.id}`} onClick={() => rotateSecret(row.id)}>轮换密钥</ActionButton> },
  ];
  const logColumns: DataTableColumn<OpenApiLogRow>[] = [
    ...
    { key: "endpoint", label: "Endpoint", render: (row) => <span><MethodPill value={row.method} /> <code className="ml-2 text-xs">{row.path}</code></span> },
    { key: "status", label: "状态", render: (row) => <span className={row.status < 400 ? "text-emerald-700" : "text-red-600"}>{row.status}</span> },
    { key: "duration", label: "耗时", render: (row) => `${row.durationMs} ms` },
    { key: "error", label: "错误", render: (row) => row.errorCode || "-" },
  ];
  ...
  return (
    <DatabasePageFrame contentClassName="py-8">
      ...
      {freshSecret && (
        <SectionCard title="新密钥" subtitle="只在本次操作后显示。" actions={<ActionButton onClick={() => setFreshSecret(null)}>隐藏</ActionButton>}>
      ...
      <SectionCard title="Client" actions={<ActionButton onClick={() => loadData()} disabled={loading}>刷新</ActionButton>}>
        <div className="mb-4 grid gap-3 md:grid-cols-[1fr_1fr_auto]">
          ...
          <div className="flex items-end">
            <ActionButton onClick={createClient} variant="primary" disabled={busy === "create" || !newClientName.trim()}>创建</ActionButton>
          </div>
        </div>
        <DataTable
          rows={data?.clients ?? []}
      ...
      <SectionCard title="Scope 授权" subtitle={selectedClient ? selectedClient.name : "先选择一个 Client。"} actions={<ActionButton onClick={saveScopes} disabled={!selectedClient || busy === `scopes-${selectedClient?.id}`}>保存</ActionButton>}>
        <div className="flex flex-wrap gap-2">
          {visibleScopes.map((scope) => (
            <CheckboxChip
              key={scope.key}
              checked={draftScopeKeys.includes(scope.key)}
              onChange={(checked) => {
