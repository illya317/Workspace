...
import { createFieldsSection, createMessageSection, createPageBody, createPageDataSection, createPanelSection, createSectionSection, type DataSurfaceColumnSpec, type FormSurfaceItemSpec, PageSurface, type BodySurfaceSectionSpec } from "@workspace/core/ui";
...
export default function SettingsApiClient({
  ...
  const registrationColumns: DataSurfaceColumnSpec<OpenApiRegistrationRow>[] = [{
    key: "label",
    label: "能力",
    cell: row => ({ kind: "stack", items: [{ kind: "text", value: row.label, emphasis: "medium" }, { kind: "text", value: row.key, tone: "muted", font: "mono" }] })
  }, {
    key: "resource",
    label: "开放资源",
    ...
    key: "console",
    label: "页面",
    cell: row => ({ kind: "link", label: "进入", href: workspacePath(row.consoleHref) })
  }];
  const endpointColumns: DataSurfaceColumnSpec<OpenApiEndpointRow>[] = [{
    ...
    key: "path",
    label: "Path",
    cell: row => ({ kind: "text", value: row.pathPrefix, font: "mono" })
  }, {
    key: "scope",
    label: "Scope",
  ...
  const clientColumns: DataSurfaceColumnSpec<OpenApiClientRow>[] = [{
    key: "name",
    label: "Client",
    cell: row => ({ kind: "stack", items: [{ kind: "text", value: row.name, emphasis: "medium" }, { kind: "text", value: row.description || "无说明", tone: "muted" }] })
  }, {
    key: "status",
    label: "状态",
  ...
  } satisfies DataSurfaceColumnSpec<OpenApiClientRow>] : [])];
  const logColumns: DataSurfaceColumnSpec<OpenApiLogRow>[] = [{
    ...
    key: "endpoint",
    label: "Endpoint",
    cell: row => ({ kind: "group", items: [{ kind: "badge", label: row.method, tone: "sky" }, { kind: "text", value: row.path, font: "mono" }] })
  }, {
    key: "status",
    label: "状态",
  ...
  const clientFormSection: BodySurfaceSectionSpec | null = canCreateClient ? {
    key: "client-form",
    body: { kind: "form", form: {
      kind: "filters",
      content: { items: [
      ...
      actions: [{ key: "create", action: "create", label: "创建", onClick: createClient, disabled: busy === "create" || !newClientName.trim() }],
    } },
  } : null;
  const sections: BodySurfaceSectionSpec[] = [
    ...
    createSectionSection("clients", {
      ...
      sections: [
        ...(clientFormSection ? [clientFormSection] : []),
        {
          key: "client-table",
          body: { kind: "data", data: {
            kind: "table",
    ...
    createPanelSection("scopes", {
      sections: [createFieldsSection("scope-form", scopeFields, {
        header: { title: "Scope 授权" },
        layout: { columns: 3 },
        actions: canGrantScopes ? [{ key: "save-scopes", action: "grant", label: "保存授权", onClick: saveScopes, disabled: !selectedClient || busy === `scopes-${selectedClient?.id}` }] : [],
      })],
    }),
