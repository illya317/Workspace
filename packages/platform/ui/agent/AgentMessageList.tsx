    ...
    minHeight: 160
      ...
      {messages.length === 0 ? <div className="flex flex-col items-center justify-center py-8 text-center">
          ...
          <div className="mt-4 flex flex-wrap gap-1.5 justify-center">
            {!hintsLoaded ? <span className="text-xs text-gray-300">加载中...</span> : hints.length > 0 ? hints.map(h => <button type="button" key={h} onClick={() => onHintClick(h)} className={[getToolbarActionClassName(), "rounded-full px-2 py-1 text-xs"].filter(Boolean).join(" ")}>
                  {h}
                </button>) : <span className="text-xs text-gray-400">暂无可用功能，请联系管理员</span>}
          </div>
            ...
            <div className="max-w-[82%]">
              ...
              {msg.role === "agent" && !!msg.data && <button type="button" onClick={() => onOpenDrawer(msg)} className={[getToolbarActionClassName(), "mt-1 px-2 py-1 text-xs"].filter(Boolean).join(" ")}>
                  查看报告 →
                </button>}
            </div>
          </div>)}
