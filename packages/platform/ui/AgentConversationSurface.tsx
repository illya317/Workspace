...
export function AgentConversationSurface({
  ...
  async function settleProposal(messageId: string, proposalId: number, action: "confirm" | "cancel") {
    ...
    const requestContextKey = activeContextKeyRef.current;
    const proposalActionKey = messages.find((message) => message.id === messageId)?.proposal?.actionKey;
    ...
    try {
      ...
      if (outcome.ok && action === "confirm" && proposalActionKey) {
        window.dispatchEvent(new CustomEvent("workspace:agent-proposal-confirmed", {
          detail: { proposalId, actionKey: proposalActionKey },
        }));
      }
    } finally {
