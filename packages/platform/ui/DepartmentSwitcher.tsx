...
export default function DepartmentSwitcher({ onChange }: { onChange?: (deptId: number | null) => void }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <InputSurface
        spec={{ valueType: "string", control: "choice", options: { source: "static", items: companies.map((c) => ({ value: c, label: c })) } }}
      ...
      />
      <InputSurface
        spec={{ valueType: "reference", control: "choice", options: { source: "static", items: deptsInCompany.map((d) => ({ value: String(d.id), label: d.id === selectedDeptId ? `${d.name}（当前）` : d.name })) } }}
