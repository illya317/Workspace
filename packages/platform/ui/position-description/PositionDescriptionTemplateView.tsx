...
export function PositionDescriptionTemplateView({
  ...
  return (
    <PageSurface
      kind="standard"
      body={createPageBody([
        createPositionDescriptionTemplateSection("position-description-template-paper", data, template),
      ], { commands: actions })}
    />
  );
}

/** @ui-structural-declaration Position-description document data mapped to the Platform preview runtime. */
export function createPositionDescriptionTemplateSection(key: string, data: PositionDescriptionTemplateData, template: PositionDescriptionTemplateDto | null): BodySurfaceSectionSpec {
  const rendered = template && isEditorDocument(template.document)
    ? buildPositionDescriptionDocument(template.document, data)
    : null;

  if (!rendered) return createEmptySection(key, { content: "未找到岗位说明书模板", compact: true });
  return createDocumentWorkspaceSection({ key, mode: "preview", document: rendered.document, values: rendered.values, slotPresentation: "value" });
}
