...
export type ExternalPartyWithRoles = Prisma.ExternalPartyGetPayload<{
  include: { roles: true };
}>;
...
export function projectExternalParty(
  ...
  const role = party.roles.find((item) => item.category === category);
  if (!role) return null;
  ...
  return {
    ...
    relatedPartyType: party.relatedPartyType as ExternalParty["relatedPartyType"],
    code: role.code,
    name: party.name,
