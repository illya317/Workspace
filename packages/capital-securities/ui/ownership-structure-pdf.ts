import type { OwnershipStructureGraph } from "../types";

const A3_LANDSCAPE_WIDTH = 1190.55;
const A3_LANDSCAPE_HEIGHT = 841.89;
const PAGE_MARGIN = 36;

export async function downloadOwnershipStructurePdf(graph: OwnershipStructureGraph) {
  const network = document.querySelector<HTMLElement>('[aria-label="关系结构图"]');
  if (!network) throw new Error("股权结构图尚未完成渲染");

  const layers = [...network.querySelectorAll("canvas")].filter((canvas) => (
    canvas.width > 0 && canvas.height > 0
  ));
  if (layers.length === 0) throw new Error("股权结构图画布不可用");

  const width = Math.max(...layers.map((canvas) => canvas.width));
  const graphHeight = Math.max(...layers.map((canvas) => canvas.height));
  const displayWidth = Math.max(network.getBoundingClientRect().width, 1);
  const pixelRatio = Math.max(width / displayWidth, 1);
  const headerHeight = Math.round(92 * pixelRatio);
  const graphCanvas = document.createElement("canvas");
  graphCanvas.width = width;
  graphCanvas.height = graphHeight;
  const graphContext = graphCanvas.getContext("2d");
  if (!graphContext) throw new Error("浏览器无法读取股权结构图画布");
  graphContext.fillStyle = "#ffffff";
  graphContext.fillRect(0, 0, width, graphHeight);
  for (const layer of layers) graphContext.drawImage(layer, 0, 0, width, graphHeight);

  const bounds = graphContentBounds(graphContext, width, graphHeight, pixelRatio);
  ...
  drawHeader(context, graph, pixelRatio, exportCanvas.width);
  context.drawImage(
    graphCanvas,
    bounds.x,
    bounds.y,
  ...
  const name = ownershipStructureName(graph).replace(/[\\/:*?"<>|]/g, "-");
  downloadBlob(
    new Blob([pdf], { type: "application/pdf" }),
    `${name}-股权结构图-${graph.asOf}.pdf`,
  );
}
...
function drawHeader(
  ...
  context.fillText(`${ownershipStructureName(graph)}股权结构图`, left, 18 * pixelRatio);
  context.fillStyle = "#64748b";
  context.font = `${14 * pixelRatio}px ui-sans-serif, system-ui, sans-serif`;
  context.fillText(`股权基准日：${graph.asOf}`, left, 55 * pixelRatio);
...
function ownershipStructureName(graph: OwnershipStructureGraph) {
