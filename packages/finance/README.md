...
- `ui/import/components/ImportPreview.tsx`：生成导入预览 sections，不保留第二套 JSX renderer。
...
- `server/statements/`：财务报表配置、映射、内部底稿事实、报表生成和差异计算服务；旧 `server/services/finance/statements` 已收口到财务业务包。
