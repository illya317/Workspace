...
import { loadConsolidationCandidateFacts, loadConsolidationScopeFacts, periodEndDate } from "./consolidation-snapshots";
...
export async function loadConsolidationOverview(
  ...
  const companyCodes = requestedBatch
    ? requestedBatch.entities.map((entity) => entity.companyCode)
    : liveScope.map((entity) => entity.companyCode);
  const candidateCompanyCodes = requestedBatch ? companyCodes : candidateScope.map((entity) => entity.companyCode);
  ...
  const companyDirectory = await loadConsolidationCompanyDirectory(candidateCompanyCodes);
  ...
  const sourceReadiness = await loadConsolidationSourceReadiness({
    companyCodes: candidateCompanyCodes,
    year: selectedPeriod.year,
    month: selectedPeriod.month,
    periodKind: selectedPeriodKind,
  });
  ...
  const includedCompanyIds = new Set((requestedBatch?.entities ?? liveScope).map((entity) => entity.companyId));
  const includedEntities = entities.filter((entity) => entity.companyId && includedCompanyIds.has(entity.companyId));
  const allSources = includedEntities.flatMap((entity) => [entity.balanceSheet, entity.incomeStatement, entity.cashFlow]);
  ...
  const invalidOwnership = includedEntities.filter((entity) => entity.role === "子公司" && (entity.shareRatio === null || entity.shareRatio <= 0 || entity.shareRatio > 1)).length;
  ...
  const investmentRows = await loadCadInvestmentVoucherFacts(companyCodes, selectedPeriodEnd);
  const investmentEvidence = investmentRows.map((item) => {
    const rateBinding = historicalApplicationByVoucher.get(item.id);
    const derivedRate = item.originalAmount && item.originalAmount > 0 && item.bookedAmountCny > 0
      ? item.bookedAmountCny / item.originalAmount
      : null;
    const transactionRate = rateBinding ? Number(rateBinding.rate.rate) : derivedRate;
    return {
      ...item,
      transactionRate,
      rateStatus: rateBinding
        ? "recorded" as const
        : derivedRate !== null
          ? "derivedFromActualCny" as const
          : item.originalAmount === null
            ? "missingOriginalCurrency" as const
            : "missingRate" as const,
    };
  });
  const missingInvestmentRateCount = investmentEvidence.filter((item) => (
    item.rateStatus === "missingOriginalCurrency" || item.rateStatus === "missingRate"
  )).length;
  const foreignEntities = includedEntities.filter((entity) => entity.entitySnapshotId && cadEntityIds.has(entity.entitySnapshotId));
  ...
  const unresolvedEliminationDecisions = eliminationTypes.flatMap((entryType) => {
    const decision = decisions.get(`elimination:${entryType}`);
    return decision?.decision === "requiresReview" ? [{ entryType, decision }] : [];
  });
  ...
  const checks: ConsolidationReadinessCheck[] = [
    ...
    { key: "fx", label: "外币折算与汇率", status: !requestedBatch || !currencyPoliciesComplete ? "blocked" : cadEntityIds.size === 0 ? "ready" : !canadaSourceStatementsReady || !closingCoverageComplete ? "blocked" : "ready", detail: !requestedBatch ? "需先生成合并批次" : !currencyPoliciesComplete ? "币种主数据仍有实体缺少本位币，不能自动折算" : cadEntityIds.size === 0 ? "批次内实体均为 CNY 本位币" : !canadaSourceStatementsReady ? "CAD 本位币主体个别三表尚未冻结完整" : !closingCoverageComplete ? `尚未取得适用日期的中国货币网中间价；${comparativeCadEntityIds.size} 个含非零上期数的主体还需要比较期汇率` : "资产负债按期末汇率，收入费用及现金流量按月平均汇率，实收资本和资本公积按实际人民币出资额或真实发生日汇率；证据已冻结", facts: { cadEntityCount: cadEntityIds.size, comparativeCadEntityCount: comparativeCadEntityIds.size, incompleteCurrencyPolicyCount: requestedBatch?.entities.filter((entity) => !entity.functionalCurrency || !entity.currencyEvidence?.trim()).length ?? 0, closingBindingCount: validClosingApplications.length, comparativeClosingBindingCount: validComparativeClosingApplications.length, closingRateId: closingRate?.id ?? null, comparativeClosingRateId: comparativeClosingRate?.id ?? null, historicalRateCount, missingInvestmentRateCount }, evidence: requestedBatch?.exchangeRates.map((rate) => rate.rateKind === "historicalCapitalAmount" ? `#${rate.exchangeRateId} v${rate.exchangeRateVersion} 历史权益实际人民币金额/派生加权汇率` : rate.rateKind === "monthlyAverage" ? `#${rate.exchangeRateId} v${rate.exchangeRateVersion} 月平均汇率 ${rate.rateDate.slice(0, 7)}` : `#${rate.exchangeRateId} v${rate.exchangeRateVersion} 期末或时点汇率 ${rate.rateDate}`) ?? [], dependencyKeys: ["sources"], resolution: consolidationReadinessResolution(requestedBatch?.id ?? null, "fx") },
    { key: "eliminations", label: "合并抵销", status: missingEliminationPackages.length > 0 || incompleteMatchingEntries > 0 || unresolvedEliminationDecisions.length > 0 ? "blocked" : inProgressEntries > 0 ? "attention" : "ready", detail: unresolvedEliminationDecisions.length > 0 ? `${unresolvedEliminationDecisions.length} 类投资与权益差额待按历史依据分类` : missingEliminationPackages.length > 0 ? `${missingEliminationPackages.length} 类抵销事项尚无分录或不适用结论` : incompleteMatchingEntries > 0 ? `${incompleteMatchingEntries} 笔内部往来/交易/资金抵销缺少双方结构化来源或差额处置` : inProgressEntries > 0 ? `${inProgressEntries} 笔抵销分录编制或复核中` : `${approvedEntries} 笔抵销分录已批准，其余类别已有不适用结论`, facts: { approvedEntries, inProgressEntries, incompleteMatchingEntries, unresolvedPackageCount: missingEliminationPackages.length, unclassifiedDifferenceCount: unresolvedEliminationDecisions.length }, evidence: [...(requestedBatch?.entries.map((entry) => `${entry.entryNo} v${entry.version} ${entry.status}`) ?? []), ...unresolvedEliminationDecisions.map(({ decision }) => decision.evidence)], dependencyKeys: ["ownership", "sources", "fx"], resolution: consolidationReadinessResolution(requestedBatch?.id ?? null, "eliminations") },
  ...
  const adjustmentComparisons = input.includeComparisons
    ? await loadConsolidationAdjustmentComparisons({ batch: requestedBatch, entities: includedEntities.flatMap((entity) => entity.companyId ? [{ companyId: entity.companyId, code: entity.code, name: entity.name, role: entity.role === "母公司" ? "parent" as const : "subsidiary" as const }] : []) })
    : [];
  ...
  const creationBlockedReasons = [
    ...
    ...(includedEntities.length > 1 ? [] : ["合并范围尚未完整建立"]),
