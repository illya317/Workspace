...
function resolution(
  ...
  if (key === "scope" || key === "ownership") return {
    ...
    actionKey: "capitalSecurities.governance.companyRelation.update",
...
export async function loadConsolidationOverview(
  ...
  const adjustmentComparisons = await loadConsolidationAdjustmentComparisons({ batch: requestedBatch, entities: entities.flatMap((entity) => entity.companyId ? [{ companyId: entity.companyId, code: entity.code, name: entity.name, role: entity.role === "母公司" ? "parent" as const : "subsidiary" as const }] : []) });
  return {
