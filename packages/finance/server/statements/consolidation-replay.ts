...
import { ACTIVE_CONSOLIDATION_ENTRY_TYPES } from "../domain/consolidation-batch-constants";
...
export function buildConsolidationReplayPackage(
  ...
  if (taxEffectCount === 0 && (
    !taxDecision
    || !["completed", "notApplicable"].includes(taxDecision.decision)
    || !taxDecision.evidence.trim()
  )) {
    return failCommand("批次缺少抵销税务影响复核结论", 409, "tax");
  }
