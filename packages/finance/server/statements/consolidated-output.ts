...
function sumLines(lines: ConsolidatedOutputLine[], predicate: (line: ConsolidatedOutputLine) => boolean) {
  return {
    amount: money(lines.filter(predicate).reduce((sum, line) => sum + line.amount, 0)),
    previousAmount: money(lines.filter(predicate).reduce((sum, line) => sum + line.previousAmount, 0)),
  };
}

function recomputeBalance(lines: ConsolidatedOutputLine[]) {
  for (const line of lines.filter((candidate) => candidate.isTotal)) {
    const total = sumLines(lines, (candidate) => (
      candidate.section === line.section
      && !candidate.isHeader
      && !candidate.isTotal
      && !candidate.isGrandTotal
    ));
    setDerivedAmount(line, total.amount, total.previousAmount);
  }
  ...
  for (const line of lines.filter((candidate) => candidate.isGrandTotal)) {
    ...
    const total = sumLines(lines, (candidate) => candidate.isTotal && sections.includes(candidate.section));
    setDerivedAmount(line, total.amount, total.previousAmount);
  }
}
...
function recomputeCashFlow(lines: ConsolidatedOutputLine[]) {
  ...
  for (const [lineCode, derivation] of Object.entries(CASH_FLOW_DERIVATIONS)) {
    ...
    if (!line) continue;
    const add = sumLines(lines, (candidate) => derivation.add.includes(candidate.lineCode));
    const subtract = sumLines(lines, (candidate) => derivation.subtract?.includes(candidate.lineCode) ?? false);
    setDerivedAmount(line, add.amount - subtract.amount, add.previousAmount - subtract.previousAmount);
  }
}
    ...
    for (const source of sourceRows) {
      ...
      for (const translatedLine of translated.data) {
        ...
        if (!existing) {
          ...
          outputByCode.set(translatedLine.lineCode, { ...translatedLine });
        } else {
