...
import {
  ...
  consolidationSourceContentFingerprint,
...
test("source content freshness ignores capture time but detects report changes", () => {
  const source = {
    companyId: 1,
    reportType: "balanceSheet",
    sourceKind: "system",
  ...
  const first = consolidationSourceContentFingerprint({
    ...source,
    reportPayload: { httpStatus: 200, capturedAt: "2026-07-21T00:00:00.000Z", payload: { totalAssets: 100 } },
  });
  const recapturedSource = {
    ...source,
    reportPayload: { httpStatus: 200, capturedAt: "2026-07-22T00:00:00.000Z", payload: { totalAssets: 100 } },
    selectedAt: "2026-07-22T00:00:01.000Z",
    id: 99,
  };
  const recaptured = consolidationSourceContentFingerprint(recapturedSource);
  const changed = consolidationSourceContentFingerprint({
    ...source,
