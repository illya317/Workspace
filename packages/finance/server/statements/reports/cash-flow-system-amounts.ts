...
async function cashBalances(companyCode: string, year: number, month: number) {
  const balances = await prisma.financeAccountBalance.findMany({
    where: {
      period: { companyCode, year, month: { in: [...new Set([1, month])] } },
  ...
  const opening = selectCashRoots(1);
  const ending = selectCashRoots(month);
  return {
    opening: roundMoney(opening.reduce((sum, item) => sum + item.openingDebit - item.openingCredit, 0)),
    ending: roundMoney(ending.reduce((sum, item) => sum + item.closingDebit - item.closingCredit, 0)),
  };
}
...
export async function computeCashFlowSystemAmounts(
  ...
  const allocations = await prisma.financeCashFlowAllocation.findMany({
    where: { companyCode, period: { year, month: { lte: month } }, voucher: { status: "posted" } },
    include: { cashFlowItem: { select: { sourceName: true } } },
  });
  ...
  for (const allocation of allocations) {
    ...
    amounts.set(lineCode, roundMoney((amounts.get(lineCode) ?? 0) + Number(allocation.amount)));
  }
  applyDerivedLines(amounts);
  const cash = await cashBalances(companyCode, year, month);
