...
export async function computeIncomeSystemAmounts(
  ...
  return computeFromVouchers(companyCode, year, month, lines);
}
...
async function computeFromVouchers(
  ...
  for (const line of config) {
    ...
    const signedAmount = line.subtract ? -amount : amount;
    result.set(line.lineCode, Math.round(signedAmount * 100) / 100);
  }
