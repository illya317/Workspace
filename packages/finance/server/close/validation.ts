...
export async function buildCompleteFinanceCloseCommand(input: CompleteFinanceCloseInput, userId: number, deps = closeValidationDependencies) {
  const invalidUser = await validateUser(userId, deps);
  if (invalidUser) return invalidUser;
  const run = await deps.findRun(input.runId);
  if (!run) return failCommand("关账运行不存在", 404, "runId");
  if (!run.company.isActive) return failCommand("关账运行所属公司未启用", 409, "runId");
  if (run.period.companyCode !== run.company.code || run.periodId !== run.period.id) {
    return failCommand("关账运行的公司与期间不一致", 409, "runId");
  }
  const requestFingerprint = sha256CanonicalJson({
  ...
  const existing = await deps.findEvent(input.idempotencyKey);
  if (existing) {
    const same = existing.eventKind === "completed"
      && existing.requestFingerprint === requestFingerprint
      && existing.run.id === run.id;
    if (!same) return failCommand("幂等键已用于不同的关账命令", 409, "idempotencyKey");
  } else if (run.status !== "open") {
    return failCommand("仅开放中的关账运行可以完成", 409, "runId");
  } else if (run.period.isClosed) {
    return failCommand("会计期间已关闭，不能重复完成关账", 409, "runId");
  } else if (run.version !== input.expectedVersion) {
    return failCommand("关账运行版本已变化，请刷新后重试", 409, "expectedVersion");
  }
  return okCommand<CompleteFinanceCloseCommand>({
    companyCode: run.company.code,
    year: run.period.year,
    month: run.period.month,
    companyId: run.companyId,
    periodId: run.periodId,
    isPeriodClosed: run.period.isClosed,
    runId: run.id,
    expectedVersion: input.expectedVersion,
    actorUserId: userId,
    idempotencyKey: input.idempotencyKey,
    requestFingerprint,
    idempotentRunId: existing?.run.id ?? null,
  });
}
