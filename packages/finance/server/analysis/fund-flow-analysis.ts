...
function companyRole(
  company: {
    parentOfRelations: Array<{ child: { code: string } }>;
    childOfRelations: Array<{ parent: { code: string } }>;
  },
  selectedCodes: Set<string>,
): "母公司" | "子公司" | "成员公司" {
  if (company.parentOfRelations.some((relation) => selectedCodes.has(relation.child.code))) return "母公司";
  if (company.childOfRelations.some((relation) => selectedCodes.has(relation.parent.code))) return "子公司";
  return "成员公司";
}
...
export async function getFundFlowAnalysis(input: FundFlowAnalysisInput): Promise<FundFlowAnalysis> {
  ...
  const [companies, workpaperPeriods] = await Promise.all([
    prisma.company.findMany({
      ...
      select: {
        ...
        name: true,
        sortOrder: true,
        parentOfRelations: { select: { child: { select: { code: true } } } },
        childOfRelations: { select: { parent: { select: { code: true } } } },
      },
      orderBy: { sortOrder: "asc" },
    }),
    prisma.financeStatementWorkpaper.findMany({
      where: { companyCode: { in: companyCodes }, reportType: "cashFlow" },
  ...
  const periods = commonPeriods(workpaperPeriods, activeCodes);
  ...
  const companySummaries = companies.map((company, index) => {
    ...
    const workpaperGap = roundMoney(row.breakdown.netCashChange - row.ledgerNetCashChange);
    const quality: FundFlowCompanySummary["quality"] = !row.workpaperExists ? "missing" : Math.abs(workpaperGap) > 0.01 || row.breakdown.qualityIssues.length > 0 ? "warning" : "ok";
    return {
      code: company.code,
      name: company.name,
  ...
  const warnings = companySummaries.flatMap((company, index) => {
    ...
    if (!row.workpaperExists) companyWarnings.push(`${company.name}：${input.year}年${month}月没有现金流量表底稿`);
    if (Math.abs(company.workpaperGap) > 0.01) companyWarnings.push(`${company.name}：现金流底稿净变动与现金科目流水相差 ${Math.abs(company.workpaperGap).toFixed(2)} 元`);
  ...
  const workpaperLoanInflow = facts.reduce((sum, row) => sum + (row.breakdown.sources.find((source) => source.key === "loanReceipt")?.amount ?? 0), 0);
  ...
  if (workpaperLoanInflow > 0.005 && ledgerBorrowingInflow < 0.005 && Math.abs(debtClosingBalance) < 0.005) {
    warnings.push(`现金流底稿列示取得借款 ${workpaperLoanInflow.toFixed(2)} 元，但现金流水未对应短期/长期借款科目且期末有息借款为 0；该金额很可能含单位往来或内部拆借，需复核分类。`);
  }
  const financing = activities.find((row) => row.key === "financing")!;
  const operating = activities.find((row) => row.key === "operating")!;
  const evidence = {
    workpaperCount: facts.filter((row) => row.workpaperExists).length,
    ...
    workpaperNetCashChange: netCashChange,
  ...
  return {
    scope: {
      ...
      label: `${companies.map((company) => company.name).join(" + ")}${companies.length > 1 ? "（管理汇总）" : ""}`,
      year: input.year,
      month,
