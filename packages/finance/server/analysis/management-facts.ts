...
type Workpaper = Awaited<ReturnType<typeof loadWorkpaper>>;
...
function loadWorkpaper(companyCode: string, year: number, month: number, reportType: string) {
...
function workpaperAmounts(workpaper: Workpaper): AmountMap {
  return Object.fromEntries(workpaper?.lines.map((line) => [
    line.lineCode,
    roundMoney(line.manualAmount + line.importedAmount),
  ]) ?? []);
}
...
async function loadIncomeAmounts(companyCode: string, year: number, month: number) {
  const workpaper = await loadWorkpaper(companyCode, year, month, "incomeStatement");
  if (workpaper) return { amounts: workpaperAmounts(workpaper), source: "workpaper" as const };
  ...
  for (const line of config) {
    const prefixes = companyCode === "05" ? line.canPrefixes ?? [] : line.chnPrefixes ?? [];
...
async function loadBalanceAmounts(companyCode: string, year: number, month: number) {
  const workpaper = await loadWorkpaper(companyCode, year, month, "balanceSheet");
  if (workpaper) return { amounts: workpaperAmounts(workpaper), source: "workpaper" as const };
...
export async function loadCompanyStatementFacts(
  ...
  const [income, priorIncome, balance, priorBalance, cashFlow] = await Promise.all([
    ...
    loadWorkpaper(companyCode, year, month, "cashFlow"),
  ]);
  return {
    ...
    cashFlow: workpaperAmounts(cashFlow),
...
export async function loadOperationalFacts(year: number, month: number) {
  ...
  return {
    ...
    shipmentAmount,
    receivedAmount,
    costAmount: roundMoney(costs.reduce((sum, row) => sum + (row.amount ?? 0), 0)),
    ...
    costCategories: namedTotals(costs, (row) => row.category, (row) => row.amount),
    topCostProducts: namedTotals(costs, (row) => row.productName, (row) => row.amount),
  };
}
