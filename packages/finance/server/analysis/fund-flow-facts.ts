...
export async function loadCompanyFundFlowFacts(companyCode: string, year: number, month: number) {
  const [config, workpaper, periods, vouchers] = await Promise.all([
    loadCashFlowConfig(companyCode, year),
    prisma.financeStatementWorkpaper.findUnique({
      where: { companyCode_year_month_reportType: { companyCode, year, month, reportType: "cashFlow" } },
      include: { lines: true },
    }),
  ...
  const balances = await loadBalances(periods.map((period) => period.id));
  ...
  return {
    workpaperExists: Boolean(workpaper),
    breakdown: buildCashFlowBreakdown(config, workpaper?.lines ?? []),
