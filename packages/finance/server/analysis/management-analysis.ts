...
export async function getManagementAnalysis(input: FundFlowAnalysisInput): Promise<ManagementAnalysis> {
  ...
  const sourceWarnings = statementFacts.flatMap((row, index) => {
    ...
    if (row.incomeSource === "ledger") messages.push(`${company.name}：利润表缺少同期底稿，已按已记账凭证方向发生额计算管理口径。`);
    if (row.balanceSource === "ledger") messages.push(`${company.name}：资产负债表缺少同期底稿，已按期末科目余额计算管理口径，未包含报表重分类。`);
