...
export async function reviewReclassResult(
  ...
  if (!(await resolveUserEmployeeName(userId))) throw new ReviewError("UNBOUND_EMPLOYEE", "当前账号未绑定员工档案，不能执行审核操作");
...
export async function createManualReclassResult(params: {
  ...
  if (!(await resolveUserEmployeeName(userId))) throw new ReviewError("UNBOUND_EMPLOYEE", "当前账号未绑定员工档案，不能执行重分类操作");
