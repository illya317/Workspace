...
export async function reviewReclassResult(
  ...
  if (!(await resolveUserBusinessActorName(userId))) throw new ReviewError("UNBOUND_EMPLOYEE", "当前账号未绑定员工档案且不是管理员，不能执行审核操作");
...
export async function createManualReclassResult(params: {
  ...
  if (!command.ok) throw new ReviewError("INVALID_INPUT", command.issue.message);
  const { periodId, voucherItemId, sourceAccount, targetAccount, amount, userId } = params;
  if (!(await resolveUserBusinessActorName(userId))) throw new ReviewError("UNBOUND_EMPLOYEE", "当前账号未绑定员工档案且不是管理员，不能执行重分类操作");
