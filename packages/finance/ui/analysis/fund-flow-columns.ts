...
export const companyColumns: DataSurfaceColumnSpec<FundFlowCompanySummary>[] = [
  ...
  { key: "inflow", label: "流入", required: true, align: "right", cell: (row) => formatFinanceAmount(row.inflow) },
  { key: "outflow", label: "流出", required: true, align: "right", cell: (row) => formatFinanceAmount(row.outflow) },
  { key: "net", label: "底稿净变动", required: true, align: "right", cell: (row) => money(row.netCashChange, true) },
  ...
  { key: "quality", label: "核对", required: true, cell: (row) => ({ kind: "badge", label: row.quality === "ok" ? "一致" : row.quality === "missing" ? "缺底稿" : "待复核", tone: row.quality === "ok" ? "green" : row.quality === "missing" ? "red" : "amber" }) },
];
