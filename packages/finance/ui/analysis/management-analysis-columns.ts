...
export const managementCompanyColumns: DataSurfaceColumnSpec<ManagementCompanyPerformance>[] = [
  ...
  { key: "source", label: "事实来源", required: true, cell: (row) => ({ kind: "badge", label: row.incomeSource === "missing" || row.balanceSource === "missing" ? "缺事实" : row.incomeSource === "workpaper" && row.balanceSource === "workpaper" ? "报表底稿" : "含总账回退", tone: row.incomeSource === "missing" || row.balanceSource === "missing" ? "red" : row.incomeSource === "workpaper" && row.balanceSource === "workpaper" ? "green" : "amber" }) },
];
