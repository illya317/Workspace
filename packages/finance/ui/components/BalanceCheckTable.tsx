...
import { createPageTableSection, type DataSurfaceColumnSpec, type DataSurfaceDisplaySpec } from "@workspace/core/ui";
...
function differenceDisplay(node: BalanceCheckAccountNode, side: "debit" | "credit"): DataSurfaceDisplaySpec {
  const hasDiff = node.diffDebit !== 0 || node.diffCredit !== 0;
  const value = side === "debit" ? node.diffDebit : node.diffCredit;
  return {
    kind: "text",
    value: node.children.length === 0 ? "—" : value !== 0 ? formatBalanceAmount(value) : "0.00",
    tone: hasDiff ? "danger" : "muted",
    emphasis: hasDiff ? "medium" : "normal",
  };
}
