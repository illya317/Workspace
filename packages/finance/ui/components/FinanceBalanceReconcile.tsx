        ...
        createInlineFieldsSection("balance-reconcile-filters", [
            {
              key: "company",
              label: "公司",
              spec: { valueType: "string", control: "choice", options: { source: "static", items: companies.map(company => ({ value: company.code, label: company.name })) } },
              value: companyCode,
            ...
            kind: "filters",
            actions: [{ key: "reconcile", action: "upload", label: loading ? "核对中..." : "开始核对", onClick: handleReconcile, disabled: loading }],
          }),
        ...(result ? [
          createMessageSection("balance-reconcile-summary", {
            content: `期间：${result.year}年${result.monthStart}月-${result.monthEnd}月；Excel 科目数：${result.excelRowCount}；系统科目数：${result.systemAccountCount}；一致科目数：${result.matchedCount}；差异科目数：${result.differences.length}`,
          }),
...
function createBalanceReconcileSuccessSection(): BodySurfaceSectionSpec {
...
function createMissingListSection(
  ...
  return createMessageSection(key, {
    tone,
    content: `${title}：${items.map(item => `${item.code} ${item.name}`).join("、 ")}`,
  });
}

function createDiffTableSection(differences: ReconcileDiff[]): BodySurfaceSectionSpec {
