...
export default function ReclassCandidateList({
  ...
  async function load() {
    ...
    try {
      const [scanRes, accRes] = await Promise.all([
        fetch(workspacePath(`/api/modules/finance/ledger/reclass-rules?companyCode=${companyCode}&year=${year}`)),
        fetch(workspacePath(`/api/modules/finance/ledger/accounts?companyCode=${companyCode}&year=${year}&scope=all&pageSize=2000`)),
      ]);
  ...
  async function saveRule(c: RuleCandidate, target: string) {
    ...
    const res = await fetch(workspacePath("/api/modules/finance/ledger/reclass-rules"), { method: "PUT", headers: { "Content-Type": "application/json" }, body });
  ...
  async function clearRule(c: RuleCandidate) {
    ...
    if (!ok) return;
    if (!(await fetch(workspacePath(`/api/modules/finance/ledger/reclass-rules/${c.existingRuleId}`), { method: "DELETE" })).ok) { showToast("清除失败", "error"); return; }
