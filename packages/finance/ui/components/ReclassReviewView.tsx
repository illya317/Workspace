...
import { createPageTableSection, type DataSurfaceColumnSpec } from "@workspace/core/ui";
...
export function useReclassReviewSurface({
  ...
  const columns: DataSurfaceColumnSpec<ReclassResultRow>[] = [{
    ...
    cell: row => {
      ...
      if (!itemSide) return { kind: "empty" };
      if (!isAbnormal) return { kind: "text", value: itemSide === "debit" ? "借" : "贷", tone: "muted" };
      return { kind: "badge", label: itemSide === "debit" ? "借" : "贷", tone: "red" };
    }
  }, {
    key: "amount",
    label: `金额${arrow("金额") ?? ""}`,
    ...
    cell: row => {
      ...
      const tone = isNormal ? "muted" : isAdjusted ? "info" : "success";
      return canRevise
        ? { kind: "action", action: { key: `target-${row.id}`, label: hasTarget ? targetDisplay(displayTarget) : "选择科目", size: "sm", onClick: () => setAdjustItem(row) } }
        : { kind: "text", value: hasTarget ? targetDisplay(displayTarget) : "未设置", tone };
    }
  }];
