  ...
  const columns: DataTableColumn<ReclassResultRow>[] = [
    ...
    },
    {
      ...
      render: (row) => {
        ...
        const className = isNormal && !hasTarget
          ? "inline-block cursor-pointer rounded border border-dashed border-gray-200 bg-white px-2 py-0.5 text-xs text-gray-400 hover:border-emerald-300 hover:text-emerald-600"
          : `inline-block cursor-pointer rounded border px-2 py-0.5 text-xs font-mono hover:ring-1 hover:ring-emerald-300 ${isNormal ? "border-gray-200 bg-gray-50 text-gray-500" : isAdjusted ? "border-blue-200 bg-blue-50 text-blue-700" : "border-emerald-200 bg-emerald-50 text-emerald-700"}`;
        return (
          <span className={className} onClick={() => canWrite && setAdjustItem(row)}>
