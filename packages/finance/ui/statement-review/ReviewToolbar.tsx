    ...
    <>
      {wp && (
        <PanelCard bodyClassName="flex items-center gap-3 px-4 py-3">
          ...
          {wp.id > 0 && !rv && <ActionButton onClick={onGenerate} disabled={loading} variant="primary">生成校对</ActionButton>}
          {wp.id > 0 && rv?.isStale && <ActionButton onClick={onGenerate} disabled={loading} variant="primary">重新生成校对</ActionButton>}
          ...
          {rv && changedCount > 0 && <ActionButton onClick={onSave} disabled={saving || isReadOnly} variant="primary">保存修改 ({changedCount})</ActionButton>}
          {rv && rv.status !== "confirmed" && <ActionButton onClick={onConfirm} disabled={saving || changedCount > 0} variant="primary">确认校对</ActionButton>}
        </PanelCard>
      )}
