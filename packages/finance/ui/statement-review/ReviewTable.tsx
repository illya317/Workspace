...
import { ActionButton, DataTable, PanelCard, TextField, type DataTableColumn } from "@workspace/core/ui";
...
export default function ReviewTable({
  ...
  const columns: DataTableColumn<RvLine>[] = [
    { key: "label", label: "项目", required: true, className: "font-medium text-slate-800", render: (line) => line.label },
    { key: "systemAmount", label: "系统建议", required: true, className: "w-28 text-right text-slate-400", headerClassName: "text-right", render: (line) => FMT(line.systemAmount) },
    { key: "workpaperAmount", label: "底稿输入", required: true, className: "w-28 text-right text-slate-600", headerClassName: "text-right", render: (line) => FMT(line.workpaperAmount) },
    {
      key: "adjustedAmount",
      label: "调整金额",
      required: true,
    ...
    { key: "finalAmount", label: "最终金额", required: true, className: "w-28 text-right font-medium text-slate-800", headerClassName: "text-right", render: (line) => FMT(getLineState(line).finalAmount) },
    {
      key: "status",
      label: "状态",
      required: true,
        ...
        return (
          <ActionButton
            ...
            className={`px-1.5 py-0.5 text-xs ${
              ...
              status === "adjusted" ? "border-blue-200 bg-blue-50 text-blue-700" :
              status === "flagged" ? "border-red-200 bg-red-100 text-red-700" :
      ...
      render: (line) => {
        ...
        return (
          <ActionButton
            ...
            disabled={isReadOnly}
            className={`w-full justify-start border-0 bg-transparent px-1 py-0.5 text-left shadow-none hover:bg-gray-100 disabled:bg-transparent ${
              state.comment ? "text-gray-600" : state.status === "flagged" ? "text-red-400 italic" : "text-gray-300"
            }`}
          >
