...
export default function ReviewClient({
  ...
  const isReadOnly = rv?.status === "confirmed" || !canUpdate;
  ...
  const rawReviewItems: Array<SurfaceToolbarItem | null> = [
    ...
    rv
      ? {
          kind: "text",
          key: "status",
          content: financeWorkflowStatusLabel(financeReviewRequestStatus(rv)),
        }
      : null,
  ...
  const reviewColumns: DataSurfaceColumnSpec<RvLine>[] = [{
    ...
    cell: (line) => {
      ...
      const workflowStatus = financeReviewLineStatus(status);
      if (isReadOnly) return financeWorkflowStatusBadge(workflowStatus);
      return {
        kind: "action",
        action: {
          key: `status-${line.lineCode}`,
          label: financeWorkflowStatusLabel(workflowStatus),
          size: "sm",
