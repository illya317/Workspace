...
import { ActionButton, FormField, PanelCard, SelectField } from "@workspace/core/ui";
...
export default function ReportTab() {
  ...
  const [reportType, setReportType] = useState<"balance" | "income" | "cashflow">(
    rtFromQuery && REPORT_TYPES.has(rtFromQuery) ? rtFromQuery as "balance" | "income" | "cashflow" : "balance",
  );
        ...
        <PanelCard bodyClassName="p-4">
          ...
          {data.totalLiabilitiesAndEquity !== undefined && (
            <p className="mt-2 text-center text-xs text-gray-400">
              ...
              {Math.abs((data.assets?.find((item) => item.isGrandTotal)?.amount || 0) - data.totalLiabilitiesAndEquity) > 0.01 && (
                <span className="ml-2 text-red-500">不平衡</span>
              )}
            </p>
          )}
        </PanelCard>
