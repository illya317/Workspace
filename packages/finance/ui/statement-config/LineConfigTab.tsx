...
import { createActionsSection, createMessageSection, createPageDataSection, createStatusSection, useFeedback, type DataSurfaceColumnSpec } from "@workspace/core/ui";
...
import { createLineMappingsPanelSpec } from "./LineMappingsPanel";
...
export function useLineConfigSections({
  ...
  const load = useCallback(async () => {
    setLoading(true);
    setError(null);
    const [configResponse, mappingResponse] = await Promise.all([fetch(workspacePath(`/api/modules/finance/statement-config?companyCode=${company}&year=${year}`)), fetch(workspacePath(`/api/modules/finance/statement-config/mappings?companyCode=${company}&year=${year}&statementType=balance`))]);
  ...
  async function saveMapping(accountCode: string, lineCode: string, operator: StatementOperator, mode: "create" | "update" = "create") {
  ...
  async function restoreDefault(accountCode: string) {
    ...
    const response = await fetch(workspacePath(`/api/modules/finance/statement-config/mappings?companyCode=${company}&year=${year}&statementType=balance&accountCode=${encodeURIComponent(accountCode)}`), {
      method: "DELETE"
    });
    setSaving(previous => {
      const next = new Set(previous);
  ...
  const columns = useMemo<DataSurfaceColumnSpec<LineTableRow>[]>(() => [{
    ...
    required: true,
    cell: row => {
      if (row.kind === "section") return { kind: "text", value: SECTIONS[row.section] || row.section, tone: "muted", emphasis: "medium" };
      return { kind: "text", value: row.line.label, tone: row.kind === "special" ? "muted" : "default", emphasis: "medium" };
    }
  }, {
    key: "section",
    label: "Section",
...
function createLineConfigTableSection({
  ...
  return createPageDataSection("line-config", {
          ...
          expandedRow: row => row.kind === "line" ? createLineMappingsPanelSpec({
              line: row.line,
              ...
              onRestoreDefault,
              onToggleOperator: (accountCode, lineCode, current) => onSaveMapping(accountCode, lineCode, current === "add" ? "subtract" : "add", "update"),
              onSaveMapping,
              onStartAdding,
              onCancelAdding,
              onNewAccountChange,
