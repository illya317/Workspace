...
function MappingAction({
  ...
  if (mapping.operator === "exclude") {
    return <ActionButton onClick={onRestoreDefault} disabled={saving}>恢复默认</ActionButton>;
  }
  if (isDefaultMapping(mapping)) {
    return <ActionButton onClick={onExcludeDefault} disabled={saving}>排除默认</ActionButton>;
  }
  return (
    <ActionButton
...
export default function LineMappingsPanel({
  ...
  const mappingColumns = useMemo<DataTableColumn<Mapping>[]>(() => [
      ...
      render: (mapping) => {
        ...
        return (
          <MappingAction
            mapping={mapping}
            saving={isSaving}
            onExcludeDefault={() => onExcludeDefault(mapping.accountCode, line.lineCode)}
            onRestoreDefault={() => onRestoreDefault(mapping.accountCode)}
            onToggleOperator={() => onToggleOperator(mapping.accountCode, line.lineCode, mapping.operator)}
          />
        );
      },
    },
    ...
    },
    {
      ...
      render: (mapping) => {
        ...
        return isDefaultMapping(mapping) || mapping.operator === "exclude" ? (
          <MappingStatus mapping={mapping} />
        ) : (
          <ActionButton onClick={() => onRestoreDefault(mapping.accountCode)} disabled={isSaving} variant="danger">
            删除配置
          </ActionButton>
        );
      },
  ...
  const inheritedColumns = useMemo<DataTableColumn<InheritedAcct>[]>(() => [
    ...
    { key: "debit", label: "借方", required: true, headerClassName: "text-right", cellClassName: "text-right text-gray-400", render: (account) => formatStatementAmount(account.closingDebit) },
    { key: "credit", label: "贷方", required: true, headerClassName: "text-right", cellClassName: "text-right text-gray-400", render: (account) => formatStatementAmount(account.closingCredit) },
    {
      key: "action",
      label: "操作",
      required: true,
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="flex items-center gap-2">
            <SelectField
              ...
              placeholder={`选择科目 (${filteredAccounts.length})`}
            ...
            <ActionButton
              onClick={() => onSaveMapping(newAccount, line.lineCode, "subtract")}
              disabled={!newAccount}
              variant="danger"
            >
              添加（减）
            </ActionButton>
            <ActionButton onClick={onCancelAdding}>取消</ActionButton>
          </div>
        </div>
      ) : (
        <ActionButton onClick={() => onStartAdding(line.lineCode)}>添加科目</ActionButton>
      )}
    </div>
  );
}
