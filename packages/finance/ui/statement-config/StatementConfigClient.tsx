...
function useStatementConfigToolbarItems() {
  ...
  const toolbarItems: SurfaceToolbarItems = [
    ...
    },
    {
      kind: "text",
      key: "loading-meta",
      content: loading ? "加载可用期间…" : `全部 tab 共享，已加载 ${availablePairs.length} 个（公司、年度）组合`,
    },
  ];
...
function TabContent({
  ...
  const pageProps = { navigation, toolbarItems, lifecycleBlocks, canCreate, canUpdate, canDelete };
...
export default function StatementConfigClient({
  ...
  return (
    <StatementConfigProvider>
      <TabContent user={user} canCreate={canCreate} canUpdate={canUpdate} canDelete={canDelete} />
    </StatementConfigProvider>
  );
}
