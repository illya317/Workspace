...
interface StatementConfigCtx {
  ...
  setCompanyYear: (c: string, y: number) => void;
...
export function StatementConfigProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(workspacePath("/api/modules/finance/ledger/periods"));
        if (!res.ok) return;
  ...
  // Pick the latest available (companyCode, year) pair.
  useEffect(() => {
