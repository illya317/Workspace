...
export default function ImportUploadForm({
  ...
  return <PageSurface kind="standard" embedded body={createPageBody(createImportUploadSections({
    companies,
    companyCode,
    importType,
    year,
...
export function createImportUploadSections({
  companies,
  companyCode,
  importType,
  year,
  ...
  onFileChange,
  onPreview
}: ImportUploadFormProps): BodySurfaceSectionSpec[] {
  return [
    {
    ...
    ...(file
      ? [{
          ...
          body: {
            kind: "section" as const,
            message: { tone: "muted" as const, content: `${(file.size / 1024).toFixed(1)} KB` },
          },
        }]
      : []),
  ];
