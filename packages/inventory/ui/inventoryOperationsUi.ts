...
import type { CreateInventoryDocumentInput, CreateInventoryItemInput, InventoryBatchDto, InventoryDocumentDto, InventoryImportBatchDto, InventoryItemDto, InventoryStocktakeDto, InventoryWorkspaceDto } from "../types";
...
export const itemColumns: DataSurfaceColumnSpec<InventoryItemDto>[] = [
  { key: "code", label: "物料编码", required: true, font: "mono", cell: (row) => row.code },
  { key: "name", label: "物料名称", required: true, cell: (row) => row.name },
...
export function emptyItemDraft(companyCode: string): CreateInventoryItemInput {
  return { companyCode, code: "", name: "", itemType: "finished_goods", specification: null, baseUnit: "盒", note: null };
}

export function itemFormSections(draft: CreateInventoryItemInput, onChange: (key: keyof CreateInventoryItemInput, value: unknown) => void): CreateSurfaceSectionSpec<FormSurfaceFieldSpec>[] {
  const text = (key: keyof CreateInventoryItemInput, label: string, required = false): FormSurfaceFieldSpec => ({ key: String(key), label, required, spec: { valueType: "string", control: "text", validation: required ? { required: true } : undefined }, value: String(draft[key] ?? ""), onChange: (value) => onChange(key, String(value ?? "")) });
  return [{ key: "item", title: "物料主数据", layout: { columns: 2, density: "compact" }, items: [text("code", "物料编码", true), text("name", "物料名称", true), text("itemType", "物料分类", true), text("specification", "规格"), text("baseUnit", "基础单位", true), { ...text("note", "备注"), span: 2, spec: { valueType: "string", control: "text", multiline: true }, rows: 2 }] }];
}
