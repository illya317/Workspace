...
import type { CreateInventoryDocumentInput, CreateInventoryItemInput, InventoryDocumentDto, InventoryWorkspaceDto } from "../types";
import {
  batchColumns,
  documentColumns,
...
const TABS: PageSurfaceTabBarItemSpec[] = [
  { key: "overview", label: "库存总览" }, { key: "items", label: "物料卡片" }, { key: "movements", label: "出入库" },
...
export default function InventoryOperationsClient({
  ...
  const [itemDraft, setItemDraft] = useState<CreateInventoryItemInput | null>(null);
  ...
  function closeDrafts() { setItemDraft(null); setDocumentDraft(null); setVoucherDraft(null); }
  ...
  async function saveItem() {
    if (!itemDraft) return;
    setSaving(true);
    try { await postJson("/api/modules/inventory/operations/items", itemDraft); setItemDraft(null); await load(); return { outcome: "saved" as const, message: "物料卡片已创建" }; }
    finally { setSaving(false); }
  }
  ...
  function itemCreateSection(): BodySurfaceSectionSpec { return { key: "item-create", chrome: "plain", body: { kind: "create", create: {
    id: "inventory-item-create", trigger: "toolbar", presentation: "modal", title: "新建物料卡片", open: Boolean(itemDraft), canCreate, disabled: saving,
    content: { kind: "sections", sections: itemFormSections(itemDraft ?? emptyItemDraft(companyCode), (key, value) => setItemDraft((current) => ({ ...(current ?? emptyItemDraft(companyCode)), [key]: value }) as CreateInventoryItemInput)) },
    submission: { action: "save", disabled: saving || !itemDraft?.code || !itemDraft.name || !itemDraft.baseUnit, execute: saveItem }, onOpenChange: (open) => setItemDraft(open ? emptyItemDraft(companyCode) : null), onCancel: () => setItemDraft(null),
  } } }; }
  ...
  function documentCreateSection(): BodySurfaceSectionSpec {
    ...
    return { key: "document-create", chrome: "plain", body: { kind: "create", create: {
  ...
  function voucherLinkSection(): BodySurfaceSectionSpec {
    ...
    return { key: "voucher-link", chrome: "plain", body: { kind: "create", create: {
...
function buildSections(view: InventoryView, workspace: InventoryWorkspaceDto | null, items: InventoryWorkspaceDto["items"], documents: InventoryWorkspaceDto["documents"], rowActions: (row: InventoryDocumentDto) => DataSurfaceRowActionSpec[]): BodySurfaceSectionSpec[] {
  ...
  if (view === "items") return [createMetricsSection("item-metrics", { metrics: [{ key: "count", label: "物料卡片", value: items.length }, { key: "quantity", label: "账面库存", value: formatNumber(items.reduce((sum, row) => sum + row.onHand, 0)) }] }), createPageTableSection("inventory-items", { rows: items, columns: itemColumns, visibleColumns: itemColumns.map((column) => column.key), rowKey: (row) => row.id, emptyText: "暂无物料卡片", presentation: { density: "compact" }, scroll: { x: true } })];
