...
export const createInventoryItemSchema = z.object({ companyCode: z.string().trim().min(1), code: z.string().trim().min(1), name: z.string().trim().min(1), itemType: z.string().trim().min(1), specification: nullableText, baseUnit: z.string().trim().min(1), note: nullableText });
export const createInventoryDocumentSchema = z.object({
  companyCode: z.string().trim().min(1), documentNo: z.string().trim().min(1), documentType: z.enum(["receipt", "issue", "adjustment", "transfer"]), documentDate: z.string().trim().min(10), counterparty: nullableText, referenceNo: nullableText, note: nullableText,
