...
import type { CreateInventoryDocumentInput, CreateInventoryItemInput, InventoryWorkspaceDto, LinkInventoryVoucherInput } from "../types";
import { inventoryAccountingAdapter } from "./accounting-adapter";
import { calculateMovingWeightedAverage } from "./calculator";
import { buildCreateInventoryDocumentCommand, buildCreateInventoryItemCommand, buildInventoryDocumentLifecycleCommand, buildLinkInventoryVoucherCommand } from "./domain/inventory-validation";
...
export async function createInventoryItem(input: CreateInventoryItemInput, userId: number) {
  const command = buildCreateInventoryItemCommand(input, userId);
  if (!command.ok) throw new Error(command.issue.message);
  input = command.data.input;
  return prisma.inventoryItem.create({ data: { ...input, specification: input.specification || null, note: input.note || null, editedBy: userId } });
}
...
export async function listInventoryWorkspace(scope: { companyCode: string; year: number; month: number }): Promise<InventoryWorkspaceDto> {
  ...
  return {
    ...
    metrics: { itemCount: itemDtos.length, onHandQuantity: snapshot.onHandQuantity, receiptQuantity, issueQuantity, nearExpiryBatchCount: batchDtos.filter((batch) => batch.status === "near_expiry" && batch.onHand > 0).length, stocktakeVariance: quantity(stocktakeDtos.reduce((sum, item) => sum + item.variance, 0)) },
  };
}
