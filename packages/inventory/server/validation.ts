...
export function buildCreateInventoryItemCommand(input: CreateInventoryItemInput, userId: number): DomainValidationResult<{ input: CreateInventoryItemInput; userId: number }> {
  const normalized = { ...input, companyCode: text(input.companyCode), code: text(input.code), name: text(input.name), itemType: text(input.itemType), baseUnit: text(input.baseUnit) };
  if (!normalized.companyCode) return failCommand("公司为必填", 400, "companyCode");
  if (!normalized.code) return failCommand("物料编码为必填", 400, "code");
  if (!normalized.name) return failCommand("物料名称为必填", 400, "name");
  if (!normalized.itemType) return failCommand("物料类型为必填", 400, "itemType");
  if (!normalized.baseUnit) return failCommand("基础单位为必填", 400, "baseUnit");
