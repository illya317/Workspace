...
export function parseInventoryWorkbook(buffer: Buffer): ParsedInventoryWorkbook {
  ...
  const issueLines = issueRows.map<InventorySourceLine>((row, index) => {
    ...
    return {
      ...
      note: String(row[17] ?? "").trim() || undefined,
    };
  });
...
export async function importInventoryWorkbook(input: { buffer: Buffer; sourceFile: string; companyCode: string; userId?: number }) {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    const warehouse = await tx.inventoryWarehouse.upsert({ where: { companyCode_code: { companyCode: input.companyCode, code: "MAIN" } }, create: { companyCode: input.companyCode, code: "MAIN", name: "主仓库" }, update: { name: "主仓库", status: "active" } });
    ...
    for (const item of itemDefinitions) {
      const saved = await tx.inventoryItem.upsert({
        ...
        create: { companyCode: input.companyCode, code: item.code, name: item.name, itemType: "finished_goods", specification: item.specification, baseUnit: item.baseUnit, sourceFile: input.sourceFile, sourceSheet: item.sheet, sourceKey: item.code, editedBy: input.userId },
        update: { name: item.name, specification: item.specification, baseUnit: item.baseUnit, sourceFile: input.sourceFile, sourceSheet: item.sheet, editedBy: input.userId, version: { increment: 1 } },
      });
    ...
    await ensurePostedDocument(tx, { companyCode: input.companyCode, sourceFile: input.sourceFile, warehouseId: warehouse.id, itemId: itemByCode.get("MASK-BOX")!, line: { sheet: "面膜", sourceRow: 3, documentNo: "OPENING-MASK-202603", documentType: "receipt", documentDate: "2026-03-31", itemCode: "MASK-BOX", itemName: "面膜", baseUnit: "盒", quantity: parsed.stocktake.bookQuantity, note: "2026年3月末期初库存" }, userId: input.userId });
    for (const line of parsed.lines) {
      ...
      await ensurePostedDocument(tx, { companyCode: input.companyCode, sourceFile: input.sourceFile, warehouseId: warehouse.id, itemId, batchId, line, userId: input.userId });
    }
    const stocktake = await tx.inventoryStocktake.upsert({
      ...
      create: { companyCode: input.companyCode, stocktakeNo: "PD-202604-MASK", warehouseId: warehouse.id, stocktakeDate: "2026-04-30", status: "reviewed", sourceFile: input.sourceFile, sourceSheet: "面膜", sourceKey: "面膜:3", createdBy: input.userId },
      update: { stocktakeDate: "2026-04-30", status: "reviewed" },
    });
    ...
    const sheetCounts = [{ sheet: "面膜", rows: 1, docs: 1 }, { sheet: "阿胶浓浆入库明细表", rows: parsed.checks.receiptRows, docs: parsed.checks.receiptRows }, { sheet: "阿胶浓浆出库明细表", rows: parsed.checks.issueRows, docs: parsed.checks.issueRows }];
    for (const summary of sheetCounts) await tx.inventoryImportBatch.upsert({ where: { companyCode_checksum_sourceSheet: { companyCode: input.companyCode, checksum, sourceSheet: summary.sheet } }, create: { companyCode: input.companyCode, sourceFile: input.sourceFile, sourceSheet: summary.sheet, checksum, itemCount: summary.sheet === "面膜" ? 1 : 2, documentCount: summary.docs, rowCount: summary.rows, warningCount: summary.sheet === "面膜" ? 1 : 0, importedBy: input.userId, note: summary.sheet === "面膜" ? "源表盘点差异为绝对值10，系统按实盘-账面记录为-10" : null }, update: { documentCount: summary.docs, rowCount: summary.rows, importedBy: input.userId } });
    return { itemCount: itemDefinitions.length, documentCount: parsed.lines.length + 1, batchCount: await tx.inventoryBatch.count({ where: { item: { companyCode: input.companyCode } } }), stocktakeVariance: parsed.stocktake.variance, checks: parsed.checks };
  });
}
...
async function cleanupStaleSourceDocuments(tx: Prisma.TransactionClient, input: { companyCode: string; sourceFile: string }, parsed: ParsedInventoryWorkbook) {
  const validKeys = new Set(["面膜:3", ...parsed.lines.map((line) => `${line.sheet}:${line.sourceRow}`)]);
...
async function ensurePostedDocument(tx: Prisma.TransactionClient, input: { companyCode: string; sourceFile: string; warehouseId: number; itemId: number; batchId?: number | null; line: InventorySourceLine; userId?: number }) {
  ...
  const document = await tx.inventoryDocument.create({ data: { companyCode: input.companyCode, documentNo: input.line.documentNo, documentType: input.line.documentType, documentDate: input.line.documentDate, status: "posted", note: input.line.note, sourceFile: input.sourceFile, sourceSheet: input.line.sheet, sourceKey, createdBy: input.userId, postedBy: input.userId, postedAt: new Date(), lines: { create: { itemId: input.itemId, warehouseId: input.warehouseId, batchId: input.batchId ?? null, quantity: input.line.quantity, unit: input.line.baseUnit, unitFactor: 1, unitPrice: input.line.unitPrice, paymentStatus: input.line.paymentStatus, invoiceStatus: input.line.invoiceStatus, sourceRow: input.line.sourceRow, sourceKey } } }, include: { lines: true } });
  const line = document.lines[0];
  const sign = input.line.documentType === "issue" ? -1 : 1;
  await tx.inventoryLedgerEntry.create({ data: { documentLineId: line.id, companyCode: input.companyCode, itemId: input.itemId, warehouseId: input.warehouseId, batchId: input.batchId ?? null, movementDate: input.line.documentDate, signedQuantity: sign * input.line.quantity, unitCost: input.line.unitPrice } });
  return document;
}
