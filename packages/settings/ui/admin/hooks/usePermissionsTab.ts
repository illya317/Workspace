...
import type { PermissionMatrixHoveredAction } from "@workspace/platform/ui/permission-matrix-model";
...
export function usePermissionsTab(
  ...
  const [resourceContext, setResourceContext] = useState<PermissionResourceContext>({
    resourceKey: null,
    scopeId: null,
    projection: "default",
  });
  const selectedResource = resourceContext.resourceKey;
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (resourceContext.scopeId) params.set("scopeId", resourceContext.scopeId);
      if (resourceContext.projection === "space") params.set("projection", "space");
  ...
  }, [enabled, resourceContext.projection, resourceContext.scopeId, selectedResource, showToast, subjectType]);
  ...
  }, [resourceContext.scopeId, selectedResource, subjectType]);
  ...
  async function toggleGrant(subject: Subject, actionKey: PermissionActionKey | string) {
    if (subjectType === "user" && !canManageUserGrants) {
      showToast("个人直授仅限内置 admin 账号维护", "error");
      return;
    }
    ...
    try {
      const res = await fetch(workspacePath("/api/settings/admin/permission-grants"), {
        ...
        body: JSON.stringify({
          ...
          ...(resourceContext.scopeId ? { scopeId: resourceContext.scopeId } : {}),
          ...(resourceContext.projection === "space" ? { projection: "space" as const } : {}),
        }),
      });
      if (res.ok) {
  ...
  return {
    ...
    selectedResource,
    resourceContext,
    setSelectedResource: (resourceKey: string | null) => setResourceContext({
      resourceKey,
      scopeId: null,
      projection: "default",
    }),
