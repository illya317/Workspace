...
import { getPermissionActionRecordSortScore, sortPermissionSubjectsByScore } from "@workspace/platform/ui/permission-matrix-model";
...
export function usePermissionFilters(
  ...
  const departmentFilterOptions = useMemo(() => {
    const options = new Map<string, { value: string; name: string; searchText: string }>();
  ...
  const nameSearchOptions = useMemo(() => {
    const options = new Map<string, { value: string; name: string; details?: string; searchText: string }>();
    for (const subject of rawSubjects) {
      const name = subject.name.trim();
      if (!name || options.has(name)) continue;
      const employeeId = String(subject.extra?.employeeId ?? "");
      const details = subjectType === "user"
        ? [employeeId, ...(subject.extra?.deptPath ?? [])].filter(Boolean).join(" · ")
        : "";
      options.set(name, {
        value: name,
        name,
