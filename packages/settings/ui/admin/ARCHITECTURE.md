...
源码分析使用递归下钻工作台。左侧“源码治理”树固定展示“产品模块 / 共享与底座 / 工程体系 -> L1 -> 任意深度 Module”，包括零直接文件但拥有子树的组合父节点；因此所有已登记 L2/L3/L4 和未来更深 Module 都有可选择入口。右侧根层保留 L1 职责与依赖关系矩阵；选择分类只看该分类的 L1 概览，选择 L1 或任意 Module 后只列直属子 Module，再沿行或左树逐层深入。父节点同时显示直接代码与完整子树代码，不能用零直接实现掩盖聚合体量；叶子展示最细 role 构成、依赖目标和 Hygiene 复核项。

子 Module 表头按语义生成而非复用一张矩阵：产品/系统源码展示入口、应用、适配、领域、契约和保障；共享/组合层展示形态、主职责、文件、依赖和跨 Module 引用；数据底座展示生命周期、模型/历史文件、数据依赖和契约引用；生产运行展示运行角色、脚本/文件、运行依赖和调用引用；开发治理展示治理形态、检查/测试文件、检查依赖和工具引用。`ops/` 自动归生产运行，`scripts/` 默认归开发治理，确实被 release/runtime 消费的脚本通过集中注册表转入生产运行。Prisma schema/migration 进入数据底座，Zod schema 仍归对应业务单元的输入边界。

L1 矩阵按默认依赖方向展示入口、业务、适配、契约和保障；同一时间只展开一组。点击聚合格按 raw role 集合分析，点击展开明细精确到单一 raw role：来源显示蓝色、目标显示橙色，只有后端文件 SCC 证明的真实循环显示绿色。顶部仍只保留总代码、依赖结构、模块内聚和 Interface 边界四项，其中模块内聚同时显示需要 Hygiene 介入的最小 Module 数。文件循环或未解耦混合职责非零时 `source-code-analysis:check` 与 `gate:domain` 失败。声明覆盖率保留在 snapshot 和 gate 中，不在页面重复展示。声明、门禁和统计口径见 `docs/engineering/deep-module-design.md`。
