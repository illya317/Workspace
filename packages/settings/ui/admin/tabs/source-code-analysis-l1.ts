...
import { sourceCodeAnalysisL1NavigationKey, type SourceCodeAnalysisColumnDisclosure } from "./source-code-analysis-capabilities";
import {
...
export interface AnalysisTableRow extends SourceCodeAnalysisModuleRow {
  displayLines: number;
  displayRoles: SourceCodeAnalysisRoleCounts;
  rowKind: "module" | "section" | "total";
}

export const SOURCE_CODE_ANALYSIS_ROW_GROUPS: ReadonlyArray<{
  key: string;
  label: string;
...
function codeVolumeDisplay(displayLines: number, sourceLines: number): string | DataSurfaceDisplaySpec {
  ...
  return sourceLines === 0 || sourceLines < 1_000
    ...
    columns.push({
      ...
      cell: (row) => row.rowKind === "section"
        ...
        : selectableAnalysisCell(
            ...
            group,
            null,
            codeVolumeDisplay(displayGroupLines(row.displayRoles, group), displayGroupLines(row.roles, group)),
            relations,
          ),
    });
    if (expanded) columns.push(...group.roles.map((role) => roleColumn(group, role, relations)));
  }
  return columns;
}
...
function totalAnalysisRow(snapshot: SourceCodeAnalysisSnapshot, moduleRows: readonly AnalysisTableRow[]): AnalysisTableRow {
  const roles = Object.fromEntries(SOURCE_CODE_ANALYSIS_ROLES.map((role) => [
    role,
    snapshot.modules.reduce((sum, sourceModule) => sum + sourceModule.roles[role], 0),
  ])) as SourceCodeAnalysisRoleCounts;
  const displayRoles = Object.fromEntries(SOURCE_CODE_ANALYSIS_ROLES.map((role) => [
    role,
    moduleRows.reduce((sum, sourceModule) => sum + sourceModule.displayRoles[role], 0),
  ])) as SourceCodeAnalysisRoleCounts;
  return {
    key: "total",
    label: "总计",
    ...
    roles,
    displayLines: moduleRows.reduce((sum, sourceModule) => sum + sourceModule.displayLines, 0),
    displayRoles,
    dependencies: [],
    dependencyCount: 0,
    crossModuleImportCount: snapshot.modules.reduce((sum, sourceModule) => sum + sourceModule.crossModuleImportCount, 0),
...
export function analysisTableRows(snapshot: SourceCodeAnalysisSnapshot): AnalysisTableRow[] {
  const balancedRoleLines = balanceCodeVolumeMatrix(snapshot.modules.map((sourceModule) =>
    SOURCE_CODE_ANALYSIS_ROLES.map((role) => sourceModule.roles[role])));
  const moduleRows = snapshot.modules.map((sourceModule, moduleIndex): AnalysisTableRow => {
    const displayRoles = Object.fromEntries(SOURCE_CODE_ANALYSIS_ROLES.map((role, roleIndex) => [
      role,
      balancedRoleLines[moduleIndex][roleIndex],
    ])) as SourceCodeAnalysisRoleCounts;
    return {
      ...sourceModule,
  ...
  const groupedRows = SOURCE_CODE_ANALYSIS_ROW_GROUPS.flatMap((group) => {
...
export function createSourceCodeAnalysisRootSection(
  ...
  if (!snapshot) {
    return {
      ...
      header: { title: "源码模块分析" },
      body: createPageBody([createStatusSection("source-code-analysis-unavailable", {
        kind: "empty",
  ...
  const selectedCell = relationSelection?.selectedCell && selectedGroup
    ...
    : null;
  const maxModuleLines = Math.max(1, ...rows.filter((row) => row.rowKind === "module").map((row) => row.displayLines));
    ...
    body: createPageBody([
      createMetricsSection("source-code-analysis-summary", {
        metrics: [
          { key: "lines", label: "总代码", value: `${formatCodeVolumeInTenThousands(snapshot.summary.lines)} 万行` },
          {
            ...
            value: {
              kind: "text",
              value: `方向 ${snapshot.summary.invalidDependencyDirectionCount.toLocaleString("zh-CN")} · 循环 ${snapshot.summary.dependencyFileCycleCount.toLocaleString("zh-CN")}`,
              tone: snapshot.summary.invalidDependencyDirectionCount > 0 || snapshot.summary.dependencyFileCycleCount > 0
                ? "danger"
                : "success",
              font: "mono",
            },
          },
          {
            ...
            value: {
              kind: "text",
              value: `未归属 ${snapshot.summary.newUnclassifiedCapabilityFileCount.toLocaleString("zh-CN")} · 混合 ${snapshot.summary.mixedResponsibilityFileCount.toLocaleString("zh-CN")} · 复核 ${snapshot.summary.moduleHealthWarningCount.toLocaleString("zh-CN")}`,
              tone: snapshot.summary.newUnclassifiedCapabilityFileCount > 0
                || snapshot.summary.mixedResponsibilityFileCount > 0
                || snapshot.summary.moduleHealthWarningCount > 0 ? "danger" : "success",
              font: "mono",
            },
          },
          {
            ...
            value: {
              ...
              tone: snapshot.summary.newCapabilityContractViolationCount > 0
                ? "danger"
                : snapshot.summary.legacyCapabilityContractViolationCount > 0 ? "warning" : "success",
              font: "mono",
            },
          },
        ],
      }),
      {
        ...createPageTableSection("source-code-analysis-table", {
          ...
          rowKey: (row) => row.key,
          rowState: (row) => row.rowKind === "section" ? "section" : row.rowKind === "total" ? "total" : "normal",
          onRowClick: onNavigate ? (row) => {
            if (row.rowKind === "module") onNavigate(sourceCodeAnalysisL1NavigationKey(row.key));
          } : undefined,
