...
- 周报群提醒的完整业务文案由 Settings 的 `NotificationGroupPolicy.messageTemplate` 持久化并在策略编辑器中维护；Work 调度器只解析登记的问候语、会议日期/类型、汇报期间/日期范围和周报/月报变量，再把渲染后的单一 `message` 交给 Platform 发布。源码不得另藏一份不可见的完整通知文案；缺失模板、错误变量或错误通知定义必须停止该策略当次投递并留下失败日志。
