...
function defaultParentCycle(cycle: SystemOkrCycle, cycles: SystemOkrCycle[]) {
  if (cycle.periodType === "half_year") return cycles.find((item) => item.periodType === "yearly" && systemOkrCycleContains(item, cycle)) ?? null;
  if (cycle.periodType === "quarterly") return cycles.find((item) => item.periodType === "half_year" && systemOkrCycleContains(item, cycle)) ?? null;
  if (cycle.periodType === "monthly") return cycles.find((item) => item.periodType === "quarterly" && systemOkrCycleContains(item, cycle)) ?? null;
  return null;
}
...
async function listVisibleSystemOkrCycles(now: Date) {
  ...
  return selectVisibleSystemOkrCycles(rows, now)
