...
import {
  assertWorkReportDirectCommitAllowed,
  resolveWorkReportActionRuntime,
...
export async function saveWorkReport(input: {
  ...
  const items = mergeReportItems(workItems, report, previous);
  return serviceOk({ report: toReportDto(report), items, groups: groupReportItems(items), period: period.dto, reportStage });
}
...
function normalizeReportItemInput(
  ...
  return {
    ...
    workPlanKind: normalizePlanKind(item.workPlanKind),
    workItemType: normalizeWorkItemType(item.workItemType),
    ...
    reportItemKind: normalizeReportItemKind(item.reportItemKind),
    ...
    selfScore: normalizeScore(item.selfScore),
    performanceScore: normalizeScore(item.performanceScore),
...
function mergeReportItems(
  ...
  const sourceKey = (workItemId: number, reportItemKind: unknown) => `${workItemId}:${normalizeReportItemKind(reportItemKind)}`;
  ...
  const rows = workItems.map((work, index) => {
    ...
    return {
      ...
      workPlanKind: normalizePlanKind(item?.workPlanKindSnapshot || work.planKind),
      workItemType: normalizeWorkItemType(item?.workItemTypeSnapshot || work.itemType),
      ...
      reportItemKind: normalizeReportItemKind(item?.reportItemKindSnapshot || work.reportItemKind),
  ...
  const extraRows = currentItems
    ...
    .map((item) => ({
      ...
      workPlanKind: normalizePlanKind(item.workPlanKindSnapshot),
      workItemType: normalizeWorkItemType(item.workItemTypeSnapshot),
      ...
      reportItemKind: normalizeReportItemKind(item.reportItemKindSnapshot),
...
function normalizePlanKind(value: unknown): "okr" | "routine" | null {
  return value === "okr" || value === "routine" ? value : null;
}
function normalizeWorkItemType(value: unknown): "objective" | "key_result" | "task" | null {
  if (value === "objective" || value === "key_result" || value === "task") return value;
  return null;
}
function normalizeReportItemKind(value: unknown): "assessment" | "current" | "routine" | "next" {
