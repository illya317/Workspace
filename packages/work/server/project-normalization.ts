...
export const COMPANY_PROJECT_SEQUENCE_START = 1;
export const COMPANY_PROJECT_SEQUENCE_END = 99;
export const OTHER_PROJECT_SEQUENCE_START = 101;
...
export async function normalizeLeadingDepartmentForProjectType(
  ...
  if (projectType === "company") {
    ...
    if (!committee) return { error: "缺少运营委员会，无法设置公司项目归口部门" };
    if (requestedId !== null && requestedId !== committee.id) {
      return { error: "公司项目归口部门只能是运营委员会" };
    }
    return {
      value: committee.id,
...
export async function generateProjectCode(input: { projectType: ProjectType; departmentCode?: string | null; dateValue?: Date | string | null }) {
  const prefixCode = input.projectType === "department" ? input.departmentCode : COMPANY_PROJECT_CODE_PREFIX;
  if (!prefixCode) return null;
  const prefix = planCodePrefix(prefixCode, input.dateValue);
  const sequenceStart = input.projectType === "other" ? OTHER_PROJECT_SEQUENCE_START : COMPANY_PROJECT_SEQUENCE_START;
  const sequenceEnd = input.projectType === "company" ? COMPANY_PROJECT_SEQUENCE_END : Number.POSITIVE_INFINITY;
  const sequenceWidth = input.projectType === "department" ? 2 : 3;
