...
export const MEETING_ACTION_TARGET_KINDS = ["work_plan", "work_item"] as const;
...
export interface MeetingActionCandidateLinkCommand {
  candidateId: number;
  action: "ignore" | "linkWorkPlan" | "createWorkPlan" | "linkWorkItem" | "createWorkItem";
}
...
export function validateMeetingActionCandidateLink(input: Record<string, unknown>): DomainValidationResult<MeetingActionCandidateLinkCommand> {
  ...
  const action = enumValue(input.action, ["ignore", "linkWorkPlan", "createWorkPlan", "linkWorkItem", "createWorkItem"] as const, "ignore", "行动候选动作");
