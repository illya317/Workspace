...
mockModule("@workspace/platform/server/auth/admin", {
  namedExports: { isSuperAdmin: async () => superAdmin },
});
...
mockModule("@workspace/platform/server/user-identity", {
  namedExports: {
    canUserActAsActiveEmployee: async () => activeEmployee || superAdmin,
  },
});
...
function reset(input: { entry: boolean; submit: boolean; active: boolean; admin?: boolean }) {
  entryAllowed = input.entry;
  submitAllowed = input.submit;
  activeEmployee = input.active;
  superAdmin = input.admin === true;
...
test("submit permission does not bypass active-employee eligibility for a regular user", async () => {
  reset({ entry: true, submit: true, active: false });
  assert.equal(await canSubmitWorkProjectAction(7), false);
});

test("root admin can initiate a Work project without an employee profile", async () => {
  reset({ entry: true, submit: true, active: false, admin: true });
