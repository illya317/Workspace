...
import { canCreateProjectAction, canReviseProjectAction, canViewProject } from "./access";
...
export async function activateProjectPlanBaseline(input: { userId: number; projectId: number; baselineId: number }) {
  ...
  if (!(await canReviseProjectAction(input.userId, input.projectId))) return serviceError("无权限", 403);
...
async function buildBaselineSnapshot(projectId: number) {
  ...
  if (!project) return [];
  return [
    baselineItem("project", project.id, null, null, null, project.name, project.status, true, project.plannedStartDate, project.plannedEndDate),
    ...project.planPhases.map((phase) => baselineItem("phase", phase.id, "project", project.id, null, phase.name, null, false, phase.plannedStartDate, phase.plannedEndDate)),
  ];
}
...
function baselineItem(
  ...
  return { itemKind, itemId, parentKind, parentId, phaseId, name, status, isMilestone, plannedStartDate, plannedEndDate };
}
