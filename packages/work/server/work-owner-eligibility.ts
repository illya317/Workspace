import { currentEmploymentDateWhere, currentOpenEndedDateWhere, employmentIsActiveOnDate, matchesFkKeyword, type FkOption } from "@workspace/platform/server/relation-registry";
...
export async function listWorkOwnerEmployeeOptions(input: WorkOwnerEligibilityInput & {
  ...
  return rows
    .map((row) => ({
      ...
      lifecycleStatus: row.employments.some((employment) => employmentIsActiveOnDate(employment, workspaceBusinessDate(new Date()))) ? "active" as const : "inactive" as const,
    }))
