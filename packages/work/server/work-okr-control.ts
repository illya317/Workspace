...
async function resolveProjectControlScope(projectId: number): Promise<ServiceResult<WorkOkrControlScope>> {
  ...
  if (project.projectType === "company") {
    ...
    if (!committee) {
      ...
      return serviceError(`${committeeName}空间未配置，不能提交项目 OKR 绩效`, 409);
    }
    return serviceOk({
      type: "committee",
...
export async function assertWorkOkrSubmissionAllowed(
  ...
  if (period) {
    const futurePerformanceIssue = workPerformanceSubmissionPeriodIssue({
      reportStage: kind === "kr_review" ? "final" : "kr",
      periodStart: period.periodStart,
      businessDate: workspaceBusinessDate(now),
    });
    if (futurePerformanceIssue) {
      return serviceError(futurePerformanceIssue.message, futurePerformanceIssue.status);
    }
  }
