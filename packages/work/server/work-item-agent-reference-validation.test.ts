...
let localParentItemType = "task";
let localParentRoutineTaskType: string | null = "standing";
    ...
    prisma: {
      ...
      workItem: {
        ...
        findFirst: async ({ where }: { where: { id: number } }) => ({
          id: where.id,
          content: "常设职责",
          itemType: localParentItemType,
          routineTaskType: localParentRoutineTaskType,
...
const { validateAgentCreateWorkItemReferences, validateAgentWorkItemReferenceChanges } = await import("./work-item-agent-reference-validation");
...
test("new KR can select its parent without pretending to change existing evidence", async () => {
  localParentItemType = "objective";
  localParentRoutineTaskType = null;
  const result = await validateAgentCreateWorkItemReferences({
    execution,
    snapshot: {
      targetType: "department",
      targetId: 825,
  ...
  assert.equal(result.ok, true);
  localParentItemType = "task";
  localParentRoutineTaskType = "standing";
});
