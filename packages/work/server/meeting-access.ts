...
export type MeetingPermissionAction = Extract<PermissionActionKey, "entry" | "create" | "update" | "delete" | "grant">;
...
export async function canUseMeetings(userId: number, role: MeetingPermissionAction = "entry") {
...
export async function hasMeetingViewAll(userId: number) {
  if (await isSuperAdmin(userId)) return true;
  return authorize({ user: userId, resourceKey: MEETING_VIEW_ALL_RESOURCE, action: "read" });
}

export async function buildVisibleMeetingWhere(userId: number) {
  if (!(await canUseMeetings(userId, "entry"))) return { id: -1 };
...
export async function getMeetingPermissions(
  ...
  const [hasEntry, hasUpdate, hasDelete, hasApprove, canViewAll] = await Promise.all([
    canUseMeetings(userId, "entry"),
    canUseMeetings(userId, "update"),
    canUseMeetings(userId, "delete"),
    evaluatePermissionAction(userId, "work.meetings", "approve"),
    hasMeetingViewAll(userId),
  ]);
  if (!hasEntry) return emptyPermissions(false);
  ...
  const canManage = hasUpdate && (ownsMeeting || participantCanManage);
  const canEdit = hasUpdate && (ownsMeeting || recordsMeeting || participantCanEdit);
