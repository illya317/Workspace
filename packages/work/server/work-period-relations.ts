import { currentEmploymentDateWhere, matchesFkKeyword, type FkOption, type FkTargetRecord } from "@workspace/platform/server/relation-registry";
...
async function employeeIdsForUser(userId: number) {
  const rows = await prisma.employee.findMany({ where: { userId, employments: { some: currentEmploymentDateWhere() } }, select: { id: true } });
