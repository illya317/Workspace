...
import { WORK_GANTT_ZOOM_OPTIONS, type WorkGanttZoom } from "../../gantt";

export type ProjectGanttZoom = WorkGanttZoom;
...
export const PROJECT_GANTT_ZOOM_OPTIONS = WORK_GANTT_ZOOM_OPTIONS;
...
export function buildProjectGanttRows(input: {
  ...
  for (const project of input.data.projects) {
    ...
    addNode({
      ...
      stages: [],
      milestoneEvents: project.actualEndDate && project.status === "done"
        ? [{ key: `project-milestone:${project.id}`, name: `${project.name} 已完成`, date: project.actualEndDate }]
        : [],
  ...
  for (const task of input.data.tasks) {
    ...
    addNode({
      ...
      stages: [],
      milestoneEvents: task.isMilestone && (task.actualEndDate || task.actualStartDate)
        ? [{ key: `task-milestone:${task.id}`, name: task.name || task.description, date: task.actualEndDate || task.actualStartDate || "" }]
        : [],
