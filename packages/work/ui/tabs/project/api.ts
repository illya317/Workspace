...
export async function listProjectSpaces() {
  const res = await fetch(workspacePath("/api/modules/work/projects/spaces"));
  if (!res.ok) {
    const data = await res.json().catch(() => ({}));
    throw new Error(data.error || "加载项目空间失败");
  }
  const data = await res.json();
  return {
    spaces: (data.spaces || []) as ProjectSpace[],
    preferredDepartmentIds: (data.preferredDepartmentIds || []) as number[],
  };
}
...
export async function saveProjectPlanGantt(projectId: number, items: Array<Pick<ProjectPlanItem, "kind" | "id" | "actualStartDate" | "actualEndDate" | "phaseId">>) {
  const res = await fetch(workspacePath(`/api/modules/work/projects/${projectId}/plan-gantt`), {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
