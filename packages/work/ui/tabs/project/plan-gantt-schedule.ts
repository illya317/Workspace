...
export function baselineMap(baseline: ProjectPlanBaseline | null) {
  const map = new Map<string, { plannedStartDate: string | null; plannedEndDate: string | null; name: string }>();
...
export function cascadePlanDates(
  ...
  while (queue.length) {
    ...
    const predecessorEnd = parsePlanDate(predecessor?.actualEndDate);
    ...
    for (const dependency of successorsByKey.get(key) || []) {
      ...
      const duration = durationDays(successor.actualStartDate, successor.actualEndDate);
      if (duration === null) continue;
      const nextStart = addDays(predecessorEnd, dependency.lagDays ?? 1);
      successor.actualStartDate = formatPlanDate(nextStart);
      successor.actualEndDate = formatPlanDate(addDays(nextStart, duration));
...
export function hasIncompleteDate(item: { actualStartDate: string | null; actualEndDate: string | null }) {
  return Boolean(item.actualStartDate) !== Boolean(item.actualEndDate);
}
