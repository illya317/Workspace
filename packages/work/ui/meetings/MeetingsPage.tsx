...
import { createMasterDetailBody, createMessageSection, createPageBody, createSectionSection, PageSurface, useFeedback } from "@workspace/core/ui";
...
export default function MeetingsPage({
  ...
  async function handleCreateMeeting() {
    if (!canCreate) throw new Error("无权限新建会议");
    if (!createDraft.title.trim() || !createDraft.typeId) throw new Error("请填写会议标题和类型");
    setSaving(true);
    try {
      const data = await requestJson<{
        meeting: MeetingDetail;
      ...
      await loadMeetings();
      return { outcome: "saved" as const, message: "会议已创建" };
    } catch (err) {
      throw err instanceof Error ? err : new Error("新建会议失败");
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <PageSurface kind="standard"
      ...
      body={createMasterDetailBody({
        master: { label: "会议列表", presentation: "compact", body: {
          kind: "selector",
          selector: {
            kind: "list",
            title: "会议列表",
            ...
            loadingText: "加载中...",
            emptyText: "暂无会议",
            items: filteredMeetings.map((item) => ({
              key: item.id,
              value: item,
              card: {
                title: item.title,
        ...
        detail: createPageBody([createSectionSection("meeting-workspace", {
          title: "会议",
