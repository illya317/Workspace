...
import { BlockCreatePanel, CalendarDateInput, CheckboxField, DatabasePageFrame, EmptyStateCard, FormField, PanelCard, SectionCard, SelectField, SelectorCard, TextField, TimeField, Toast, Toolbar, ToolbarOptionGroup, getToolbarActionClassName } from "@workspace/core/ui";
...
export default function MeetingsPage({
  ...
  useEffect(() => {
    if (selectedId) void loadDetail(selectedId);else setMeeting(null);
  ...
  return <DatabasePageFrame>
      <BlockCreatePanel title="会议" canCreate creating={creating} disabled={saving} submitting={saving} submitDisabled={saving || !createDraft.title.trim() || !createDraft.typeId} addLabel="新建会议" submitLabel="保存会议" onStartCreate={() => setCreating(true)} onCancelCreate={() => setCreating(false)} onSubmitCreate={() => void handleCreateMeeting()} createContent={<div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
            <SelectBox label="会议类型" value={createDraft.typeId} options={types.map(type => ({
        value: String(type.id),
        label: type.name
      }))} onChange={typeId => setCreateDraft(draft => ({
        ...draft,
        typeId
      }))} />
            <InputBox label="会议主题" value={createDraft.title} onChange={title => setCreateDraft(draft => ({
        ...draft,
        title
      }))} />
            <InputBox label="地点" value={createDraft.location} onChange={location => setCreateDraft(draft => ({
        ...draft,
        location
      }))} />
            <InputBox label="开始时间" kind="datetime" value={createDraft.startAt} onChange={startAt => setCreateDraft(draft => ({
        ...draft,
        startAt
      }))} />
            <InputBox label="结束时间" kind="datetime" value={createDraft.endAt} onChange={endAt => setCreateDraft(draft => ({
        ...draft,
        endAt
      }))} />
        ...
        value: "public",
        label: "模块内公开"
      }]} onChange={visibility => setCreateDraft(draft => ({
        ...draft,
        visibility: visibility as CreateMeetingDraft["visibility"]
      }))} />
            <InputBox label="参会用户 ID" value={createDraft.participantUserIds} onChange={participantUserIds => setCreateDraft(draft => ({
        ...draft,
        participantUserIds
      }))} />
            <InputBox label="说明" value={createDraft.description} onChange={description => setCreateDraft(draft => ({
        ...draft,
        description
      }))} className="xl:col-span-2" />
          </div>} bodyClassName="p-4">
        <div className="space-y-4">
          ...
          key: "refresh",
          section: "action",
          content: <button type="button" onClick={() => void loadMeetings()} disabled={loading || saving} className={getToolbarActionClassName("secondary", "sm")}>刷新</button>
        }]} />
        <div className="grid gap-4 xl:grid-cols-[20rem_minmax(0,1fr)]">
          <MeetingList loading={loading} meetings={filteredMeetings} selectedId={selectedId} onSelect={setSelectedId} />
          <main className="min-w-0">
            {!meeting ? <EmptyStateCard compact={false}>{detailLoading ? "加载中..." : "暂无会议"}</EmptyStateCard> : <div className="space-y-4">
                ...
                <div className="grid gap-4 2xl:grid-cols-2">
                  <Section title="参会人">
                    ...
                    {canEdit && <InlineForm>
                        <InputBox label="用户 ID" value={participantDraft.userId} onChange={userId => setParticipantDraft(draft => ({
                      ...draft,
                      userId
                    }))} />
                        <SelectBox label="角色" value={participantDraft.role} options={ROLE_OPTIONS} onChange={role => setParticipantDraft(draft => ({
                      ...draft,
                      role,
                      canVote: role === "owner" || role === "voter"
                    }))} />
                        <FormField label="可投票" layout="inline" className="self-end">
                          <CheckboxField checked={participantDraft.canVote} onChange={checked => setParticipantDraft(draft => ({
                        ...draft,
                        canVote: checked
                      }))} />
                        </FormField>
                        <div className="self-end">
                          <button type="button" disabled={saving || !participantDraft.userId} onClick={() => void mutate<{
                        meeting: MeetingDetail;
                  ...
                  <Section title="议程">
                    ...
                    {canEdit && <InlineForm>
                        <InputBox label="议题" value={agendaDraft.title} onChange={title => setAgendaDraft(draft => ({
                      ...draft,
                      title
                    }))} />
                        <InputBox label="说明" value={agendaDraft.description} onChange={description => setAgendaDraft(draft => ({
                      ...draft,
                      description
                    }))} />
                        <div className="self-end">
                          <button type="button" disabled={saving || !agendaDraft.title.trim()} onClick={() => void mutate<{
                        meeting: MeetingDetail;
                      }>(`/api/modules/work/meetings/${meeting.id}/agenda`, agendaDraft, "议题已新增", () => setAgendaDraft({
                        title: "",
                        description: ""
                  ...
                  <Section title="纪要">
                    ...
                    {canEdit && <InlineForm>
                        <AgendaSelect meeting={meeting} value={minuteDraft.agendaItemId} onChange={agendaItemId => setMinuteDraft(draft => ({
                      ...draft,
                      agendaItemId
                    }))} />
                        <InputBox label="内容" value={minuteDraft.content} onChange={content => setMinuteDraft(draft => ({
                      ...draft,
                      content
                    }))} className="md:col-span-2" />
                        <div className="self-end">
                          <button type="button" disabled={saving || !minuteDraft.content.trim()} onClick={() => void mutate<{
                        meeting: MeetingDetail;
                      }>(`/api/modules/work/meetings/${meeting.id}/minutes`, normalizeOptionalIds(minuteDraft), "纪要已记录", () => setMinuteDraft({
                        agendaItemId: "",
                        content: ""
                      }))} className={getToolbarActionClassName("primary", "sm")}>
                            记录纪要
                  ...
                  <Section title="表决">
                    <ProposalList meeting={meeting} saving={saving} onVote={(proposalId, choice) => void mutate<{
                    meeting: MeetingDetail;
                  }>(`/api/modules/work/meetings/${meeting.id}/votes`, {
                    action: "cast",
                    proposalId,
                    choice
                  }, "投票已保存")} onClose={proposalId => void mutate<{
                    meeting: MeetingDetail;
                  }>(`/api/modules/work/meetings/${meeting.id}/votes`, {
                    action: "close",
                    proposalId
                    ...
                    {canEdit && <InlineForm>
                        <AgendaSelect meeting={meeting} value={proposalDraft.agendaItemId} onChange={agendaItemId => setProposalDraft(draft => ({
                      ...draft,
                      agendaItemId
                    }))} />
                        <InputBox label="表决事项" value={proposalDraft.title} onChange={title => setProposalDraft(draft => ({
                      ...draft,
                      title
                    }))} />
                      ...
                      value: "anonymous",
                      label: "匿名展示"
                    }]} onChange={voteVisibility => setProposalDraft(draft => ({
                      ...draft,
                      voteVisibility
                    }))} />
                        <InputBox label="最低人数" value={proposalDraft.minVotesRequired} onChange={minVotesRequired => setProposalDraft(draft => ({
                      ...draft,
                      minVotesRequired
                    }))} />
                        <InputBox label="内容" value={proposalDraft.content} onChange={content => setProposalDraft(draft => ({
                      ...draft,
                      content
                    }))} className="md:col-span-2" />
                        <div className="self-end">
                          <button type="button" disabled={saving || !proposalDraft.title.trim()} onClick={() => void mutate<{
                        meeting: MeetingDetail;
                      }>(`/api/modules/work/meetings/${meeting.id}/votes`, {
                        action: "create",
                        ...normalizeOptionalIds(proposalDraft)
                      }, "表决已创建", () => setProposalDraft({
                        agendaItemId: "",
                        title: "",
                        content: "",
                        voteVisibility: "named",
                  ...
                  <Section title="决议 / 指导">
                    ...
                    {canEdit && <InlineForm>
                        <AgendaSelect meeting={meeting} value={decisionDraft.agendaItemId} onChange={agendaItemId => setDecisionDraft(draft => ({
                      ...draft,
                      agendaItemId
                    }))} />
                        <SelectBox label="类型" value={decisionDraft.kind} options={DECISION_KIND_OPTIONS} onChange={kind => setDecisionDraft(draft => ({
                      ...draft,
                      kind
                    }))} />
                        <InputBox label="生效日期" kind="date" value={decisionDraft.effectiveDate} onChange={effectiveDate => setDecisionDraft(draft => ({
                      ...draft,
                      effectiveDate
                    }))} />
                        <InputBox label="标题" value={decisionDraft.title} onChange={title => setDecisionDraft(draft => ({
                      ...draft,
                      title
                    }))} />
                        <InputBox label="内容" value={decisionDraft.content} onChange={content => setDecisionDraft(draft => ({
                      ...draft,
                      content
                    }))} className="md:col-span-2" />
                        <div className="self-end">
                          <button type="button" disabled={saving || !decisionDraft.title.trim()} onClick={() => void mutate<{
                        meeting: MeetingDetail;
                      }>(`/api/modules/work/meetings/${meeting.id}/decisions`, normalizeOptionalIds(decisionDraft), "决议已保存", () => setDecisionDraft({
                        agendaItemId: "",
                        proposalId: "",
                        kind: "decision",
                        title: "",
                  ...
                  <Section title="行动候选">
                    <CandidateList meeting={meeting} actionDrafts={actionDrafts} saving={saving} onDraftChange={(candidateId, draft) => setActionDrafts(current => ({
                    ...current,
                    [candidateId]: draft
                  }))} onAction={(candidateId, action, draft) => void mutate<{
                    meeting: MeetingDetail;
                  }>(`/api/modules/work/meetings/${meeting.id}/action-candidates`, actionPayload(candidateId, action, draft, user), "行动候选已处理")} />
                    {canEdit && <InlineForm>
                        <AgendaSelect meeting={meeting} value={candidateDraft.agendaItemId} onChange={agendaItemId => setCandidateDraft(draft => ({
                      ...draft,
                      agendaItemId
                    }))} />
                        <DecisionSelect meeting={meeting} value={candidateDraft.decisionId} onChange={decisionId => setCandidateDraft(draft => ({
                      ...draft,
                      decisionId
                    }))} />
                      ...
                      value: "project_task",
                      label: "项目任务"
                    }]} onChange={targetKind => setCandidateDraft(draft => ({
                      ...draft,
                      targetKind
                    }))} />
                        <InputBox label="候选事项" value={candidateDraft.title} onChange={title => setCandidateDraft(draft => ({
                      ...draft,
                      title
                    }))} />
                        <InputBox label="说明" value={candidateDraft.description} onChange={description => setCandidateDraft(draft => ({
                      ...draft,
                      description
                    }))} className="md:col-span-2" />
                        <div className="self-end">
                          <button type="button" disabled={saving || !candidateDraft.title.trim()} onClick={() => void mutate<{
                        meeting: MeetingDetail;
                      }>(`/api/modules/work/meetings/${meeting.id}/action-candidates`, normalizeOptionalIds(candidateDraft), "行动候选已新增", () => setCandidateDraft({
                        agendaItemId: "",
                        decisionId: "",
                        title: "",
                        description: "",
...
async function requestJson<T>(path: string, init?: {
  method?: string;
