    ...
    sections: [
      createListSection("participant-list", {
        ...
        items: meeting.participants.map((participant) => ({
          key: participant.id,
          title: participant.userName || "未知人员",
      ...
      ...(canEdit ? [createInlineFieldsSection("participant-form", [
        ...
        actions: [{ key: "save-participant", action: "save", label: "保存参会人", disabled: saving || !participantDraft.userId, onClick: () => void onMutate<{ meeting: MeetingDetail }>(`/api/modules/work/meetings/${meeting.id}/participants`, participantDraft, "参会人已保存") }],
      })] : []),
    ],
  });
}
  ...
  return createPanelSection("agenda", {
    ...
    sections: [
      ...
      ...(meeting.permissions.canEdit ? [createInlineFieldsSection("agenda-form", [
        ...
        actions: [{ key: "add-agenda", action: "add", label: "新增议题", disabled: saving || !agendaDraft.title.trim(), onClick: () => void onMutate<{ meeting: MeetingDetail }>(`/api/modules/work/meetings/${meeting.id}/agenda`, agendaDraft, "议题已新增", () => onAgendaDraftChange({ title: "", description: "" })) }],
      })] : []),
    ],
  });
}
  ...
  return createPanelSection("minutes", {
    ...
    sections: [
      ...
      ...(meeting.permissions.canEdit ? [createInlineFieldsSection("minute-form", [
        ...
        actions: [{ key: "add-minute", action: "edit", label: "记录纪要", disabled: saving || !minuteDraft.content.trim(), onClick: () => void onMutate<{ meeting: MeetingDetail }>(`/api/modules/work/meetings/${meeting.id}/minutes`, normalizeOptionalIds(minuteDraft), "纪要已记录", () => onMinuteDraftChange({ agendaItemId: "", content: "" })) }],
      })] : []),
    ],
  });
}
    ...
    sections: [
      createListSection("proposal-list", {
        ...
        items: meeting.proposals.map((proposal) => ({
          ...
          meta: proposal.votes.length ? proposal.votes.map((vote) => `${vote.voterName || "未知人员"}：${voteChoiceLabel(vote.choice)}`).join(" · ") : undefined,
          actions: [
            ...
            ...(meeting.permissions.canApprove && proposal.status === "open" ? [{ key: "close", label: "关闭表决", icon: "approve" as const, variant: "secondary" as const, size: "sm" as const, disabled: saving, onClick: () => void onMutate<{ meeting: MeetingDetail }>(`/api/modules/work/meetings/${meeting.id}/votes/${proposal.id}/close`, {}, "表决已关闭") }] : []),
      ...
      ...(meeting.permissions.canEdit ? [createInlineFieldsSection("proposal-form", [
        ...
        actions: [{ key: "create-proposal", action: "create", label: "创建表决", disabled: saving || !proposalDraft.title.trim(), onClick: () => void onMutate<{ meeting: MeetingDetail }>(`/api/modules/work/meetings/${meeting.id}/proposals`, normalizeOptionalIds(proposalDraft), "表决已创建", () => onProposalDraftChange({ agendaItemId: "", title: "", content: "", voteVisibility: "named", minVotesRequired: "" })) }],
      })] : []),
    ],
  });
}
  ...
  return createPanelSection("decisions", {
    ...
    sections: [
      ...
      ...(meeting.permissions.canEdit ? [createInlineFieldsSection("decision-form", [
        ...
        actions: [{ key: "save-decision", action: "save", label: "保存决议", disabled: saving || !decisionDraft.title.trim(), onClick: () => void onMutate<{ meeting: MeetingDetail }>(`/api/modules/work/meetings/${meeting.id}/decisions`, normalizeOptionalIds(decisionDraft), "决议已保存", () => onDecisionDraftChange({ agendaItemId: "", proposalId: "", kind: "decision", title: "", content: "", effectiveDate: "" })) }],
      })] : []),
    ],
  });
}
  ...
  return createPanelSection("candidates", {
    ...
    sections: [
        ...
        items: meeting.actionCandidates.map((candidate) => {
          ...
          return {
            ...
            meta: candidate.status === "linked"
              ? candidate.linkedWorkPlanTitle ? `目标计划：${candidate.linkedWorkPlanTitle}` : candidate.linkedWorkItemTitle ? `工作项：${candidate.linkedWorkItemTitle}` : "已链接"
              : undefined,
            sections: meeting.permissions.canEdit && candidate.status === "candidate" ? [
              {
                ...createInlineFieldsSection(`candidate-${candidate.id}-draft`, [
                  textField("workPlanId", "目标计划 ID", draft.workPlanId, (workPlanId) => onActionDraftsChange({ ...actionDrafts, [candidate.id]: { ...draft, workPlanId } })),
              ...
              },
              {
                ...createActionsSection(`candidate-${candidate.id}-actions`, [
                  { key: "linkWorkPlan", label: "链接目标计划", icon: "link", variant: "secondary", size: "sm", disabled: saving || !draft.workPlanId, onClick: () => void onMutate<{ meeting: MeetingDetail }>(`/api/modules/work/meetings/${meeting.id}/action-candidates`, actionPayload(candidate.id, "linkWorkPlan", draft, user), "行动候选已处理") },
                  { key: "createWorkPlan", label: "创建目标计划", icon: "generate", variant: "secondary", size: "sm", disabled: saving, onClick: () => void onMutate<{ meeting: MeetingDetail }>(`/api/modules/work/meetings/${meeting.id}/action-candidates`, actionPayload(candidate.id, "createWorkPlan", draft, user), "行动候选已处理") },
      ...
      ...(meeting.permissions.canEdit ? [createInlineFieldsSection("candidate-form", [
        ...
        selectField("targetKind", "目标", candidateDraft.targetKind, [{ value: "work_plan", label: "目标计划" }], (targetKind) => onCandidateDraftChange({ ...candidateDraft, targetKind })),
        ...
        actions: [{ key: "add-candidate", action: "add", label: "新增候选", disabled: saving || !candidateDraft.title.trim(), onClick: () => void onMutate<{ meeting: MeetingDetail }>(`/api/modules/work/meetings/${meeting.id}/action-candidates`, normalizeOptionalIds(candidateDraft), "行动候选已新增", () => onCandidateDraftChange({ agendaItemId: "", decisionId: "", title: "", description: "", targetKind: "work_plan" })) }],
      })] : []),
    ],
  });
}
