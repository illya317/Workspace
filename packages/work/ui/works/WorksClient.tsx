...
export default function WorksClient({ user, initialTarget, shellTitle, shellBackHref }: {
  user: SessionUser;
  ...
  const canManageOkrSettings = Boolean(user.isSuperAdmin || spaces.some((space) => space.actionPermissions.canManagePermissions));
  ...
  const periodFilteredPlans = useMemo(() => filteredPlans.filter((plan) => planMatchesPeriodFilter(plan, planPeriodFilter)), [filteredPlans, planPeriodFilter]);
  ...
  const navigationItems = useMemo(
    () => {
      ...
      const scopedItems = workViewNavigationItemsForSpace(baseItems, currentSpace?.targetType);
      return responsiveWorkNavigationItems(scopedItems);
    },
    [canManageOkrSettings, currentSpace?.targetType],
  );
