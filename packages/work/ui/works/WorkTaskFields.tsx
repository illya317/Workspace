...
export function useWorkTaskFormSurface({
  ...
  const fields: FormSurfaceFieldSpec[] = [
    { key: "content", label: "节点内容", required: true, spec: { valueType: "string", control: "text", state: disabled ? "disabled" : "normal" }, value: draft.content, placeholder: "输入目标、结果或执行任务", onChange: (value) => patch({ content: String(value ?? "") }) },
    ...
    ...(draft.periodType ? [
      { key: "periodStart", label: "周期开始", spec: { valueType: "date", control: "temporal", precision: "date", state: disabled ? "disabled" : "normal" }, value: draft.periodStart, onChange: (value: unknown) => patch({ periodStart: String(value || "") }), placeholder: "选择日期" },
      { key: "periodEnd", label: "周期结束", spec: { valueType: "date", control: "temporal", precision: "date", state: disabled ? "disabled" : "normal" }, value: draft.periodEnd, onChange: (value: unknown) => patch({ periodEnd: String(value || "") }), placeholder: "选择日期" },
    ...
    { key: "parent", label: "上级节点", spec: { valueType: "string", control: "choice", options: { source: "static", items: parentOptions, visibleCount: 5 }, state: disabled || parentOptions.length === 0 ? "disabled" : "normal" }, value: draft.parentWorkItemId ? String(draft.parentWorkItemId) : "", placeholder: "根节点", onChange: (value) => {
    ...
    ...(isKr ? [
      { key: "krStartValue", label: "结果起点", spec: { valueType: "number", control: "number", state: disabled ? "disabled" : "normal" }, value: numberValue(draft.krStartValue), onChange: patchNumber("krStartValue") },
      { key: "krCurrentValue", label: "结果当前", spec: { valueType: "number", control: "number", state: disabled ? "disabled" : "normal" }, value: numberValue(draft.krCurrentValue), onChange: patchNumber("krCurrentValue") },
      { key: "krTargetValue", label: "结果目标", spec: { valueType: "number", control: "number", state: disabled ? "disabled" : "normal" }, value: numberValue(draft.krTargetValue), onChange: patchNumber("krTargetValue") },
    ...
    { key: "description", label: "描述", span: "wide", spec: { valueType: "string", control: "text", multiline: true, state: disabled ? "disabled" : "normal" }, value: draft.description, placeholder: "描述目标、结果口径、交付物或拆解口径", onChange: (value) => patch({ description: String(value ?? "") }) },
  ];
