...
import { getWorkReportDraft, saveWorkReport } from "./api";
...
export interface WorkReportsController {
  ...
  reportingSettings: WorkReportingSettings;
...
export function useWorkReportsController({ target, onToast, enabled }: {
  ...
  const [reportingSettings, setReportingSettings] = useState<WorkReportingSettings>(DEFAULT_REPORTING_SETTINGS);
  ...
  useEffect(() => {
    if (periodType !== "weekly" && periodType !== "monthly") return;
    if (reportingSettings[periodType].enabled) return;
    const fallback = periodType === "weekly" ? "monthly" : "weekly";
    if (!reportingSettings[fallback].enabled) return;
    setPeriodType(fallback);
    setPeriodStart(periodStartForSelection(fallback, getCurrentWeekStart(), cycleOptions));
  }, [cycleOptions, periodType, reportingSettings]);
  ...
  const loadDraft = useCallback(async (options?: { isCancelled?: () => boolean }) => {
    ...
    try {
      ...
      setReportingSettings(data.reportingSettings ?? DEFAULT_REPORTING_SETTINGS);
  ...
  }, [draft, loadDraft, onToast, periodStart, periodType, saving, target]);
  ...
  const actionDisabled = loading || saving || !hasSavableDraft || draft?.reportingPolicy?.submissionAllowed === false;
  ...
  return {
    ...
    draft,
    loading,
    canEditDraft: draft?.canEdit === true
      && draft.actionRuntime.editability === "editable"
      && draft.reportingPolicy?.submissionAllowed !== false,
    periodType,
    periodStart,
    selectedCycle,
    cycleOptions,
...
const DEFAULT_REPORTING_SETTINGS: WorkReportingSettings = {
