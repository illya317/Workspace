...
export function workReportingSection(props: WorkReportingSectionProps): BodySurfaceSectionSpec {
  ...
  const reportingPolicy = props.draft?.reportingPolicy;
  const reportingStatus = !reportingPolicy
    ? null
    : !reportingPolicy.enabled
      ? { label: "已停用", tone: "muted" as const }
      : reportingPolicy.isLate
        ? { label: reportingPolicy.allowLateSubmission ? "已逾期，可补交" : "已截止", tone: "warning" as const }
        : null;
  ...
  return {
    ...
    header: {
      ...
      badges: [
        ...
        ...(reportingPolicy ? [{ key: "deadline", label: `截止 ${reportingPolicy.deadline}`, tone: "muted" as const }] : []),
        ...(reportingStatus ? [{ key: "reporting-status", ...reportingStatus }] : []),
      ],
    },
    body: {
      kind: "data",
