...
import { WORK_PLAN_FUTURE_FILTER_OPTIONS, type WorkPlanFutureFilter } from "./work-plan-future-filter";
...
export function createWorkToolbarItems({
  ...
  onPlanFutureFilterChange,
  onStatusFilterChange,
}: {
  hasSpace: boolean;
  activeTab: string;
  planPeriodFilter: WorkPlanPeriodFilter;
  planFutureFilter: WorkPlanFutureFilter;
  ...
  onPlanFutureFilterChange: (value: WorkPlanFutureFilter) => void;
  ...
  return [
    ...(activeTab === "tasks" ? [
      ...
      {
        kind: "option-group" as const,
        key: "plan-future-filter",
        value: planFutureFilter,
        options: WORK_PLAN_FUTURE_FILTER_OPTIONS,
        onChange: (value: string) => onPlanFutureFilterChange(value as WorkPlanFutureFilter),
        ariaLabel: "未来计划范围",
