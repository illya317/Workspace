...
export function ReportDraftTable({
  ...
  const rows = useMemo(() => draft?.items.map((item, index) => ({ ...item, rowIndex: index })) || [], [draft]);
  const columns: DataTableColumn<WorkReportItem & { rowIndex: number }>[] = [
    {
      key: "title",
      label: "事项",
      required: true,
      ...
      render: (item) => item.workItemId && item.source !== "stale" ? (
        <div className="break-words text-sm font-medium text-slate-900">{item.title}</div>
      ) : (
        <TextField
          value={item.title}
          readOnly={!canEdit}
          placeholder="填写事项"
          onChange={(value) => onUpdate(item.rowIndex, { title: value })}
          className="!h-9"
        />
      ),
    },
    {
      ...
      render: (item) => (
        <TextareaField
          ...
          onChange={(value) => onUpdate(item.rowIndex, { doneThisWeek: value })}
          className="resize-y"
        />
      ),
    },
    {
      ...
      render: (item) => (
        <TextareaField
          ...
          rows={3}
          placeholder="下周准备做什么"
          onChange={(value) => onUpdate(item.rowIndex, { planNextWeek: value })}
          className="resize-y"
        />
      ),
    },
    {
      ...
      render: (item) => canEdit && item.source !== "work" ? (
        <ActionButton variant="danger" onClick={() => onRemove(item.rowIndex)} className="!h-9 !px-3">
          移除
        </ActionButton>
      ) : <span className="text-xs text-slate-400">锁定</span>,
    },
  ];

  return (
    <TableScrollFrame className="overflow-y-hidden rounded-lg border border-slate-200 bg-white">
      <DataTable
        rows={rows}
        columns={columns}
        visibleColumns={[]}
...
function ReportStack({ space }: { space: WorkReportCollectionSpace }) {
  return (
        ...
        >
          ...
          <div className="space-y-2">
            {report.items.map((item) => (
              <div key={item.id || `${report.id}-${item.title}`} className="grid gap-2 text-xs leading-5 text-slate-600 lg:grid-cols-[10rem_minmax(0,1fr)_minmax(0,1fr)]">
