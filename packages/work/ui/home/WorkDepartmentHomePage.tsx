...
const baseDepartmentHomeTab = { key: "overview", label: "部门总览" };
...
export function WorkDepartmentHomePageView({
  ...
  contributions?: DepartmentHomeViewContribution[];
}) {
  ...
  const departmentHomeTabs = useMemo(
    () => [
      baseDepartmentHomeTab,
  ...
  const layout = {
    ...
    toolbarItems,
    masterLabel: "组织层级",
    ratio: [0.32, 0.68] as [number, number],
  };
  ...
  return renderAppShellPage({
    ...
    headerSelector,
    children: (
      <DepartmentHomeViewLayoutProvider value={layout}>
        {ActiveContributionView && activeDepartment
          ? <ActiveContributionView departmentId={activeDepartment.id} />
          : <DepartmentHomeViewSurface right={right} />}
      </DepartmentHomeViewLayoutProvider>
    ),
  });
}
