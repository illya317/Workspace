...
export default function QcBatchListClient({ initialRows, products, canCreate, canDelete, canExport }: Props) {
  ...
  const createItems: FormSurfaceItemSpec[] = [
    {
      ...
      label: "产品",
      required: true,
      spec: { valueType: "string", control: "choice", options: { source: "static", visibleCount: 5, items: products.map((product) => ({ value: product.id, label: product.productName })) } },
      value: productKey,
    ...
    },
    {
      ...
      value: batchNumber,
      onChange: (value) => setBatchNumber(String(value ?? "")),
      placeholder: "请输入批号",
    },
  ];
  const createBody: BodySurfaceProps = {
    kind: "create",
    create: {
      id: "qc-batch-create",
      trigger: "toolbar",
      ...
      submission: { action: "save", disabled: creatingBatch || !productKey || !batchNumber.trim(), execute: createBatch },
  ...
  return (
    <PageSurface kind="standard"
      ...
      body={createPageBody([
        { key: "qc-batch-create", body: createBody, chrome: "plain" },
        { key: "qc-workbench", body: createFixedSidebarBody({ left: queueBody, right: detailBody }) },
      ])}
