...
import { ActionButton, PanelCard, TextareaField } from "@workspace/core/ui";
  ...
  return (
    ...
    >
      <div className="mb-2 flex items-start justify-between gap-3">
        ...
        <ActionButton className="px-2 py-1" onClick={onClose} aria-label="关闭字段反馈">
          ×
        </ActionButton>
      </div>
      ...
      {entries.length > 0 ? (
        <div className="mb-2 max-h-32 space-y-2 overflow-y-auto rounded-md border border-amber-200 bg-amber-50 px-2 py-2">
      ...
      <div className="mt-3 flex justify-end gap-2">
        <ActionButton className="px-3 py-2 text-xs" onClick={onClose} disabled={saving}>
          取消
        </ActionButton>
        <ActionButton variant="primary" className="px-3 py-2 text-xs" onClick={onSave} disabled={saving || loading}>
          {saving ? "保存中" : "保存"}
        </ActionButton>
      </div>
    </PanelCard>
