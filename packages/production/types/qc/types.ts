...
export interface QcBatchSignature {
  id: number;
  fieldKey: string;
  scopeKey: string;
  ...
  authMethod: "active_session" | "legacy_file_import";
}
...
export interface QcBatchList {
  ...
  counts: { total: number; draft: number; submitted: number; signatureCount: number; fieldValueCount: number };
}

export interface QcBatchCreateInput { productId: number; productKey: string; batchNumber: string }
