...
export { countEditorDocument } from "./editor-adapter-audit";
...
export function convertQcTemplateToEditorDocument(
  detail: QcTemplateDetail,
  options: { dryingWeightMultipliers?: Record<string, number> } = {},
): QcEditorImportResult {
  ...
  annotateEditorSlots(blocks, fieldModel, options.dryingWeightMultipliers);
  ...
  const sourceCounts = countQcTemplateDetail(detail, fieldModel);
  const editorCounts = countEditorDocument(document, fieldModel);
  const audit = buildImportAudit(sourceCounts, editorCounts);
...
function partToEditorParts(part: QcLayoutPart, context: ConversionContext): EditorInlinePart[] {
  ...
  if (part.type === "text" || part.type === "hint" || part.type === "note") return [{ type: "text", text: part.text || "" }];
  if (part.type === "param") return [{ type: "text", text: part.defaultValue || part.name || "" }];
  if (part.type === "section_heading") return [{ type: "text", text: part.text || "" }];
  if (part.type === "test_value") return [{ type: "text", text: testValue(part, context.test) }];
  ...
  const commonMetadata = { source: sourceContext(context) };
  if (fieldKey) {
