...
function standardText(recordConfig: Record<string, unknown>) {
  const configuredTemplate = asString(recordConfig.standard_template);
  const template = getTenantProfile().docs.standardTemplateAliases[configuredTemplate] ?? configuredTemplate;
  ...
  if (template === "named_impurities_external_standard_limits") return namedImpuritiesExternalStandardLimitsText(params);
...
function namedImpuritiesExternalStandardLimitsText(params: Record<string, unknown>) {
