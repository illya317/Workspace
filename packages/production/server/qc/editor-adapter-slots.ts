...
import { normalizeFormulaText } from "@workspace/platform/formula";

type SlotKind = NonNullable<EditorInlinePart["slotKind"]>;
type Counters = { x: number; p: number; y: number; z: number };

export function annotateEditorSlots(
  blocks: EditorBlock[],
  fieldModel: EditorFieldModel,
  dryingWeightMultipliers: Record<string, number> = {},
) {
  insertMissingFormulaInputSlots(blocks, fieldModel, dryingWeightMultipliers);
  ...
  rewriteFormulaReferences(blocks, fieldModel, dryingWeightMultipliers);
}

function insertMissingFormulaInputSlots(
  blocks: EditorBlock[],
  fieldModel: EditorFieldModel,
  dryingWeightMultipliers: Record<string, number>,
) {
  ...
  Object.values(fieldModel.formulas).forEach((formula) => {
    ...
    formula.dependencyFieldKeys.forEach((fieldKey) => {
      ...
      if (!field || !shouldMaterializeFormulaInput(field, dryingWeightMultipliers)) return;
...
function shouldMaterializeFormulaInput(field: EditorFieldDefinition, dryingWeightMultipliers: Record<string, number>) {
  if (constantFieldReplacement(field, dryingWeightMultipliers)) return false;
...
function rewriteFormulaReferences(blocks: EditorBlock[], fieldModel: EditorFieldModel, dryingWeightMultipliers: Record<string, number>) {
  clearReferenceFormulaTexts(blocks, fieldModel);
  const references = formulaReferenceMap(blocks, fieldModel, dryingWeightMultipliers);
...
function formulaReferenceMap(blocks: EditorBlock[], fieldModel: EditorFieldModel, dryingWeightMultipliers: Record<string, number>) {
  ...
  Object.values(fieldModel.fields).forEach((field) => {
    const replacement = field.alias ?? constantFieldReplacement(field, dryingWeightMultipliers);
...
function constantFieldReplacement(
  field: EditorFieldModel["fields"][string],
  dryingWeightMultipliers: Record<string, number>,
) {
  const numeric = numericText(field.defaultValue) ?? numericText(field.recommendedValue);
  if (numeric) return numeric;
  const source = (field.source ?? field.metadata?.source ?? {}) as JsonObject;
  const productKey = typeof source.productKey === "string" ? source.productKey : "";
  const multiplier = dryingWeightMultipliers[productKey];
  if (multiplier && field.name === "规格" && field.unit === "mg") return String(multiplier);
  return null;
}
