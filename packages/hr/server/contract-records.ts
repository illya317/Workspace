...
export function normalizeContractRecord(contract: Record<string, unknown>) {
  const next = { ...contract };
  const mapping = getTenantProfile().hr.options.insuranceStatusMapping;
  if (!next.insuranceStatus && typeof next.isInsuredHere === "boolean") next.insuranceStatus = next.isInsuredHere ? mapping.insured : mapping.uninsured;
...
function contractRowFromRaw(
  ...
  return {
    ...
    isInsuredHere: contract.insuranceStatus === insuredStatus,
