...
import { loadHrPerformanceReportCollection } from "./performance-report-collection";
...
export async function executeListHrPerformanceDashboardRouteCommand(command: {
  ...
  const [hasEffectiveRead, canReadSummary, currentEmployee] = await Promise.all([
    ...
    prisma.employee.findFirst({
      where: { userId: command.userId, employments: { some: currentEmploymentDateWhere() } },
  ...
  const submissionRows = submissions
    ...
    .filter((row) => matchAnyField(row as unknown as Record<string, unknown>, keyword));
  const contributionDepartments = departments
  ...
  const contributionProjects = projects
  ...
  const reportCollection = await loadHrPerformanceReportCollection({
    audienceType: dashboardProjection.audienceType ?? "personal",
    cycle: activeCycle,
    employees: scopedEmployees,
    departments: contributionDepartments,
    projects: contributionProjects,
  });
  const reportingAudience = dashboardProjection.audienceType ?? "personal";
  const reportingFor = (audience: PerformanceAudience, id: number) => (
    reportingAudience === audience ? reportCollection.entries.get(id) ?? null : null
  );
  const personalContributionRows = employeeRows.map((employee) => ({
    ...employee,
    reporting: reportingFor("personal", employee.id),
  }));
  const departmentContributionRows = contributionDepartments
    .map((department) => ({
      id: department.id,
      ...
      status: "现用",
      reporting: reportingFor("department", department.id),
    }));
  const projectContributionRows = contributionProjects
    .map((project) => ({
      id: project.id,
      code: project.code || "",
      name: project.name,
      ...
      status: "开启",
      reporting: reportingFor("project", project.id),
    }));
  return serviceOk({
    ...
    contributionDirectories: {
      personal: personalContributionRows,
      department: departmentContributionRows,
      project: projectContributionRows,
    },
    reportingSummary: reportCollection.summary,
...
export async function validateHrPerformancePayload(input: {
  ...
  const [employee, cycle, duplicate] = await Promise.all([
    prisma.employee.findUnique({
      ...
      select: { id: true, employeeId: true, name: true, userId: true, employments: { where: currentEmploymentDateWhere(), select: { id: true }, take: 1 } },
    }),
...
function toAttendanceRow(employee: HrPerformanceAudienceEmployee) {
  ...
  return {
    ...
    company: primary?.reportingCompany?.party.name || employment?.currentCompany || "",
