...
export async function createCompany(command: CrudCreateCommand) {
  return executeCreate(command, { entityType: "Company", modelKey: "company" as const }, (body) => {
...
export async function updateCompanyField(command: CrudUpdateFieldCommand) {
  return executeUpdateField(command, COMPANY_CONFIG);
}

export async function deleteCompany(command: CrudDeleteCommand) {
  return executeDelete(command, COMPANY_CONFIG);
}
