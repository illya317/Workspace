...
async function findOperatingCommitteeId() {
  const committee = await prisma.department.findFirst({
    where: { code: "OPS", hierarchyKind: "G", isArchived: false },
    select: { id: true },
  });
...
async function resolveDepartmentParent(hierarchyKind: DepartmentHierarchyKind, level: number, code: string, parentId: number | null) {
  ...
  if (level === 1) {
    ...
    if (hierarchyKind === "M") {
      ...
      if (!operatingCommitteeId) return "缺少运营委员会，无法维护 M1 组织";
      if (parentId !== null && parentId !== operatingCommitteeId) return "M1 组织上级组织必须是运营委员会";
...
async function validateManagerEmployees(value: unknown, managerPositionId: number | null) {
  ...
  const employees = await prisma.employee.findMany({
    where: {
      ...
      positions: { some: { positionId: managerPositionId, OR: [{ endDate: null }, { endDate: "" }, { endDate: { gte: new Date().toISOString().slice(0, 10) } }] } },
    },
    select: { id: true },
  });
