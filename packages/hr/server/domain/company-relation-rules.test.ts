...
test("rejects self ownership and reversed effective periods", () => {
  assert.equal(
    validateCompanyRelationBusinessRules(relation({ parentId: 4, childId: 4 }), []),
    "持股方和被持股方不能相同",
  );
  assert.equal(
    validateCompanyRelationBusinessRules(relation({
      parentId: 4,
      childId: 5,
...
test("blocks overlapping controlling parents but allows consecutive periods", () => {
  const existing = relation({
    ...
    effectiveTo: date("2026-06-30"),
  });
  const overlapping = relation({
    parentId: 20,
    childId: 30,
    effectiveFrom: date("2026-06-30"),
    effectiveTo: date("2026-12-31"),
  });
  const consecutive = relation({
    parentId: 20,
    childId: 30,
  ...
  assert.equal(
    validateCompanyRelationBusinessRules(overlapping, [existing]),
    "同一有效期间内，被持股公司只能有一个并表控制方",
  );
  assert.equal(validateCompanyRelationBusinessRules(consecutive, [existing]), null);
});

test("same parent-child relation uses non-overlapping period facts instead of overwriting history", () => {
  const existing = relation({
    id: 1,
    parentId: 10,
    childId: 30,
...
test("blocks only control cycles that exist in the same effective period", () => {
  ...
  const overlappingReturnEdge = relation({
    id: 3,
    parentId: 3,
    childId: 1,
  ...
  const laterReturnEdge = relation({
    ...overlappingReturnEdge,
    effectiveFrom: date("2026-07-01"),
  });

  assert.equal(
    validateCompanyRelationBusinessRules(candidate, [firstEdge, overlappingReturnEdge]),
    "并表控制关系不能形成循环",
  );
