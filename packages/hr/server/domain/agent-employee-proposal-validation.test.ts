...
test("HR Agent proposal commands validate identity, field, value, and batch operator", () => {
  assert.deepEqual(buildAgentEmployeeDraftCommand({
    ...
    ok: true,
    data: { employeeId: "00001", keyword: "", field: "school", value: "北京大学" },
  });
...
test("HR Agent proposal snapshots require unique employee ids, versions, and scalar old values", () => {
  assert.deepEqual(parseExpectedEmployeeFieldSnapshots([
    { employeeId: "00001", version: 4, oldValue: "本科" },
  ...
  assert.throws(() => parseExpectedEmployeeFieldSnapshots([
    { employeeId: "00001", version: 1, oldValue: null },
    { employeeId: "00001", version: 2, oldValue: null },
  ]), /重复工号/);
  assert.throws(() => parseExpectedEmployeeFieldSnapshots([
    { employeeId: "00001", version: 1, oldValue: { unsafe: true } },
  ]), /版本快照无效/);
});

test("HR Agent proposal snapshot rejects a changed version or old field value", () => {
  const expected = { employeeId: "00001", version: 4, oldValue: "本科" };
  assert.equal(employeeFieldSnapshotMatches({ employeeId: "00001", version: 4, education: "本科" }, "education", expected), true);
  assert.equal(employeeFieldSnapshotMatches({ employeeId: "00001", version: 5, education: "本科" }, "education", expected), false);
  assert.equal(employeeFieldSnapshotMatches({ employeeId: "00001", version: 4, education: "硕士" }, "education", expected), false);
});
