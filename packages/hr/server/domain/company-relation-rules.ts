...
export function validateCompanyRelationDateRange(
  effectiveFrom: Date | null,
  effectiveTo: Date | null,
): string | null {
...
export function validateCompanyRelationBusinessRules(
  ...
  const overlappingSameRelation = others.find((relation) => (
  ...
  if (overlappingSameRelation) return "同一持股关系的有效期间不能重叠";
  if (!candidate.isConsolidated) return null;

  const otherConsolidatedRelations = others.filter((relation) => relation.isConsolidated);
  const overlappingParent = otherConsolidatedRelations.find((relation) => (
    relation.childId === candidate.childId
    && relation.parentId !== candidate.parentId
    && rangesOverlap(toEffectiveRange(candidate), toEffectiveRange(relation))
  ));
  if (overlappingParent) return "同一有效期间内，被持股公司只能有一个并表控制方";

  if (hasTemporalPath(candidate.childId, candidate.parentId, toEffectiveRange(candidate), otherConsolidatedRelations)) {
    return "并表控制关系不能形成循环";
  }
  return null;
}
...
function hasTemporalPath(
  ...
  const queue: Array<{ companyId: number; range: EffectiveRange }> = [
    { companyId: fromCompanyId, range: candidateRange },
  ];
  ...
  while (queue.length > 0) {
    ...
    visitedRanges.set(
      current.companyId,
      [...knownRanges.filter((known) => !rangeContains(current.range, known)), current.range],
    );
