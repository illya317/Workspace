...
const ATTACHMENT_ROOT = "hr/employment-agreements/attachments";
...
function attachmentDirectory(attachmentUid: string) {
  return path.posix.join(ATTACHMENT_ROOT, attachmentUid);
}
...
export async function storeEmploymentAgreementAttachment(input: {
...
export async function removeUncommittedEmploymentAgreementAttachment(attachmentUid: string) {
  await rm(absolutePath(attachmentDirectory(attachmentUid)), { recursive: true, force: true }).catch(() => undefined);
}

export async function readEmploymentAgreementAttachmentFile(input: {
  storagePath: string;
  ...
  if (!fileStat.isFile() || fileStat.size !== input.expectedSizeBytes) throw new Error("协议附件文件校验失败");
  const buffer = await readFile(filePath);
  if (checksum(buffer) !== input.expectedChecksumSha256) throw new Error("协议附件校验和不匹配");
  return buffer;
}
