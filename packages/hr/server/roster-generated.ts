...
function rosterEmployeeWhere(): Prisma.EmployeeWhereInput {
  const hrConfig = getTenantProfile().hr;
  const virtualPersonnelType = hrConfig.options.virtualEmployeePersonnelType;
  return {
    employments: {
      some: currentEmploymentDateWhere(),
      none: currentEmploymentDateWhere({
        OR: [
          ...hrConfig.roster.excludedEmploymentTitles.map((title) => ({ title })),
          { personnelType: virtualPersonnelType },
        ],
      }),
    },
  };
...
function isEmployeeActive(employee: RosterEmployeeRecord) {
  const today = workspaceBusinessDate(new Date());
  return employee.employments.some((employment) => employmentIsActiveOnDate(employment, today));
}

function departmentNameFor(position: RosterEmployeeRecord["positions"][number] | null) {
  if (!position) return "";
  return position.position?.department?.name || position.department?.name || "";
}
