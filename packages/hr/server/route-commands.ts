...
import {
  defineBusinessActionProducerAdapter,
  executeBusinessActionProducer,
...
import { HR_AUDIT_ENTITY_TYPES } from "./audit-entities";
import { buildHrAuditRestoreCommand } from "./domain/audit-restore-validation";

export function buildHrRouteCommand<T>(command: T) {
  return okCommand(command);
}

export async function executeHrAuditRestoreCommand(input: { historyId: number; userId: number }) {
  const command = buildHrAuditRestoreCommand(input);
  if (!command.ok) return { success: false as const, error: command.issue.message, status: command.issue.status || 400 };
  return restoreAuditLogSnapshot(command.data.historyId, command.data.userId, {
...
export async function executeRosterCommand(command: {
  ...
  return { employees: rows, fields: rosterFields(), visibleFields, allCompanies, allDepts };
}

export async function produceRosterGeneratedCsv(command: Parameters<typeof renderRosterGeneratedCsv>[0]) {
  ...
  return serviceOk(new Response(`\uFEFF${csv}`, {
    headers: {
...
const rosterGeneratedCsvProducer = defineBusinessActionProducerAdapter({
  businessActionKey: "hr.roster.generated.export",
  ...
  validate: (input: Parameters<typeof buildRosterGeneratedCsvCommand>[0]) => (
    mapValidationToServiceResult(buildRosterGeneratedCsvCommand(input))
  ),
  execute: produceRosterGeneratedCsv,
});

export function executeRosterGeneratedCsvCommand(
  input: Parameters<typeof buildRosterGeneratedCsvCommand>[0],
) {
  return executeBusinessActionProducer({
