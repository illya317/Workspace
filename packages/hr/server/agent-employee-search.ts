...
import { currentOpenEndedDateWhere, employmentIsActiveOnDate } from "@workspace/platform/server/relation-registry";
...
export type AgentEmployeeCandidate = {
  ...
  alias: string | null;
  title: string | null;
  employments: Array<{ isActive: boolean; joinDate?: string | null; leaveDate?: string | null }>;
};
...
export async function searchAgentEmployeeDirectory(keyword: string, limit = DEFAULT_AGENT_EMPLOYEE_LIMIT) {
  ...
  const positions = selected.length > 0
    ? await prisma.eDP.findMany({
        where: currentOpenEndedDateWhere({ employeeId: { in: selected.map((item) => item.id) } }),
        select: {
          employeeId: true,
          isPrimary: true,
  ...
  return {
    ...
    items: selected.map((candidate) => {
      ...
      return {
        ...
        employmentStatus: candidate.employments.some((employment) => employmentIsActiveOnDate(employment, workspaceBusinessDate(new Date())))
          ? "在职" as const
          : "非在职" as const,
      };
    }),
