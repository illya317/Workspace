...
function employmentPositionNames(positions: Array<{
  isPrimary?: boolean;
...
export function listEmployments(input: ListEmploymentsInput) {
  return listEmploymentsWithDepartmentScope(input, false);
}

export function listCurrentDepartmentEmployments(
  input: ListEmploymentsInput & { readonly departmentId: number },
) {
  return listEmploymentsWithDepartmentScope(input, true);
}

async function listEmploymentsWithDepartmentScope(
  input: ListEmploymentsInput,
  currentDepartmentOnly: boolean,
) {
  const where: Prisma.EmploymentWhereInput = {};
  const today = workspaceBusinessDate(new Date());
  if (input.departmentId) {
    Object.assign(where, currentDepartmentOnly
      ? buildCurrentEmploymentDepartmentScopeWhere(input.departmentId)
      : buildEmploymentDepartmentScopeWhere(input.departmentId));
  }
  if (input.isActive !== null && input.isActive !== "") {
    if (input.isActive === "true") Object.assign(where, currentEmploymentDateWhere());
    if (input.isActive === "false") where.NOT = currentEmploymentDateWhere();
  }
    ...
    include: {
      employee: {
        select: {
          ...
          employeeId: true,
          name: true,
          positions: { where: currentOpenEndedDateWhere(), include: employeePositionFilterInclude },
        },
      },
    },
    orderBy: { id: "asc" },
