...
import { createMessageSection, createPageBody, createPageTabBar, PageSurface, useFeedback } from "@workspace/core/ui";
...
import { createPeopleTabConfigs } from "@workspace/hr/constants/tab-configs";
...
const OrganizationChartTab = dynamic(() => import("./organization/OrganizationChartTab"));
...
export default function HRClient({
  ...
  const { contractConfig, edpConfig, employeeConfig, employmentConfig } = useMemo(
    () => createPeopleTabConfigs(tenantConfig),
    [tenantConfig],
  );
  ...
  const canExportGenerated = hrUser.isAdmin || canExportGeneratedRoster;
  ...
  if (renderedView === "organization") {
    if (renderedChild === "chart") return <OrganizationChartTab surface={surface} />;
    return (
  ...
  if (renderedView === "generated") {
    return <RosterGeneratedTab variant={activeGeneratedVariant} canExport={canExportGenerated} surface={surface} />;
  }
