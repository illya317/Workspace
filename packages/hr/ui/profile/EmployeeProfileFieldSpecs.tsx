...
import { normalizeHrMajorItems } from "@workspace/hr/constants/field-options";
...
export function profileFieldSpec(
  ...
  if (field.type === "school") {
    return {
      ...
      spec: {
        valueType: "string",
        control: "choice",
        options: { source: "static", items: (field.options ?? []).map((option) => ({ value: option, label: option })), visibleCount: 5 },
        state: fieldDisabled ? "disabled" : "normal",
      },
  ...
  if (field.type === "professionalTitle") {
    return {
      ...
      value: field.options?.includes(String(record[field.key] ?? "")) ? String(record[field.key]) : null,
      placeholder: "未设置",
      onChange: (next) => onChange(field.key, next === null || next === undefined || next === "" ? null : String(next)),
    };
  }
  ...
  if (field.type === "fk" && field.entity) {
    ...
    const isEdpReportToPosition = field.fkKey === "hr.edp.reportToPosition";
    const rawDepartmentId = isEdpPosition || isEdpReportToPosition ? record.departmentId : null;
    const departmentId =
      typeof rawDepartmentId === "number" || typeof rawDepartmentId === "string"
        ? rawDepartmentId
        : null;
    const rawReportingCompanyId = isEdpPosition ? record.reportingCompanyId : null;
    const reportingCompanyId =
      typeof rawReportingCompanyId === "number" || typeof rawReportingCompanyId === "string"
        ? rawReportingCompanyId
        : null;
    const rawSourcePositionId = isEdpReportToPosition ? record.positionId : null;
    const sourcePositionId =
      typeof rawSourcePositionId === "number" || typeof rawSourcePositionId === "string"
        ? rawSourcePositionId
        : null;
    const positionDisabled = isEdpPosition && (!reportingCompanyId || !departmentId);
    const reportToDisabled = isEdpReportToPosition && (!sourcePositionId || !departmentId);
    if (fieldDisabled) {
    ...
    return {
      ...
      spec: {
        valueType: "reference",
        control: "reference",
        state: positionDisabled || reportToDisabled ? "disabled" : "normal",
        options: {
          source: "remote",
          ...
          queryParams: isEdpPosition
            ...
            : isEdpReportToPosition
              ? { positionId: sourcePositionId, departmentId }
              : undefined,
        },
      },
    ...
    if (field.key === "rank") {
      return {
        ...
        spec: {
          valueType: "string",
          control: "choice",
          options: { source: "grouped", groups: rankGroups(field.options || []), groupLabel: "职级序列", optionLabel: "等级" },
          state: fieldDisabled ? "disabled" : "normal",
        },
    ...
    return {
      ...
      spec: {
        ...
        options: {
          ...
          commonValues: field.key === "ethnicity" ? field.commonOptions : undefined,
