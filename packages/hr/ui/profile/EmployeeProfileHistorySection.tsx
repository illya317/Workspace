...
export function HistorySection({
  ...
  return <SectionShell title="历史记录" subtitle="记录谁在什么时候修改了哪些字段。" actions={<CommandButton variant="secondary" onClick={onRefresh}>刷新</CommandButton>} className={className}>
      <div className="space-y-3">
        ...
        return <DisclosureRecordCard key={entry.id} expanded={expanded} onToggle={() => onToggle(entry.id)} header={<div><div className="text-sm font-semibold text-slate-900">{headerTitle}</div><div className="mt-1 text-xs text-slate-500">{new Date(entry.createdAt).toLocaleString("zh-CN", {
              hour12: false
            })} · {entry.entityType} #{entry.entityId} · v{entry.version}</div></div>} summary={<span className="text-xs text-slate-400">{expanded ? "收起" : "展开"}</span>}>
                {entry.action === "create" ? <EmptyStateCard compact>创建时的初始快照。</EmptyStateCard> : <DataTable rows={entry.changes} columns={changeColumns} visibleColumns={["field", "from", "to"]} density="compact" rowKey={change => `${entry.id}-${change.field}`} />}
