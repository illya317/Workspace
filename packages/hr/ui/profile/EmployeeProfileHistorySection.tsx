...
function historyDateFormatter(timeZone: string) {
  let formatter = historyDateFormatters.get(timeZone);
  if (!formatter) {
    ...
    historyDateFormatters.set(timeZone, formatter);
  }
  return formatter;
}
      ...
      return {
        ...
        header: {
          ...
          items: [
            ...
            {
              kind: "text" as const,
              value: `${historyDateFormatter(businessTimeZone).format(new Date(entry.createdAt))} · ${entry.entityType} #${entry.entityId} · v${entry.version}`,
              tone: "muted",
            },
          ],
        },
...
export function createHistorySection(props: HistorySectionProps, businessTimeZone: string): BodySurfaceSectionSpec {
  return createRecordSection("history", historySectionSurface(props, businessTimeZone));
}
...
export function HistorySection(props: HistorySectionProps) {
  ...
  return <BodySurface {...createPageBody([createHistorySection(props, businessTimeZone)])} />;
}
