...
import {
  ...
  agreementMissingTermFields,
  agreementSupplementPatch,
...
export function useContractSections({
  ...
  function setField(key: string, value: unknown) {
    setDraft((current) => {
      if (key.startsWith("termSupplement:")) {
        const [, termUid, field] = key.split(":") as [string, string, "effectiveFrom" | "effectiveThrough"];
        if (!termUid || (field !== "effectiveFrom" && field !== "effectiveThrough")) return current;
        return {
          ...current,
          termSupplements: {
            ...current.termSupplements,
            [termUid]: {
              effectiveFrom: current.termSupplements[termUid]?.effectiveFrom ?? null,
              effectiveThrough: current.termSupplements[termUid]?.effectiveThrough ?? null,
  ...
  function startTermCorrection(termUid: string) {
    const term = selected?.terms.find((item) => item.termUid === termUid) ?? null;
    if (!term) return;
    setDraft((current) => ({
      ...current,
  ...
  async function submit(kind: EmploymentAgreementCommandKind) {
    ...
    if (kind === "create") {
      ...
      const patch = kind === "supplement-missing"
        ? agreementSupplementPatch(selected, draft)
        : agreementContentPatch(selected, draft, kind);
  ...
  const missingTermFields = agreementMissingTermFields(selected);
  ...
  const inlineSupplement = Boolean(
    ...
    && (missingContentFields.length > 0 || missingTermFields.length > 0),
  );
  ...
  const agreementActions = selected && canEdit
    ? editMode === "correct" ? [
        ...
        ...(inlineSupplement ? [{
          ...
          disabled: saving || Object.keys(supplementPatch).length === 0,
  ...
  const termActions = selected && canEdit ? [{
  ...
  const detail = selected ? {
    ...
    ...(detailMode === "supplement-missing" ? {
      mutation: {
        ...
        targetFields: [
          ...missingContentFields.map((field) => `content.${field}`),
          ...missingTermFields.map((field) => field.path),
        ],
    ...
    supplemental: agreementHistorySupplemental(selected, canEdit ? {
      onEditTerm: startTermCorrection,
      onCorrectContent: () => startEdit("correct"),
    } : undefined),
  } : undefined;
