...
function yearDays(year: number) {
  ...
  for (let mask = 0x8000; mask > 0x8; mask >>= 1) {
    if (LUNAR_INFO[year - 1900] & mask) sum += 1;
  }
...
function lunarDayName(day: number) {
  ...
  if (day === 20) return "二十";
  if (day === 30) return "三十";
  return `${DAY_PREFIX[Math.floor((day - 1) / 10)]}${["一", "二", "三", "四", "五", "六", "七", "八", "九"][(day - 1) % 10]}`;
}

export function solarToLunarBirthday(value: unknown) {
  if (!value) return "";
  const match = String(value).match(/^(\d{4})-(\d{2})-(\d{2})$/);
  if (!match) return "";
  const [, y, m, d] = match;
  const date = new Date(Date.UTC(Number(y), Number(m) - 1, Number(d)));
  if (Number.isNaN(date.getTime()) || Number(y) < 1900 || Number(y) > 2100) return "";
  ...
  return `${isLeap ? "闰" : ""}${MONTH_NAMES[lunarMonth - 1]}月${lunarDayName(offset + 1)}`;
}
