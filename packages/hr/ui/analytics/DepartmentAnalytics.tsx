...
export function useDepartmentAnalyticsSections({ departments, edps }: { departments: Department[]; edps: EDP[] }): BodySurfaceSectionSpec[] {
  ...
  const columns = useMemo<DataSurfaceColumnSpec<(typeof stats.deptWithHeadcount)[number]>[]>(() => [
    { key: "name", label: "组织", required: true, emphasis: "medium", cell: (department) => department.name },
    {
      key: "level",
      label: "层级",
      required: true,
    ...
    { key: "headcount", label: "编制", required: true, align: "right",  tone: "muted", cell: (department) => department.headcount || "—" },
    {
      ...
      cell: (department) => {
        ...
        if (diff > 0) return { kind: "text", value: `+${diff} 超编`, tone: "danger" };
        if (diff < 0) return { kind: "text", value: `${diff} 缺编`, tone: "warning" };
        return { kind: "text", value: "正常", tone: "success" };
      },
    },
  ], []);
  ...
  return [
        createMetricsSection("stats", {
            metrics: [
              { key: "departments", label: "组织总数", value: departments.length },
              { key: "l1", label: "管理一级(M1)", value: stats.l1 },
              { key: "l2", label: "部门(M2)", value: stats.l2 },
              { key: "l3", label: "子部门(M3)", value: stats.l3 },
        ...
        createAnalysisSection("department-tree", {
          ...
          sections: [
            createInlineFieldsSection("department-tree-filters", [
              { key: "search", label: "搜索", spec: { valueType: "string", control: "text" }, value: search, onChange: (value) => setSearch(String(value ?? "")), placeholder: "搜索组织名称、编码、别名..." },
            ]),
            {
            key: "tree",
