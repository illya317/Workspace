...
export function useContractAnalyticsSections({
  ...
  const columns = useMemo<DataSurfaceColumnSpec<EnrichedContract>[]>(() => [{
    ...
    required: true,
    cell: contract => {
      if (contract.daysLeft === null || Number.isNaN(contract.daysLeft)) return { kind: "empty" };
      if (contract.daysLeft < 0) return { kind: "text", value: `超${Math.abs(contract.daysLeft)}天`, tone: "danger", emphasis: "medium" };
      return { kind: "text", value: `${contract.daysLeft}天`, tone: contract.daysLeft <= 30 ? "danger" : contract.daysLeft <= 90 ? "warning" : "muted", emphasis: "medium" };
    }
  }, {
    key: "status",
    label: "状态",
