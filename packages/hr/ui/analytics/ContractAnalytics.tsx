...
export default function ContractAnalytics({ contracts }: { contracts: Contract[] }) {
  ...
  const columns = useMemo<DataTableColumn<EnrichedContract>[]>(() => [
    { key: "employeeId", label: "工号", required: true, cellClassName: "font-mono text-slate-500", render: (contract) => contract.employeeId },
    { key: "employeeName", label: "姓名", required: true, cellClassName: "font-medium", render: (contract) => contract.employeeName },
    { key: "company", label: "公司", required: true, cellClassName: "text-slate-500", render: (contract) => contract.company || "—" },
    { key: "contractType", label: "合同类型", required: true, cellClassName: "text-slate-500", render: (contract) => contract.contractType || "—" },
    { key: "nearestEnd", label: "最近到期日", required: true, cellClassName: "text-slate-700", render: (contract) => contract.nearestEnd || "—" },
    {
      key: "daysLeft",
      label: "剩余天数",
      required: true,
      render: (contract) => {
        ...
        return (
          <span className={`font-medium ${contract.daysLeft <= 30 ? "text-rose-600" : contract.daysLeft <= 90 ? "text-amber-600" : "text-gray-600"}`}>
            {contract.daysLeft}天
          </span>
        );
      },
    },
    {
      ...
      required: true,
      render: (contract) => (
        <span className={`text-xs px-1.5 py-0.5 rounded ${statusBadge(contract.status)}`}>
  ...
  return (
    <div className="space-y-6">
        ...
        toolbar={
          ...
          <div className="flex gap-1">
            {(["all", "expiring30", "expiring90", "expired"] as const).map((f) => (
              <ActionButton key={f} onClick={() => setFilter(f)} variant={filter === f ? "primary" : "secondary"}>
      ...
      >
        ...
        />
        {filtered.length > 100 && (
          <p className="py-2 text-center text-xs text-gray-400">还有 {filtered.length - 100} 条，请使用搜索或筛选</p>
        )}
      </AnalysisBlock>
    </div>
  );
