...
function auditDateFormatter(timeZone: string) {
  let formatter = auditDateFormatters.get(timeZone);
  if (!formatter) {
    ...
    auditDateFormatters.set(timeZone, formatter);
  }
  return formatter;
}
...
export function createAuditLogRecord({
  ...
  onToggle,
  onRestore,
}: AuditLogEntryProps, businessTimeZone: string): DataSurfaceRecordSpec {
  ...
  const header: DataSurfaceRecordSpec["header"] = {
    kind: "text",
    value: `${entry.tag ? entry.tag.replace("V0:", "基线 ") : `V${entry.version}`} · ${auditDateFormatter(businessTimeZone).format(new Date(entry.createdAt))} · ${entry.editorName} · ${entry.entityName}${entry.action === "create" ? " · 创建记录" : ""}`,
    emphasis: "medium",
    wrap: "truncate",
  };
  ...
  return (
    <BodySurface {...createPageBody([
        createRecordSection(`audit-entry-${props.entry.id}`, {
          records: [createAuditLogRecord(props, businessTimeZone)],
        }),
      ])} />
  );
}
