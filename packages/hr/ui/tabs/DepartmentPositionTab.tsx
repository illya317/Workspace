...
import { departmentPath, parseAlias } from "./department-position/utils";
import type { RosterSurfaceTabBarProps } from "../roster-surface";

type DepartmentPositionSearchOption = SurfaceAutocompleteOptionSpec;

const DEPARTMENT_SEARCH_SUBTITLE = "部门";
const POSITION_SEARCH_SUBTITLE = "岗位";
const POSITION_SEARCH_SUBTITLE_SEPARATOR = " · ";
...
export default function DepartmentPositionTab({
  ...
  const departmentCreateRuntime = departmentActionRuntimes?.create ?? null;
  const departmentUpdateRuntime = departmentActionRuntimes?.update ?? null;
  useEffect(() => {
    ...
    onFocusDepartmentConsumed?.();
  }, [departments, focusDepartmentId, mode, onFocusDepartmentConsumed, setSelection, setShowArchived]);
  useEffect(() => {
    ...
    onFocusPositionConsumed?.();
  }, [focusPositionId, mode, onFocusPositionConsumed, positions, setSelection, setShowArchived]);
  const {
    archivedDepartments,
    archivedPositions,
    createPositionCode,
  ...
  const organizationSelectedDepartment = isOrganizationMode && selectedPosition?.departmentId
    ...
    : selectedDepartment;
  const departmentPositionSearchOptions = useMemo(
    () => buildDepartmentPositionSearchOptions(departments, positions, departmentById),
    [departmentById, departments, positions],
  );
  const handleDepartmentPositionSearchSelect = useCallback((nextSelection: Selection) => {
    if (!nextSelection) return;
    setCreatePanel(null);
    selectItem(nextSelection);
  }, [selectItem, setCreatePanel]);
  ...
  useEffect(() => {
    ...
    setCollapsedDepartments((prev) => setSelectorTreeNodeExpanded(prev, activeOrganizationRootId, true));
  ...
  return (
    <DepartmentPositionMainContent
      ...
      showArchived={showArchived}
	      search={search}
      searchOptions={departmentPositionSearchOptions}
	      onSearchChange={setSearch}
      onSearchSelect={handleDepartmentPositionSearchSelect}
	      onCreatePanelChange={setCreatePanel}
      onLoadData={loadData}
...
function buildDepartmentPositionSearchOptions(
  departments: Department[],
  positions: Position[],
  departmentById: Map<number, Department>,
): DepartmentPositionSearchOption[] {
  const departmentOptions = departments.map((department) => ({
    value: `department:${department.id}`,
    name: department.name,
    details: DEPARTMENT_SEARCH_SUBTITLE,
    searchText: [
      department.code,
      department.name,
      department.alias,
      parseAlias(department.alias),
      departmentPath(department, departmentById),
    ].filter(Boolean).join(" "),
  }));
  const positionOptions = positions.map((position) => {
    ...
    return {
      ...
      name: position.name,
      details: position.departmentName
        ? `${POSITION_SEARCH_SUBTITLE}${POSITION_SEARCH_SUBTITLE_SEPARATOR}${position.departmentName}`
        : POSITION_SEARCH_SUBTITLE,
      searchText: [
        position.code,
        position.codeRaw,
        position.name,
        position.alias,
        parseAlias(position.alias),
