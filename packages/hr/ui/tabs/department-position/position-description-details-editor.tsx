...
import { createFormSection, createPageBody, BodySurface, type FormSurfaceProps, useFeedback } from "@workspace/core/ui";
import {
  DETAIL_FIELD_ORDER,
...
export function usePositionDescriptionDetailsSurface({
  ...
  const fields = buildPositionDescriptionDetailFields({
    ...
    positionNames,
    departmentNames,
    hrRanks: tenantConfig.hr.options.ranks,
    hrOfficeLocations: tenantConfig.hr.options.officeLocations,
    positionDescriptionOptions: tenantConfig.hr.positionDescriptionOptions,
    feedback,
    updateDetailValue,
    updateDetailField,
  });
...
export function PositionDescriptionDetailsEditor(props: PositionDescriptionDetailsSurfaceProps) {
  ...
  return (
    <BodySurface {...createPageBody([createFormSection("position-description-details", surface)])} />
  );
}
