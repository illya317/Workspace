...
import { createResponsibilityField } from "@workspace/platform/ui";
...
/** @ui-structural-declaration Complete responsibility repeatable group. */
export function buildDutyField(
  ...
  return createResponsibilityField({
    key: detailKey,
    title: label,
    records,
    disabled,
    confirmDelete: feedback.confirmDelete,
    onChange: (nextRecords) => updateDetailValue(detailKey, nextRecords),
  });
}

/** @ui-structural-declaration Complete change-history repeatable group. */
...
export function buildWorkEnvironmentFields(
  ...
  label: string,
  fieldValue: unknown,
  options: { workAreas: string[]; environmentFactors: string[] },
): FormSurfaceItemSpec<FormSurfaceLooseItem>[] {
  const items = normalizeWorkEnvironments(fieldValue);
  const usedAreas = new Set(items.map(item => item.area));
  const availableAreas = options.workAreas.filter(area => !usedAreas.has(area));
  ...
  return [
    {
      ...
      items: items.map((item, index) => {
        ...
        const availableFactors = options.environmentFactors.filter((factor) => !item.factors.includes(factor));
        return {
          key: `${item.area}-${index}`,
          actions: disabled ? undefined : [{
            key: "delete-area",
          ...
          items: [
            {
              key: "area",
              label: "工作区域",
              spec: { valueType: "string", control: "choice", state: disabled ? "disabled" as const : "normal" as const, options: { source: "static", items: optionSpecs(areaOptions), searchPlaceholder: "搜索工作区域" } },
              value: item.area,
              placeholder: "选择工作区域",
            ...
            },
            {
              ...
              append: disabled ? undefined : {
                field: {
                  key: "appendFactor",
                  label: "",
                  spec: { valueType: "string", control: "choice", state: availableFactors.length === 0 ? "disabled" as const : "normal" as const, options: { source: "static", items: optionSpecs(availableFactors), visibleCount: 6, searchPlaceholder: "搜索环境因素" } },
                  value: "",
    ...
    ...(!disabled ? [{
      key: `${key}-addArea`,
      label: "新增工作区域",
      spec: { valueType: "string", control: "choice", state: availableAreas.length === 0 ? "disabled" as const : "normal" as const, options: { source: "static", items: optionSpecs(availableAreas), searchPlaceholder: "搜索工作区域" } },
      value: "",
      placeholder: "新增工作区域",
...
/** @ui-structural-declaration Complete experience-requirement repeatable group. */
