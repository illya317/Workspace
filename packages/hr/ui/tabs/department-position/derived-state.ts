...
export function useDepartmentPositionDerivedState({
  ...
  const departmentStats = useMemo(() => {
    ...
    for (const department of departments) {
      const parentId = displayParentId(department, departmentById);
    ...
    return statsById;
  }, [departmentById, departments, positionsByDepartment]);
  ...
  const selectedDepartmentStats = selectedDepartment
    ...
    : null;
  const rootDepartments = useMemo(() => departments
    .filter((department) => displayParentId(department, departmentById) == null)
    .sort((a, b) => a.hierarchyKind.localeCompare(b.hierarchyKind) || a.level - b.level || a.id - b.id), [departmentById, departments]);
