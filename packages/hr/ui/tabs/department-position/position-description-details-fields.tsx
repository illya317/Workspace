...
function rankGroups(rankOptions: string[]) {
...
export function buildPositionDescriptionDetailFields({
  ...
  function tagListField({
    ...
    return {
      ...
      onUpdateLabel: (_item: string, index: number, next: string) => onItemsChange(uniqueStrings(items.map((item, itemIndex) => itemIndex === index ? next : item))),
      disabled: fieldDisabled,
      removeConfirmMessage: (item: string) => `确定删除「${item || label}」吗？删除后需要保存才会生效。`,
  ...
  function entityTagListField(key: string, label: string, value: unknown, entity: string, validNames?: Set<string>): FormSurfaceItemSpec<FormSurfaceLooseItem> {
    ...
    return {
      ...
      onUpdateLabel: (_item: string, index: number, next: string) => updateDetailValue(key, uniqueStrings(items.map((item, itemIndex) => itemIndex === index ? next : item))),
      disabled,
      removeConfirmMessage: (item: string) => `确定删除「${item || label}」吗？删除后需要保存才会生效。`,
      itemTitle: (item: string) => (validNames && !validNames.has(item) ? "当前主数据中未找到对应记录" : undefined),
      itemClassName: (item: string) =>
  ...
  function renderPrimitiveFields(key: string): FormSurfaceItemSpec<FormSurfaceLooseItem>[] {
    ...
    if (key === "rank") {
      return [{
        ...
        spec: {
          ...
          control: "choice",
          state: disabled ? "disabled" : "normal",
          options: { source: "grouped", groups: rankGroups(hrRanks), groupLabel: "职级序列", optionLabel: "等级" },
        },
    ...
    if (key === "education") {
      return [{
          ...
          spec: {
            ...
            control: "choice",
            state: disabled ? "disabled" : "normal",
            options: { source: "static", items: optionSpecs(positionDescriptionOptions.educationRequirements) },
          },
          value: String(fieldValue || positionDescriptionOptions.defaultEducationRequirement),
          placeholder: positionDescriptionOptions.defaultEducationRequirement,
          onChange: (next) => updateDetailValue(key, next || positionDescriptionOptions.defaultEducationRequirement),
        }];
    }
    if (key === "major") {
      ...
      return [{
        ...
        disabled,
        removeConfirmMessage: (item: HRMajorItem) => `确定删除专业要求「${item.category || ""}${item.specialty ? ` / ${item.specialty}` : ""}」吗？删除后需要保存才会生效。`,
    ...
    if (key === "workEnvironments") {
      return buildWorkEnvironmentFields(fieldContext, key, DETAIL_FIELD_LABELS[key] || key, fieldValue, positionDescriptionOptions);
    }
    ...
    if (key === "salaryType" || key === "officeLocation" || key === "workSchedule") {
      const options = key === "salaryType"
        ? positionDescriptionOptions.salaryTypes
        : key === "officeLocation"
          ? hrOfficeLocations
          : positionDescriptionOptions.workSchedules;
      return [{
          key,
          label: DETAIL_FIELD_LABELS[key] || key,
          spec: {
            ...
            control: "choice",
            state: disabled ? "disabled" : "normal",
            options: { source: "static", items: optionSpecs(options) },
          },
          value: fieldValue,
          placeholder: "未设置",
          onChange: (next) => updateDetailValue(key, next || null),
