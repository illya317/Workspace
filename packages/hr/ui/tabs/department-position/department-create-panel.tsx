...
import { createPageBody, BodySurface, type CreateSurfaceProps, type FormSurfaceItemSpec } from "@workspace/core/ui";
...
export function useDepartmentCreateSurface({
  ...
  open,
  onOpenChange,
}: DepartmentCreatePanelProps & { open: boolean; onOpenChange: (open: boolean) => void }): CreateSurfaceProps {
  const defaultManagementRootParentId = useMemo(
    () => Array.from(departmentById.values()).find(isOperatingCommittee)?.id ?? null,
    [departmentById],
  );
  ...
  const parentOptions = useMemo(() => {
    return [
      { value: "", label: "无" },
      ...departments
        .filter((d) => canUseDepartmentAsParentForHierarchy({ candidate: d, hierarchyKind, level }))
        .map((d) => ({ value: String(d.id), label: `${d.name}（${d.code}）` })),
    ];
  }, [departments, hierarchyKind, level]);
...
export function DepartmentCreatePanel(props: DepartmentCreatePanelProps & { open: boolean; onOpenChange: (open: boolean) => void }) {
  ...
  return (
    <BodySurface {...createPageBody([{ key: "department-create", chrome: "plain", body: { kind: "create", create } }])} />
  );
}
