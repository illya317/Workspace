...
export function isEditableHrTableCell(
  ...
  field: FieldConfig,
  config: TabConfig,
  virtualPersonnelType: string,
) {
  return !(
    config.entityType === "Employment"
    && field.key === "personnelType"
    && item.personnelType === virtualPersonnelType
  );
}
...
export default function EditableTable({
  ...
  loading,
  emptyText,
  virtualPersonnelType: configuredVirtualPersonnelType,
}: EditableTableProps) {
  const tenantConfig = useTenantConfig();
  const virtualPersonnelType = configuredVirtualPersonnelType ?? tenantConfig.hr.options.virtualEmployeePersonnelType;
  ...
  const columns = useMemo<DataSurfaceColumnSpec<Record<string, unknown>>[]>(
    () => fields.map((field) => {
      ...
      return {
        ...
        cell: (item) => {
          const rowEditable = editableCell && isEditableHrTableCell(item, field, config, virtualPersonnelType ?? "");
    ...
    [canEdit, config, editMode, editingCell, fields, onStartEdit, renderEditInput, virtualPersonnelType],
  );
