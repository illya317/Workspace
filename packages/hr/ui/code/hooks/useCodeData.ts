...
export function useCodeData({
  ...
  const regulatedManagementGroup = useTenantConfig().organization.managementGroups.regulated;
  ...
  const load = useCallback(async () => {
    ...
    const resolveCodes = (selected: string): string[] => {
      ...
      if (!c) return [];
      if (c.managementGroup === regulatedManagementGroup) {
        return companies.filter((x) => x.managementGroup === regulatedManagementGroup).map((x) => x.code);
      }
      return companies.filter((x) => x.managementGroup !== regulatedManagementGroup).map((x) => x.code);
    };
    ...
    const [codesRes, empRes] = await Promise.all([
      fetch(url),
      fetch(
        workspacePath(`/api/modules/hr/roster?company=${encodeURIComponent(selectedCompany || "")}`)
      ),
    ]);
    if (codesRes.ok) {
    ...
    setLoading(false);
  }, [resolvedApiPath, selectedCompany, departmentCode, companies, regulatedManagementGroup]);
