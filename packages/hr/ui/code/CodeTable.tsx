...
type CodeDisplayRow =
  ...
  | { kind: "code"; id: string; item: CodeItem }
  | { kind: "summary"; id: string; label: string; total: number; grand?: boolean }
  | { kind: "add"; id: string };
...
export default function CodeTable({
  ...
  const rows: CodeDisplayRow[] = [
    ...(bioCodes.length > 0
      ? [
          { kind: "group" as const, id: "group-bio", label: "常规体系" },
          ...bioCodes.map((item) => ({ kind: "code" as const, id: `code-${item.code}`, item })),
          { kind: "summary" as const, id: "summary-bio", label: "小计", total: bioTotal },
        ]
      : []),
    ...(pharmaCodes.length > 0
      ? [
          { kind: "group" as const, id: "group-pharma", label: "GMP" },
          ...pharmaCodes.map((item) => ({ kind: "code" as const, id: `code-${item.code}`, item })),
          { kind: "summary" as const, id: "summary-pharma", label: "小计", total: pharmaTotal },
        ]
      : []),
    { kind: "summary", id: "summary-grand", label: "合计", total: grandTotal, grand: true },
    ...(editMode && hrCanEdit(user) ? [{ kind: "add" as const, id: "add-row" }] : []),
  ];
  ...
  const columns: DataTableColumn<CodeDisplayRow>[] = [
    {
      ...
      render: (row) => {
        ...
        return (
          <span
            className={onSelect ? "cursor-pointer hover:text-emerald-600" : ""}
            onClick={() => onSelect?.(row.item.code)}
          >
      ...
      render: (row) => {
          ...
          return (
            <div className="flex items-center gap-1">
              <TextField
                ...
                className="h-7 px-2 py-0.5 text-xs"
              />
              <ActionButton onClick={handleAdd} variant="primary" className="px-3 py-1 text-xs">
                添加
              </ActionButton>
            </div>
          );
        ...
        return (
          <span
            ...
            onClick={() => handleNameClick(row.item)}
          >
            {row.item.name || "-"}
          </span>
        );
    ...
    },
    {
      ...
      render: (row) => {
        if (row.kind === "summary") {
          return (
            <span className={`${row.grand ? "bg-emerald-100 font-bold" : "bg-emerald-50 font-medium"} rounded-full px-2 py-0.5 text-xs text-emerald-700`}>
              {row.total}
            </span>
          );
        }
        ...
        return (
          <span
            className="cursor-pointer rounded-full bg-slate-100 px-2 py-0.5 text-xs hover:bg-slate-200"
