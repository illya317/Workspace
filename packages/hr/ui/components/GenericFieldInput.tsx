...
interface GenericInputSpecInput {
  field: FieldConfig;
  value: unknown;
...
export function createGenericEditInputSpec(input: GenericInputSpecInput): DataSurfaceCellInputSpec {
  return { kind: "input", ...createGenericInputControl(input) };
}

export function createGenericInputControl({
  field,
  value,
  onChange,
  onKeyDown,
  inputRef,
  fkConfig,
}: GenericInputSpecInput): Omit<DataSurfaceCellInputSpec, "kind"> {
  ...
  if (field.type === "chineseId") return { spec: { valueType: "string", control: "text" }, value: normalizeChineseIdNumber(value) ?? "", maxLength: 18, onChange: next => onChange(normalizeChineseIdNumber(String(next ?? ""))?.slice(0, 18) ?? null), onKeyDown, inputRef, density: "compact" };
  return { spec: { valueType: field.type === "number" ? "number" : "string", control: field.type === "number" ? "number" : "text" }, type: field.type === "number" ? "number" : "text", value: String(value ?? ""), onChange, onKeyDown, inputRef, density: "compact" };
}
