...
export default function MajorPicker({
  ...
  return <div ref={containerRef} className={`relative min-w-0 flex-1 ${className ?? ""}`}>
      <SearchInput value={display} onChange={next => {
      setSearching(true);
      setKeyword(next);
    ...
    }} disabled={disabled} placeholder={current?.specialty || "搜索专业"} />
      {searching && !disabled && <PanelCard className="absolute left-0 top-[calc(100%+0.35rem)] z-50 w-max min-w-full max-w-[min(34rem,calc(100vw-2rem))]" bodyClassName="max-h-64 overflow-auto p-2">
