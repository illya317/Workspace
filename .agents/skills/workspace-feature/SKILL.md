...
description: Implement user-visible Workspace business features across domain UI, services, page shells, and API shells. Use for product behavior, forms, lists, interactions, business bug fixes, or domain feature delivery; do not use for schema ownership, platform governance, or deployment.
---
...
- 开工前确认根 `AGENTS.md` 的 Role Gate，并确认读取 router 后的第一条角色声明更新已写明 `主角色: Feature`。
- 如果任务主体是 schema、架构契约、运维、清债或独立审查，改用对应 `workspace-*` skill；不要把普通 Feature 任务扩成平台治理。
...
- 普通短字段表格优先随页面自然展开；不要默认声明横向或纵向内滚动，也不要只为保留表头而制造固定高度视窗。只有矩阵、不可压缩列或明确需要固定视窗时才声明 `scroll`，并说明判断依据。
