...
- FK 候选项查询是通用机制，但 API 入口必须保留业务域权限边界，例如 `app/api/hr/reference-options`、`app/api/work/reference-options`；route 只调用 Platform factory，不写业务查询，Platform 只能消费 `packages/platform/module-registry.ts` 中的模块契约聚合能力，禁止手写 import 某几个业务包拼业务能力。
