...
- **当前并行线程**: Architecture 线程只改架构规则、文档、Platform/API/service 收敛和 baseline ratchet；Feature 线程正在做 `/work`/`packages/work` 体验统一，特别是 `/work/plans` 左右分栏；Production/QC 线程可改 `config/scripts/generate-product-stage-tests.mjs` 与生成的 pharma-qc JSON/cache。互相不要提交、回滚或格式化对方范围。
...
- **Level 2 工作方式**: `npm run arch:level2` 用来发现结构漂移并拆成文件级任务；真正强制的项必须进入 `arch:gate`，baseline 只能减少，不能为新违规扩写。
- **交接要可执行**: 给其他 agent 的任务必须写清楚目标文件、动作类型（move/delete/refactor/rewrite）、归属角色、验证命令和不能碰的并行范围。
