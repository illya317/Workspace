...
- Work 业务体验调整只改 `packages/work` / `app/(modules)/work` / `app/api/modules/work/*` 或必要 Core UI 基建。`/work/plans` 列表展开目标是左右分栏，不要用遮罩式整屏 overlay 灰掉详情区；若抽通用可折叠分栏，放 Core，Work 只接业务数据。
