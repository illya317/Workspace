...
- **Level 1 硬门禁**: `npm run arch:check` 会执行 `level1:check`，包括 dependency-cruiser DAG、`moduleDefinition` 注册校验、`authorize()`/API 结构校验。新增代码绕过 Core UI、跨业务包 import、缺模块注册、API 新增裸 `checkPermission` 或裸 Prisma，都会失败。
...
- 下拉、搜索、筛选、日期、确认弹窗、Toast、表格、分页、Tab、页面模板先查 `docs/reusable-components.md`；已有 Core/Platform/App 组件时禁止重复造车，字段展示和选择面板必须解耦。`npm run arch:check` 会检查疑似重复基础组件文件名，新增例外必须写入脚本 allowlist 并说明迁移计划。
- API route 只做认证、权限、参数校验、调用 service、返回 DTO；新增权限判断必须通过 `server/auth/authorize.ts` 的 `authorize()` 或委托给已经使用 `authorize()` 的平台 wrapper。
- 每个业务包必须在 `packages/<domain>/module.ts` 导出 `moduleDefinition`，并在 `packages/platform/modules.tsx` 注册；`packages/project` 禁止新增，工作计划/工作清单/工作汇报/历史记录统一归 Work。
- `dependency-cruiser.config.cjs` 和 `scripts/check/check-package-boundaries.js` 共同执行 DAG：Core 不依赖 Platform/Apps，业务包之间不直接互相 import，Platform 只允许在模块注册聚合点读取业务包注册信息。
