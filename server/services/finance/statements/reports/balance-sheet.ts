...
// ─── Response format (backward-compatible with ReportLines.tsx) ──
...
function filterBySections(
  lines: (ComputedLine & { _section: string })[],
  sections: string[],
): ReportLineItem[] {
...
export function generateBalanceSheet(
  ...
  const { lines, diagnostics } = computeBalanceSheet(
    BALANCE_SHEET_LINES, balances, reclass,
  );

  // Attach section info to each computed line for filtering
  const sectionMap = new Map(BALANCE_SHEET_LINES.map((c) => [c.lineCode, c.section]));
