...
for value in "$SERVER" "$REMOTE_DIR" "$WORKSPACE_GATEWAY_NGINX_SITE" "${DEPLOY_PROFILE_PREPARED_STATE_ROOT:-}"; do
...
ssh "${SSH_OPTIONS[@]}" "$SERVER" "
  chmod 700 '$REMOTE_TOOL_ROOT/apply-deploy-unit.sh' '$REMOTE_TOOL_ROOT/switch-deploy-gateway.sh' '$REMOTE_TOOL_ROOT/promote-deploy-profile.sh' '$REMOTE_TOOL_ROOT/rollback-deploy-profile.sh'
...
if [ "$COMMAND" = "rollback" ]; then
  ssh "${SSH_OPTIONS[@]}" "$SERVER" \
    "REMOTE_DIR='$REMOTE_DIR' WORKSPACE_GATEWAY_NGINX_SITE='$WORKSPACE_GATEWAY_NGINX_SITE' DEPLOY_PACKAGE_VERSION='$DEPLOY_PACKAGE_VERSION' DEPLOY_STARTED_EPOCH_SECONDS='$DEPLOY_STARTED_EPOCH_SECONDS' '$REMOTE_TOOL_ROOT/apply-deploy-unit.sh' rollback '$UNIT_ID'"
  exit 0
fi
