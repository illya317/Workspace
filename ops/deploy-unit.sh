...
for value in "$SERVER" "$REMOTE_DIR" "$WORKSPACE_GATEWAY_NGINX_SITE" "$WORKSPACE_MONOLITH_WECOM_PROCESS_NAME" "$WORKSPACE_RUNTIME_PM2_RUNNER" "${DEPLOY_PROFILE_PREPARED_STATE_ROOT:-}"; do
  if [[ "$value" == *"'"* ]]; then
