...
const unitProductionInvariants = readFileSync("ops/release/deploy/unit-production-invariants.sh", "utf8");
...
test("client deploy accepts only trusted artifacts while rollback remains an explicit operator action", () => {
  ...
  assert.match(client, /source "\$PROJECT_ROOT\/ops\/release\/deploy\/unit-production-invariants\.sh"/);
  assert.match(unitProductionInvariants, /runuser -u workspace-runtime -- test -x '\$REMOTE_DIR\/\.workspace'/);
  ...
  assert.ok(apply.indexOf("qualify_apply_deploy_unit_lock") < apply.indexOf('[ -d "$CONFIG_ROOT" ]'));
  assert.match(apply, /共享 runtime config root 不存在，unit apply 禁止创建或重设它/);
  assert.doesNotMatch(apply, /mkdir -p "\$CONFIG_ROOT"/);
  assert.doesNotMatch(apply, /chmod[^\n]*"\$CONFIG_ROOT"/);
