...
if [ -f .next/BUILD_ID ] && [ "$(cat .next/BUILD_ID)" = "$RELEASE_SOURCE_SHA" ]; then
  echo "==> production build 可用；继续执行一次性 PostgreSQL migration/seed 与全量 E2E"
  set +e
  start_disposable_postgresql \
    && RELEASE_SOURCE_SHA="$RELEASE_SOURCE_SHA" bash ./ops/run-release-e2e.sh
  e2e_status=$?
  set -e
else
...
echo "    disposable-postgresql + full-e2e: $([ "$e2e_status" = "0" ] && echo passed || echo "failed ($e2e_status)")"
if [ "$ci_status" != "0" ] || [ "$e2e_status" != "0" ]; then
