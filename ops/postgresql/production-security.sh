...
managed_process_names() {
  ...
  case "$mode" in
    prepared)
      node "$SCRIPT_DIR/production-pm2-plan.mjs" names --plan "$1" --require-prepared-owner
      ;;
    current-restore)
      node "$SCRIPT_DIR/production-pm2-plan.mjs" names --plan "$1" --require-current-specs
      ;;
    *)
      node "$SCRIPT_DIR/production-pm2-plan.mjs" names --plan "$1"
      ;;
  esac
}
...
if [ "$COMMAND" = apply ]; then
  ...
  verify_workspace_http 1
  finance_bot_contract prepare
  managed_names="$(managed_process_names "$backup_dir/pm2-plan.json" prepared)" || {
    echo "[错误] 无法验证 Gateway owner 对应的 PM2 plan" >&2
    exit 1
  }
  [ -n "$managed_names" ] || { echo "[错误] PM2 plan 未包含受管进程" >&2; exit 1; }
  write_receipt applying "$backup_dir"
  systemctl stop finance-bot.service 2>/dev/null || true
  install_hba transition "$backup_dir"
  while IFS= read -r name; do legacy_pm2 delete "$name" >/dev/null 2>&1 || true; done <<< "$managed_names"
  ...
  managed_names="$(managed_process_names "$backup_dir/pm2-plan.json" current-restore)" || {
    echo "[错误] 无法验证 Gateway owner 对应的 PM2 plan" >&2
    exit 1
  }
  [ -n "$managed_names" ] || { echo "[错误] PM2 plan 未包含受管进程" >&2; exit 1; }
  ...
  if [ -x "$RUNTIME_RUNNER" ]; then
    while IFS= read -r name; do "$RUNTIME_RUNNER" delete "$name" >/dev/null 2>&1 || true; done <<< "$managed_names"
  ...
  while IFS= read -r name; do legacy_pm2 delete "$name" >/dev/null 2>&1 || true; done <<< "$managed_names"
  node "$SCRIPT_DIR/production-pm2-plan.mjs" apply --plan "$backup_dir/pm2-plan.json" \
    --runner "$rollback_runner" --current-owner
  legacy_pm2 save >/dev/null
