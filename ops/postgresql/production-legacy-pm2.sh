#!/usr/bin/env bash
set -euo pipefail
LEGACY_USER="${WORKSPACE_LEGACY_OS_USER:-ubuntu}"
LEGACY_HOME="${WORKSPACE_LEGACY_HOME:-/home/ubuntu}"
LEGACY_ENV_FILE="${WORKSPACE_LEGACY_ENV_FILE:-/home/ubuntu/workspace/.workspace/.env}"
PM2_BINARY="${WORKSPACE_PM2_BINARY:-/usr/bin/pm2}"
[ "$(id -u)" -eq 0 ] || { echo "[错误] 必须由 root/sudo 调用" >&2; exit 77; }
[ -r "$LEGACY_ENV_FILE" ] || { echo "[错误] legacy env 不可读" >&2; exit 1; }
process_environment=()
for key in PORT HOSTNAME BUILD_VERSION NEXT_PUBLIC_BUILD_VERSION PG_POOL_MAX PG_APPLICATION_NAME \
  WORKSPACE_DEPLOY_UNIT_ID WORKSPACE_INTERNAL_ORIGIN WORKSPACE_INTERNAL_SIGNING_PRIVATE_KEY_FILE \
  WORKSPACE_INTERNAL_TRUSTED_PUBLIC_KEYS_FILE WORKSPACE_INTERNAL_REPLAY_DIRECTORY WECHAT_BOT_BRIDGE_URL; do
  value="${!key-}"
  [ -z "$value" ] || process_environment+=("WORKSPACE_PM2_PROCESS_$key=$value")
done
exec runuser -u "$LEGACY_USER" -- env -i HOME="$LEGACY_HOME" USER="$LEGACY_USER" LOGNAME="$LEGACY_USER" \
  PATH="/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin" PM2_HOME="$LEGACY_HOME/.pm2" \
  "${process_environment[@]}" \
  WORKSPACE_LEGACY_ENV_FILE="$LEGACY_ENV_FILE" /bin/bash -c '
    set -euo pipefail
    declare -A process_values=()
    for key in PORT HOSTNAME BUILD_VERSION NEXT_PUBLIC_BUILD_VERSION PG_POOL_MAX PG_APPLICATION_NAME \
      WORKSPACE_DEPLOY_UNIT_ID WORKSPACE_INTERNAL_ORIGIN WORKSPACE_INTERNAL_SIGNING_PRIVATE_KEY_FILE \
      WORKSPACE_INTERNAL_TRUSTED_PUBLIC_KEYS_FILE WORKSPACE_INTERNAL_REPLAY_DIRECTORY WECHAT_BOT_BRIDGE_URL; do
      prefixed="WORKSPACE_PM2_PROCESS_$key"
      process_values["$key"]="${!prefixed-}"
      unset "$prefixed"
    done
    set -a
    . "$WORKSPACE_LEGACY_ENV_FILE"
    set +a
    for key in PORT HOSTNAME BUILD_VERSION NEXT_PUBLIC_BUILD_VERSION PG_POOL_MAX PG_APPLICATION_NAME \
      WORKSPACE_DEPLOY_UNIT_ID WORKSPACE_INTERNAL_ORIGIN WORKSPACE_INTERNAL_SIGNING_PRIVATE_KEY_FILE \
      WORKSPACE_INTERNAL_TRUSTED_PUBLIC_KEYS_FILE WORKSPACE_INTERNAL_REPLAY_DIRECTORY WECHAT_BOT_BRIDGE_URL; do
      value="${process_values[$key]-}"
      [ -z "$value" ] || export "$key=$value"
    done
    exec "$0" "${@:1}"
  ' "$PM2_BINARY" "$@"
