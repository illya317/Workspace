...
test("CNB deployment path does not call GitHub APIs, Actions, Releases, or evidence", () => {
  ...
  assert.doesNotMatch(cnbPipeline, /npm run deploy:preflight:ci|npm run docs:check|npm run test:node/);
  assert.match(cnbPipeline, /build-standalone-artifact\.sh/);
  assert.match(cnbPipeline, /bash \.\/ops\/deploy\.sh/);
});

test("local full CI is the only quality gate before CNB packaging", () => {
  ...
  assert.match(releaseToCnb, /--local-ci-receipt/);
  assert.match(releaseToCnb, /--local-ci-receipt "\$LOCAL_CI_RECEIPT"/);
  assert.match(cnbPipeline, /cnb-deploy-request\.mjs validate/);
});
