...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    --local-ci-receipt)
      shift
      [ "$#" -gt 0 ] || { echo "[错误] --local-ci-receipt 缺少文件路径"; exit 1; }
      LOCAL_CI_RECEIPT="$1"
      ;;
    -h|--help)
      cat <<'EOF'
...
选项:
  ...
  --local-ci-receipt FILE   必填；当前 Git tree 的本地 npm run check:ci 凭证
...
[ -n "$LOCAL_CI_RECEIPT" ] && [ -f "$LOCAL_CI_RECEIPT" ] || {
  echo "[错误] 必须提供可读的 --local-ci-receipt"
  exit 1
}
LOCAL_CI_RECEIPT="$(cd "$(dirname "$LOCAL_CI_RECEIPT")" && pwd)/$(basename "$LOCAL_CI_RECEIPT")"
...
request_args=(
  ...
  --repository "$CNB_REPO"
  --local-ci-receipt "$LOCAL_CI_RECEIPT"
  --output "$REQUEST_FILE"
)
