...
开工顺序：先读 `AGENTS.md`，再读 `docs/engineering/project-overview.md` 确认项目地图和事实来源，最后按任务进入对应 `docs/roles/*.md`。
...
| 执行 Architecture 拆出的 Structure 任务包 | Feature / Data / Operations | `docs/engineering/structure-agent-execution.md`、对应模块 `ARCHITECTURE.md` |
...
1. 先读 `docs/engineering/structure-agent-execution.md`，确认动作类型、目标层、依赖顺序和 baseline 权限。
...
收口人只在相应风险存在时增加 `arch:gate`、`check:data` 或 `typecheck:scope -- <package>`。全仓 typecheck/build 只留给 CI/发布，不因为局部 agent 任务默认启动。

本地开发固定使用 3000 端口且全机只允许一个实例。统一运行 `npm run dev`，不得附加端口参数；3000 已占用时复用现有 Workspace 实例，不得改用其他端口。需要人工确认时运行：
