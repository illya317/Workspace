...
- 没有“全部/默认”项的互斥筛选可声明 `accordionTrigger: "active"`：当前值始终作为母按钮，点母按钮只展开/收起而不改值，其余选项全部作为子项。
...
- “筛选条件 / 更多操作”必须通过 portal 挂到页面根节点，覆盖移动端底栏并使用独立滚动正文；标题与关闭入口常驻，正文限制在动态视口内并保留 `safe-area-inset-bottom`，不能使用触发器下方的 dropdown 代替。
- “更多操作”按“操作 / 显示与设置”分组；即时动作选择后关闭，列显隐、条/页等设置型控件留在面板中连续调整。面板支持背景关闭、Escape、焦点回收和从移动端切到桌面断点时自动关闭。
- `page-size` 是离散设置：桌面使用只读触发器打开单选列表，移动端在“显示与设置”中直接显示紧凑分段选项；任何端都不得把条/页渲染成可自由输入或可清空的文本框。
- “筛选条件”使用紧凑密度；标题区到第一个筛选控件只保留正常触控间距，不得叠加大段 section/card padding。
- 业务仍只声明原有 typed item，不声明移动端位置、优先级或折叠方式；Core 根据 item 语义和 ActionGlyph 顺序自动决定。
- `PageSurface.tabbar` 在移动端 2-3 项时使用紧凑分段切换；超过 3 项或存在 children 时改为“当前栏目 + 切换栏目”底部面板，父栏目与子栏目使用可完整换行的分组选择项，禁止负边距和截断破坏中文标签。
