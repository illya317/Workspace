...
- L1：公开入口，只允许 `PageSurface`、`FormSurface`、`DataSurface` 和 `useFeedback`。页面布局只能从 `PageSurface` 进入。
...
- 只从 `@workspace/core/ui` 使用公开入口：`PageSurface`、`FormSurface`、`DataSurface`、`useFeedback`。页面级导航、toolbar、分页必须通过 `PageSurface.navigation`、`PageSurface.toolbar`、`PageSurface.footer` 表达。
...
- 不直接 import L2/L3 组件作为业务 runtime 组件；过渡期只允许上述 Surface contract / business alias type-only 引用。
...
Core UI 的 owner taxonomy 是五个 L1 family，不等于五个 runtime Surface：

| L1 family | L2 owner | 说明 |
|---|---|---|
...
| `feedback` | `feedback.service` / `feedback.renderer` / `feedback.compat` | `useFeedback`、反馈 renderer 和历史兼容入口。 |
...
`NavigationSurface`、`FormSurface`、`DataSurface` 不决定页面位置。`NavigationSurface` 归 `common.chrome`，只能是 PageSurface 内部 renderer 或正文 primitive；`FormSurface` / `DataSurface` 只能选择正文内容形态，不承载页面级 toolbar/pagination。
...
- 业务 runtime 不直接 import `Toolbar`、`PanelCard`、`DataTable`、`SelectField`、`ConfirmModal` 等 L2/L3 entry；通过 `PageSurface` 五段协议、正文 Surface 的 kind/spec 或 `useFeedback` 表达。
...
- 业务侧左侧列表、目录树、选择区和流程步骤链接通过 `PageSurface.body` 中的 navigation typed block 或正文 Surface spec 表达；`NavigationSurface`、`SelectorPanel`、`SelectorList`、`SelectorTree`、`SelectorCard` 是 L2/L3 或 Core 内部组合件，不作为页面布局入口。
...
新增 L1 Surface 时必须显式设置 registry `uiLevel: 1`。L1 名单由 gate 硬锁为 `PageSurface`、`FormSurface`、`DataSurface`、`useFeedback`；任何其他 registry entry 声明或解析为 L1 都会失败。L2/L3 可以省略 `uiLevel` 使用默认派生；L4+ 必须由 `isCoreUiComponentVisibleInShowcase` 隐藏，且不得作为 UI 组件库主展示根节点或可见直接关系暴露。

新增或迁移 registry entry 时必须填写 `ownerL1`、`ownerL2`、`role`、`publicUse`。第一阶段允许旧 entry 缺字段，但 `arch:level2` 会报告 `coreUiMissingOwnership`、`coreUiInvalidOwnership`、`coreUiCommonDomainDependency`、`coreUiSiblingL2Coupling`、`businessCommonRendererImports`、`domainSharedL2LayoutShells`、`surfaceOwnsPageChrome`。日常/CI 通过 `check:hygiene:warn` 只提示不阻断；Hygiene strict 通过 `check:hygiene` 按 baseline 强制，只允许债务减少，不能为了消警把 domain shared shell 注册成 Core/Page API。
...
- 业务 UI 和 `app/(modules)` 只能从 `@workspace/core/ui` runtime 引入 `PageSurface`、`FormSurface`、`DataSurface`、`useFeedback`；页面级 `NavigationSurface tabs/pagination`、`FormSurface/DataSurface toolbar`、`DataSurface pagination`、`AppShell/PageShell` 直连由 `pageSurfaceLayoutProtocolWarnings` hygiene baseline 锁定，只能减少。
