...
- 重点审查业务是否直接 import `SelectorCard`，或手写 `PanelCard + SelectorCard` 作为左侧选择区；业务应改用 cards-only `SelectorSurface` spec，再由 `BodySurface kind="section" layout="split"` 的 `left` 承载。
...
- `createBodySplitSection({ left, drawerLeft, right, side, layout })`：生成 `BodySurface kind="section" layout="split"`；左右两侧都接 `BodySurfaceProps`，左侧可以是 `kind="selector"`，也可以是普通 section/data/form/document 等正文。手机端由 Core 先全屏显示左侧，完成选择后全屏进入右侧详情，并提供返回左侧的页面内导航，不再使用悬浮抽屉。
- `createFixedSidebarBody({ left, right })`：生成固定侧栏 + 主详情的响应式 split 结构；业务只声明两侧 BodySurface，不声明宽度、间距或移动端换序；手机端同样走左侧列表到右侧详情的全屏推进。
