...
面向授权人员的完整 `resource.action` 含义、具体业务动作、继承方式和 `approve` 专项说明见自动生成的 [`全项目权限 Action 授权手册`](../../generated/permission-actions.md)。授权时不要只凭 action 名或本页的摘要表判断。

生产权限的每日复查、授权后即时复查、核准基线和异常通知规则见 [`权限定期复查与异常通报`](./permission-review.md)。
...
完整清单不再手工维护，统一由 [`全项目权限 Action 授权手册`](../../generated/permission-actions.md) 从当前 registry 生成。手工表曾多次遗漏新资源或保留旧 action，不能继续作为授权依据。
...
npm run typecheck:scope -- platform
```
