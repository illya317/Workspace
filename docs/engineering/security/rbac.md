...
| subjectType | 说明 |
|---|---|
| `user` | 例外性直接授权到用户；只能由内置 root admin 维护，常态基线应为零 |
...
`setSubjectPermissionActionGrant(s)` 是显式 action grant 的共享写入口。PostgreSQL 事务先按稳定顺序获取授权域 advisory lock：授权域包含本次目标资源和调用方声明的授权前置资源，并为每个资源展开自身、所有 registry 祖先，以及递归 `capabilityOwnerKey` owner 及其祖先；随后再按稳定顺序获取 `subjectType + subjectId + resourceKey + actionKey + scopeId` 授权元组锁。取得锁后，同一事务才重新校验主体存在、root identity 禁止授权、个人直授的操作者仍为有效 root admin、Resource FK 和调用方 `beforeMutation(tx)` 授权前置条件，然后写入 grant 与对应台账。标准后台请求对个人直授的新增与撤销都只接受 root admin；共享 setter 额外阻断任何缺少有效 root 操作者的个人直授新增，避免内部调用绕过。
...
业务权限默认发给岗位或部门。个人直授不配套申请、审批、有效期或例外流程；确需例外时由 root admin 直接维护并同步 permission-review 核准基线。一次性清理或组织迁移使用维护型 data release，必须写权限台账，不能靠 seed 或部署启动脚本静默改权。
...
- 治理委员会空间：执行负责人是业务 manager。
- 公司空间：默认全员 viewer。

业务 manager 不等于 `grant`。空间权限矩阵可按对应 scoped `grant`、全局 `grant` 或 root identity 开放查看，但当前空间授权主体仍是 user，因此只有 root identity 可以实际新增或撤销；非 root 矩阵必须保持只读。未来若空间授权改为岗位/部门主体，再单独扩展写入规则。
...
- 是唯一可维护 `UserResourceActionGrant` 的操作者；这不表示 admin 自己会成为个人授权主体。
...
- 左侧：资源树。
- 顶部：员工 / 岗位 / 部门切换；员工矩阵用于查看有效权限，只有 root admin 可以切换个人直授，其他授权管理员为只读。
...
- 部门、公司、治理委员会空间里的业务动作在 `space.<target>.projects` / `space.<target>.tasks` 上授权。
