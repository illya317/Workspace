...
> 调研日期：2026-06-04
> 对比目的：借鉴 ERPNext 成熟方案，确认当前 action grant 模型的合理性。
...
| **动作授权层** | Role doctype + Has Role 子表 | `UserResourceActionGrant` / `PositionResourceActionGrant` / `DepartmentResourceActionGrant` |
...
| **文档/对象层** | DocPerm（doctype JSON 中 `permissions` 数组） | Resource tree（`finance.ledger`, `hr.roster`） |
...
**核心差异**：ERPNext 是**文档为中心**——权限围绕 doctype（"Sales Order" 这种业务对象）配置；我们是**页面为中心**——权限围绕 resourceKey（`finance.ledger` 这种页面路由）配置。

---

## 2. 动作层对比

| **ERPNext（14 层）** | **我们（action grant）** |
|----------------------|-----------------|
| `read` — 查看文档 | `entry` / `read` — 页面进入 / 数据读取 |
| `write` — 编辑 | `update` — 编辑数据 |
| `create` — 新建 | `create` — 新建 |
| `delete` — 删除 | `delete` — 删除数据 |
| `submit` — 提交（状态流转） | `submit` — 发起流程 |
| `cancel` — 取消 | `reject` / `reverse` 按业务语义拆分 |
| `amend` — 修正 | `revise` — 修订 |
| `print` — 打印 | （无） |
| `email` — 邮件 | （无） |
| `report` — 报表 | （无） |
...
| 缺口 | 影响 | 建议 |
|------|------|------|
| 字段级权限 | 敏感字段只能按业务 service 单独拦截 | 暂不引入通用字段权限，先用 DTO/service guard |
| 数据范围权限 | scoped action grant 只覆盖已声明 scope | 资源必须在 `permission-resource-policy` 显式声明 scope |
| 审批 SoD | 同人提交与审批目前只提示 | 后续按流程资源决定是否阻断 |
...
| 分配方式 | **ERPNext** | **我们的系统** |
|----------|-------------|----------------|
| **直接分配** | User → Has Role 子表 | `UserResourceActionGrant`（直接给用户赋权） |
| **间接分配** | Role Profile 角色包 | `PositionResourceActionGrant`（岗位继承）+ `DepartmentResourceActionGrant`（部门继承） |
| **默认角色** | `All`（全员）、`Guest`（访客） | 无 |
| **超级用户** | `Administrator`、`System Manager` | root identity |
| **祖先传播** | 无 | ✅ 子资源权限自动使祖先可见（`finance.ledger` → `finance` 自动可见） |
...
| **页面门禁怎么做** | 没有页面门禁，Desk 角色 + doctype 权限决定用户能看到什么 | `requireResourceAccess("finance.ledger")` 直接做路由门禁 |
...
| **配置界面** | Permission Manager UI + doctype JSON | 权限矩阵 UI + resource/action policy 代码声明 |
...
4. **三层继承**：用户 + 岗位 + 部门三层 action grant 叠加，ERPNext 只有直接分配和 Role Profile 两层

---

## 6. 结论

- 我们的 action grant 已覆盖基础 CRUD、流程、生命周期、导入导出、授权和管理动作
