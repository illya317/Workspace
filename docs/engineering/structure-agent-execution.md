...
| Core UI registry | `packages/core/ui/registry/component-registry.ts` | 给 AST/pattern scan 提供可消费的 Core UI primitive/page shell 白名单 | Architecture/Core |
...
| 搜索框收口 | 先确认 `SearchInput` / `FkFieldInput` / `SearchableOptionInput` 是否覆盖场景 -> 删除 app/业务包一次性搜索控件 -> 业务侧只传 value/options/fkKey -> 确认 `nativeSearchInputFiles` 仍为 0 |
...
`nativeSearchInputFiles` 是搜索型原生 input 的硬约束，baseline 必须保持空数组。Feature/UI 发现旧的一次性搜索控件时应删除并改用 Core `SearchInput` / `FkFieldInput` / `SearchableOptionInput`，不要维护旧控件，也不要为新文件扩写 baseline。
...
| Feature | `npm run arch:gate`; 有类型风险时 `npm run typecheck:scope -- <package>`；涉及 UI 时按需浏览器验证 |
