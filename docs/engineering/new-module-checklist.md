...
- [ ] GET → `read` 或仅入口 `entry`；POST → `create`/`submit`/`import` 等真实语义；PUT/PATCH → `update`/`revise`/`grant`/`configure` 等真实语义；DELETE → `delete` 或软删除语义 `archive`
...
```bash
npm run typecheck:scope -- <domain>
npm run typecheck:scope -- app-<unit>
