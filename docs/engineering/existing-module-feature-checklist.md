...
这类任务不要套 `docs/engineering/new-module-checklist.md` 的“新模块接入”全流程，按下面的轻量流程执行。
...
- [ ] GET 用 `read`；纯入口只用 `entry`
- [ ] POST/PUT/PATCH 按真实语义用 `create`、`update`、`submit`、`revise`、`grant`、`configure` 等
- [ ] DELETE 用 `delete`
...
- [ ] 优先挂到当前模块已有资源，例如 `finance.ledger.read/update`
- [ ] 页面可见性和 API 权限保持一致
...
- 查看类：`*.read`
- 进入页面类：`*.entry`
- 提交/审核类：`*.submit` / `*.approve` / `*.reject`
- 删除类：`*.delete`

## 6. 文档只补真正变化的边界

- [ ] 如果只是模块内扩展，优先更新该模块的 `ARCHITECTURE.md`
- [ ] 只在新增独立模块时才走 `docs/engineering/new-module-checklist.md`
...
只有出现类型风险或需要类型诊断时，才补跑 `npm run typecheck:scope -- <domain>`。普通模块内小改不跑全仓 lint、TypeScript 或 build。
...
如果这次需求的核心是“再长出一个新的业务域”，就看 `docs/engineering/new-module-checklist.md`。
