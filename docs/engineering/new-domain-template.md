...
权限 action 必须使用 `packages/platform/action-registry.ts` 已注册的新动作。常用映射：

- `entry` / `read` — 页面进入、列表/详情读取。
- `create` / `update` / `delete` — 新增、修改、删除记录。
- `archive` / `revise` / `reverse` — 生命周期归档、受控修订、撤销/反向业务处理。
- `submit` / `approve` / `reject` — 流程提交、通过、驳回。
- `import` / `export` / `apiUse` / `share` — 导入、外发、API 调用、共享。
- `grant` / `configure` / `audit` — 授权、配置、审计。
...
- [ ] 运行本地约束：`npm run arch:gate && npm run lint:changed && npm run typecheck:scope -- <domain>`；全仓 type/build 由 CI/发布收口
