...
Deploy 通知只使用明确的双时钟合同。`deployRequestedAt` 在 `ops/publish.sh deploy` 收到命令时记录，覆盖入口预检、single-flight/retry fence、Ready 复验和生产执行；`mutationStartedAt` 只在 Full 或 unit 持锁完成生产语义复验、即将越过实际 mutation barrier 时记录。终态同时记录 `finishedAt`、`endToEndDurationSeconds` 与 `mutationDurationSeconds`，禁止用含糊的单一 duration 兼任两种口径。正常部署开始和进入 mutation barrier 都不发消息；超过软阈值最多发送一次带当前 phase/elapsed/blocker 的提醒；失败或取消立即发送终态；只有 health/version/content digest 全部验收通过才发送唯一成功消息。Neko 的渲染真源是 tracked `ops/postgresql/production-finance-bot-deploy-renderer.py`，由 `production-finance-bot-hook.sh` 合入 hardened runtime；禁止直接修改 `/usr/local/bin/finance-bot.py` 或 `/usr/local/lib/workspace-security/finance-bot.py`。
