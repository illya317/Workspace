...
正式单 unit 发布必须先运行 `OPS_ENV_FILE=/path/to/private/.env ops/publish.sh prepare --deploy-unit <unit>`；shadow 使用同 unit 的 `--shadow-unit` prepare。该入口参考 Full 保留干净 committed release worktree、精确 tree 回执、缓存隔离和一次性 PostgreSQL migration/seed，但静态范围只包含共享发布协议、deploy graph `privateSourceRoots` 的 lint/Node 与少量部署协议测试，不重复 Full 的 domain/UI/docs/data/full lint/全部 Node。编译范围由 deploy graph 的 package + `app-<unit>` closure 派生，lint cache 使用 `.cache/eslint/units/<unit>.eslintcache`，production build 使用 `.cache/next-units/<unit>`，浏览器只运行 unit contract 声明的 suite，并直接启动刚生成的目标 artifact。成功回执位于 `.cache/release-check/units/<unit>.json`，绑定 source、tree、unit、contract、graph、artifact 与 schema-v2 unit-CI 证据；Full、不同 unit 和 target 不得交叉复用。随后 `deploy --deploy-unit <unit>` 或 `deploy --shadow-unit <unit>` 只消费相同目标的回执。
