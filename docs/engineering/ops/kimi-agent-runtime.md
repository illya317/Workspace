...
- Platform 固定只向模型暴露 `workspace.api.discover`、`workspace.api.read`、`workspace.api.proposeMutation`；每次 Wire tool call 都重新校验活跃 Workspace binding、Agent action ceiling、请求人权限和可双主体判定时的虚拟员工 actor 权限。profile 列表只返回当前请求人至少可用一个 connector 的虚拟员工。没有 Workspace 绑定的本地 Codex、CI、服务器虚拟员工不能进入页面助手。虚拟员工账号固定不可登录，不能被当作浏览器会话。
...
- 澄清只允许询问普通业务用户能够回答的业务选择或事实。业务 API 路径和请求结构必须从受控 catalog 或业务 contract 响应发现，不能编造；源码路径、组件名、schema、内部实现和服务器信息不属于 Agent 能力，也不能要求用户粘贴代码或提供文件列表。
- 所有业务写入都只能由 `workspace.api.proposeMutation` 冻结为 proposal；请求人确认后，独立 Workspace API 固定原 profile/actor 和原始 method/path/query/body，原子抢占状态、重新鉴权，再调用同一标准业务 API。抢占记录带执行 token 与租约；dispatch 后异常只能落为“结果未知、需人工核对”，不得自动重放。模型没有 direct write、领域 executor 或源码 PR 特例。
...
Agent 会话消息、摘要和图片默认保存在 `$WORKSPACE_CONFIG_DIR/agent`。如需通过 `AGENT_DATA_DIR` 覆盖，必须使用绝对路径；服务端不接受相对路径，也不会把运行态存储回退到源码 checkout。Agent runtime 不接收源码 worktree/cache 配置，不挂载应用源码、`.env`、数据库或服务器 home；模型只能经 Platform 注入的固定 connector 访问已登记业务 API。

生产 Ubuntu/Debian 服务器执行：

```bash
export WORKSPACE_CONFIG_DIR=/srv/example-workspace/.workspace
