...
- The runtime receives execute-only ACLs on the production home and release roots. Hardened installation backs them up for rollback, every deploy reconciles them idempotently, and the internal health endpoint fails closed when `WORKSPACE_CONFIG_DIR` is no longer traversable; it must never require directory listing or write access on those roots.
