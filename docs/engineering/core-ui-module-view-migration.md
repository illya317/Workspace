...
| `tabbed-block-host` | 已撤销；页面 tabs 只由 `PageSurface.tabbar` 声明，正文只返回 `BodySurfaceProps`/typed sections |
...
| `navigation-surface-spec` | 已补 `NavigationSurface`：tabs、pagination、context selector；业务实体 list/tree 选择统一走 `SelectorSurface` |
...
| `packages/production/ui/qc/QcBatchStagePrecheck.tsx:precheck-header` | migrated | Platform `StageFlowSurface` + `PageSurface` heading | QC 阶段和检测项导航由业务 workflow stage-flow 声明承载，不再复用页面 tabs 导航协议 |
