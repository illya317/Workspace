...
| Settings `settings` -> `/settings` | Platform | `settings.account` -> `/settings/account`; `settings.admin` -> `/settings/admin`; `settings.api` -> `/settings/api`; `settings.ui` -> `/settings/ui` | `workspace-shell` | account owns inbox, transparent notification catalog and permission-scoped personal subscriptions; admin owns reminder routing/run governance; API has Platform APIs; UI registry is page-only |
