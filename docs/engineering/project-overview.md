...
| Settings `settings` -> `/settings` | Platform | `settings.account` -> `/settings/account`; `settings.admin` -> `/settings/admin`; `settings.api` -> `/settings/api`; `settings.ui` -> `/settings/ui` | `workspace-shell` | account/admin/api have Platform APIs; UI registry is page-only |
| Agent `agent` (headless) | Platform | none | `assistant` | `/api/agent` and toolbar use `agent.assistant`; profile-only source/PR tools additionally require `agent.source` |
