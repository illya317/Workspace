...
| 新建正文 | `PageSurface.create` / `BodySurface kind="create"` | 页面级新建只占一个 `PageSurface.create` slot，可选 inline/block/modal；局部新建固定 `trigger: surface`，可选 block/modal、block anchor 和 typed form/sections | 业务声明 `trigger: toolbar`、直接 import 内部 create renderer、自行拼新建动作区，或为 section header create 手工放 anchor/加号 |
