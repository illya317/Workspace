...
| 页面内容/分栏能力 | `createMasterDetailBody` 与 `BodySurface` section/disclosure spec | 页面内容留白、卡片、章节、空态、折叠面板、主列表 + 详情分栏；实体选择主栏声明 `master.presentation: "compact"`，桌面把完整选择卡投影为身份、状态和一个辅助事实，未声明比例时使用稳定侧栏，移动端仍显示完整字段；折叠和移动端推进由 Core 统一管理 | 业务直接 import `PageShell`、`PageContent`、`PanelCard`、`SectionCard`、`WorkspaceSplitPage`，或在业务层手写分栏、卡片字段裁剪、drawer、折叠状态和按钮 |
