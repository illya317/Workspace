...
| 表格/记录/指标能力 | `BodySurface` data/metrics/record spec | 标准列表、批量表格、记录卡、指标卡；Core 统一约束表头、文字和动作组不得越出单元格，动作较多时自动换行 | 业务直接 import `DataSurface`、`DataTable`、`StructuredTable`、`MetricCard`、`NumberCell`、`AmountCell` 或手搓表格 DOM |
...
- 标准新建只声明 `CreateSurface`：`trigger` 决定 `+` 在 Page Toolbar 或所属 Surface，`presentation` 决定 inline/block/modal，`anchor` 只决定 block 内容 target，`content` 决定单 form 或多 sections。inline 固定为 toolbar + 单 form + 无 anchor；所有非 inline 组合复用同一套 grid 表单格式。业务侧显式 Page toolbar 或手写 Selector/Section/Data 新建入口已禁止。
