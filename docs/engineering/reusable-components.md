...
- 已经下沉到 `packages/finance/ui`：`FinanceShell`、`CompanyPeriodPicker`、`FinanceFilters`、`Pagination`、重分类配置/审核组件。旧 `app/finance/components` 兼容目录已删除。`FinanceShell` 这类 domain shared L2 layout shell 只算历史债或 thin domain adapter，禁止注册为 Core/Page API；长期应由 route/module 层生成 `PageSurface` props。
