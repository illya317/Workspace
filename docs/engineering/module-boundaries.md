...
- `packages/platform/ui` 已接收登录后的 Portal、普通 L1 模块首页、AppShell、跨页 NavLink、用户菜单、设置页和审计日志 UI；普通 L1 必须挂 `ModuleHomePage`，或只做鉴权后 redirect 到已注册的默认 L2 页面。`/work` 是 Work L1 主入口，使用 Work 自有入口卡片；部门空间由 `/work/department/:departmentId` 进入。
