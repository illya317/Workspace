...
- pre-commit 默认只运行 staged/changed 增量；只有显式 `PRE_COMMIT_FULL=1` 才运行 `npm run check:ci` 并按当前 tree 写入全量通过记录。pre-push 使用风险分类运行 `npm run check:push`，显式 `PRE_PUSH_FULL=1` 才全量；`publish.sh push` 走 GitHub 候选 PR，`publish.sh deploy` 则完全由 CNB 对当前已提交 source SHA/tree 做预检、构建和部署。不要手动用 `--no-verify` 绕过检查。
