...
- pre-commit 默认只运行 staged/changed 增量；只有显式 `PRE_COMMIT_FULL=1` 才运行 `npm run check:ci` 并按当前 tree 写入全量通过记录。pre-push 使用风险分类运行 `npm run check:push`，显式 `PRE_PUSH_FULL=1` 才全量；`publish.sh push` 走 GitHub 候选 PR。正式 Full 发布必须先用 `publish.sh prepare` 在本地聚合跑完完整 CI/E2E；正式单 unit 发布必须先用 `publish.sh prepare --deploy-unit <unit>` 跑完共享发布协议、unit 私有 lint/Node、graph compiler closure、目标 build/E2E，不重复 Full 前半段。两者都使用干净 release worktree 并写入 source/tree/target 回执，再由唯一生产入口 `publish.sh deploy` 只消费匹配回执并交给 CNB；禁止用 deploy/CNB 逐个发现代码错误。不要手动用 `--no-verify` 绕过检查。
