...
- 默认只运行当前阶段定义的门禁：提交前使用 staged snapshot；push 后使用远端 base/head affected scope；发布使用 validate 生成的同一 artifact。`check:ci`、`check:blockers`、`check:data`、`check:hygiene`、净增行预算等专项入口只在用户明确提出或定位对应故障时运行，不由风险或改动量自动追加。
- 开工先 stage 本任务文件；pre-commit 在临时 Git snapshot 中只检查 staged 内容。push 后由 GitHub 使用远端 base/head 识别改动 owner，并把反向依赖纳入受影响范围；风险和改动量只做诊断，不扩大门禁。正式发布先 `publish.sh prepare` 冻结 release tree，再用 `publish.sh validate`（或 `validate --local`）验证受影响依赖闭包并冻结 artifact；`publish.sh deploy`（或 `deploy --direct`）只消费同一 artifact，不重复源码检查或构建。除非用户明确要求，不追加全量或其他门禁，也不要用 `--no-verify` 绕过既定检查。
