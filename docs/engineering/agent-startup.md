...
- pre-commit 默认只运行 staged/changed 增量；只有显式 `PRE_COMMIT_FULL=1` 才运行 `npm run check:ci` 并按 staged tree 写入可供发布复用的全量凭证。pre-push 使用风险分类运行 `npm run check:push`，显式 `PRE_PUSH_FULL=1` 才全量；`publish.sh push` 是可选 GitHub 候选入口，`publish.sh deploy` 强制当前 tree 的本地全量凭证后直接进入 CNB。不要手动用 `--no-verify` 绕过检查。
