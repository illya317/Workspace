...
- pre-commit 默认只运行 staged/changed 增量；只有显式 `PRE_COMMIT_FULL=1` 才运行 `npm run check:ci` 并按当前 tree 写入全量通过记录。pre-push 使用风险分类运行 `npm run check:push`，显式 `PRE_PUSH_FULL=1` 才全量；`publish.sh push` 走 GitHub 候选 PR。正式发布必须先用 `publish.sh prepare` 在本地聚合跑完完整 CI/E2E 并写入当前 tree 回执，再由唯一生产入口 `publish.sh deploy` 只消费回执并交给 CNB；禁止用 deploy/CNB 逐个发现代码错误。不要手动用 `--no-verify` 绕过检查。
