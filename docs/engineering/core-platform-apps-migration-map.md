...
- 页面源码通过 route groups 收口：`app/(modules)/*` 放全部正常 L1 页面壳，`app/(system)/*` 只放平台保留页，`app/(auth)/*` 放登录；URL 仍使用 `/hr`、`/finance`、`/work` 等业务路径。
...
| 平台壳、模块首页、导航、用户菜单、Portal、审计 UI、Admin 管理后台 | `packages/platform` | 旧平台页面壳和 Portal | `packages/platform/ui/*`, `packages/settings/ui/admin/*`, `packages/platform/module-registry.ts`, `packages/platform/modules.tsx`, `packages/platform/audit/*` | `app/(modules)/settings/admin/page.tsx`、`app/(system)/portal/page.tsx` 只做鉴权和挂载 | 进行中；Platform UI 仍有 Structure 页面设计债务 baseline，后续以 Core primitive 收敛 |
...
| Finance 页面模板、筛选、公司期间、分页、表格工具栏、预算、成本、重分类 UI | `packages/finance` + `packages/core` | `app/finance/*/components/*`, `server/services/finance*` | `packages/finance/ui/*`, `packages/finance/server/*`, `packages/finance/types/*`, `packages/finance/constants/*` | 基本完成：页面壳/筛选/分页/表格/预算/成本/重分类共享 UI 直接从 `@workspace/finance/ui` 消费；财务数据导入 L2 已删除，底层解析器只供总账核对和受控脚本复用 | 旧 app 真实实现已删除；后续只允许 route 薄壳 |
...
| Settings 设置 | `packages/settings` | 旧 `app/settings/*` 与 Platform 内实现 | `packages/settings/ui/*`, `packages/settings/server/*`, `app/(modules)/settings/**/page.tsx` 薄壳 | 是：page 鉴权后挂载 package component | app 与 Platform 旧 UI 已删除 |
| Docs 文档中心与模板编辑 | `packages/docs` | 文档中心存量 route 与 Platform 内实现 | `packages/docs/ui/*`, `packages/docs/server/*`，`app/(modules)/docs/*` 只保留 route/layout | 是 | Platform 旧实现已删除 |
...
| External 外部关系 | `packages/external` | `app/external/types.ts`, `app/external/*/*Client.tsx` | `packages/external/types/*`, `packages/external/ui/*`, `packages/external/module.ts` | 是：外部关系子页只做鉴权、AppShell 和挂载 package component | 投资人关系已迁出；客户/供应商占位 UI 保留 |
| Capital Securities 资本证券 | `packages/capital-securities` | 原 `external.investors` 与 G 线组织入口 | `packages/capital-securities/ui/*`, `packages/capital-securities/types/*`, `packages/capital-securities/module.ts` | 是：页面只做鉴权、AppShell 和挂载 package component | 投资人迁入；治理架构 API 使用 Platform 组织单元服务 |
| Inventory 库存旧入口 | 无独立包；归入 Production 后续范围 | 无运行 UI | 后续若恢复库存能力，应放入 `packages/production` 或明确新业务包 | 是：无真实 page shell | 顶层 route 已删除 |
| Agent 工具与模型接入 | `packages/agent` | 旧 Platform Agent runtime | `packages/agent/ui/*`, `packages/agent/server/*`, `app/(modules)/agent/page.tsx` 薄壳 | 是 | Platform 旧实现已删除；共享鉴权和委托签名留在 Platform contract |
...
3. **Platform 平台能力**：继续收窄为跨 L1 的 registry、auth、RBAC、运行时 contract 和平台壳，不回收 Agent / Docs / Settings 业务实现。
