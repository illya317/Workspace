...
| 源码模块声明 | `npm run source-code-analysis:check` | 校验 L1 与八类受管 L1 的统一递归 Module contract、最深唯一归属、精确文件 Interface、祖先/后代及跨分支依赖方向、递归边界债务 baseline 保持为空、混合职责；任意长度的 TS/JS/Python/Shell/workflow/package-script 一方文件循环（含 type-only 与测试）无条件阻断。普通叶子 Module 达到 10,000 实现行、80 个实现文件或 15 个叶子依赖时输出 Hygiene warning；`entry/orchestrator` 只检查 3,000 行薄编排预算，不套用普通叶子的文件数和 fan-out。人工 `accepted` 最长 90 天且结构变化自动失效。运维专项注册方向、新脚本 450 行上限和历史超大脚本只减不增仍继续校验。 |
