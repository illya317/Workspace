...
| 单元生产发布门禁 | `OPS_ENV_FILE=/path/to/private/.env ops/publish.sh prepare --deploy-unit <id>` | 复用 Full 的干净 committed release worktree、精确 tree 回执和缓存纪律，但只运行共享发布协议、deploy graph 派生的 unit 私有目录 lint/Node、package + `app-<unit>` scopes、独立 Next artifact、一次性 PostgreSQL 和目标浏览器证据。不会重复 Full 的全库 domain/UI/docs/data、full lint 或全部 Node。回执、ESLint cache 和 `.cache/next-units/<unit>` 均按 unit 隔离；不能替代或复用 Full 回执。 |
