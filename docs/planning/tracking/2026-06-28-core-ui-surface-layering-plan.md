...
- `SelectorPanel` 已在 cards-only `SelectorSurface` 收敛后删除；`CreatePanel` 保持 internal renderer，公开声明已收口为单一 `CreateSurface`。
- 两者均已撤掉旧 `runtime` exposure；选择区只保留 `SelectorSurface` 声明路径。
...
- `PageSurface.body` / `PageSurface.tabbar` 是当前页面入口；旧顶层 `blocks/empty/actions/tabs/activeTab/activeChild/onTabChange/onChildChange` 已删除，不保留兼容迁移。
...
- 关键声明与 helper 已移动到 `packages/core/ui/`、`packages/core/ui/helpers/`、`packages/core/ui/services/`；`host` 目录保持空。
...
- `SelectorPanel`：Library、Work、Platform 调用点最终迁到结构化 `SelectorSurface`；过渡 helper 已在消费者归零后删除。
...
2. **CreatePanel 拆成声明接口**
   - 已删除失去职责的 `CreatePanel`、`BlockCreatePanel`，并去掉 `CreateStartButton` 等 renderer 上的 public declares。
   - 标准创建内容通过唯一 `CreateSurface` 声明；`trigger`、`presentation`、`anchor`、`content` 正交，所有非 inline 组合复用同一 typed FormSurface grid renderer。
   - Renderer 和按钮位置、样式、顺序留在 internal。

3. **SelectorPanel 拆成声明接口**
   - 已去掉 `SelectorPanel` renderer 上的 public declares。
   - 过渡期曾使用 `createSelectorPanelSection` helper；结构化 `SelectorSurface` 完成迁移后已删除该 helper。
