...
Do not continue the old "catch-all navigation + miscellaneous L2" direction. The current target model is:
...
| `common.selection` | `SearchableOptionInput`, `SelectorPanel`, `SelectionGrid`, `FieldValueFilter`, FK/search selection |
...
- A business package directly imports Common renderers such as `Toolbar`, `TabBar`, `InputSurface`, `Badge`, `SelectorPanel`, or `Pagination`. That is historical debt unless it is showcase/core-internal code.
...
- Do not move components across families based on the old navigation/chrome model.
- Collect current historical debt by category; page chrome debt now has a strict zero target:
  ...
  - direct `Toolbar` / `TabBar` / `Pagination` usage.
