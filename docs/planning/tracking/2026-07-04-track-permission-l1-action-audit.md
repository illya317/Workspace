...
| 11 | `agent` | Done | `/agent` and `/api/agent` | The Agent resource is now a visible L1 page: profile and capability discovery require `read`; message submit and own proposal confirm/cancel require `submit`; proposal confirmation is not `approve`, cancellation is not `reverse`, and delegated execution rechecks the requester, virtual actor, global action ceiling, and profile tool allowlist before every tool call and confirmed write. | Originally audited 2026-07-04; visible dual-identity runtime re-audited 2026-07-15. |
