...
> 历史路线图说明：本文记录早期治理过程。涉及新模块目录、service 位置和包边界时，以 `docs/engineering/module-boundaries.md`、`docs/engineering/architecture-governance.md`、`docs/engineering/reusable-components.md` 为准；不要按本文早期 `server/services/<domain>` 模板新增业务代码。
...
- `README.md` 改成项目地图。
- `AGENTS.md` 增加入口规则，细则沉到 `docs/engineering/agent-handbook.md`。
- 新增 `docs/engineering/architecture-governance.md`。
...
- `README.md` 只放项目总地图。
- `docs/engineering/architecture-governance.md` 放长期规则。
- 本文件已归档为 stale，仅保留现状优化路线图的历史语境。
...
- 模型已按领域拆分：
  ...
  ```
- 每个 model 前已加 `///` 注释。
- 新增 `docs/engineering/schema-governance.md` 和 `scripts/check-schema-governance.js`（7 条硬约束）。
...
- 新增 `docs/engineering/module-boundaries.md`。
...
现行新模块模板见 `docs/engineering/new-domain-template.md`。页面/API 只保留 Next 壳，真实 UI/service 放 `packages/<domain>`。
