...
- Work 下承接：
  - 项目管理：原 HR Project / EmployeeProject 的归属迁移，以及项目任务、项目阶段、依赖、基线和甘特。
  - 工作计划：`/work/tasks`。
- HR 页面不再展示“项目”入口。
...
- 项目业务访问规则统一由 `packages/work/server/access.ts` 计算。可见来源是创建人、主导部门负责人、项目 RASCI 成员、所属标准业务空间和 root admin；可写/管理/删除继续按项目角色、负责人和标准业务空间规则收敛。
