...
- route 不超过 120 行
- GET 用 `finance.statementConfig.read`
- PUT/PATCH 用 `finance.statementConfig.update`
- 参数非法返回 400
...
| API | 权限 |
|---|---|
| GET `/api/modules/finance/statement-config` | `finance.statementConfig.read` |
| PUT `/api/modules/finance/statement-config` | `finance.statementConfig.update` |

报表配置由 API contract 的 `finance.statementConfig` 资源与 `requiredActions` 在 gateway 统一鉴权，不复用 ledger 或 statements 权限。
...
- `docs/engineering/existing-module-feature-checklist.md`
