...
当前标记：`finance.ledger` 为 `legacy-fallback`；`finance.statement`、`finance.analysis`、`finance.budget`、`finance.cost` 为 `hybrid-analysis`；`finance.tax`、`finance.treasury` 为 `erpnext-owned`；旧 `production.inventory` 入口已关闭，历史数据暂存不再作为生产管理模块开放。
