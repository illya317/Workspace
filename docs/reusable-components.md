...
| FK 搜索输入 | `@workspace/core/ui` 的 `FKSearchInput` 或基于它的领域薄包装 | 选择部门、员工、计划、科目等关联实体 | 把 FK 搜索做成自由 `entity: string` 一次性控件 |
...
| 两段式筛选值 | Core `SearchInput` / `SelectField` / `OptionPicker` / `FKSearchInput` / `CalendarDateInput` | 第一段选择查询字段；第二段按查询 contract 选择值。普通模糊搜索输出 `queryParam + value`，FK 搜索输出明确实体引用，布尔/固定枚举走标准下拉 | 从表字段自动推导筛选参数、为本地枚举值再写一次性搜索框，或把普通字段值做成字段选择同款下拉 |
...
- `app/*` 和 `packages/*` 禁止新增搜索型原生 `<input>`；内容检索用 `SearchInput`，FK 用 `FKSearchInput` 或领域薄包装，下拉内检索用 `SelectField` / `OptionPicker`。
