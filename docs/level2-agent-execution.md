...
1. **新增 Core UI 公共入口**：从 `packages/core/ui/index.ts` 导出的组件、页面骨架、primitive，必须登记到 `packages/core/ui/component-registry.ts`，填写中文 `description`、中文 `example`，必要时补 `includes` 和 `RegistryBrowserCard` 示例。
