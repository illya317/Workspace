...
| 平台认证、权限、Prisma 入口、CRUD 工厂、历史、FK 解析 | `packages/platform/server` | `lib/permissions.ts`, `lib/history.ts`, `lib/resolve-fk.ts`；低层 token 仍在 `lib/auth/token.ts` | `packages/platform/server/*`, `packages/platform/resources.ts` | 部分完成：`lib/auth.ts`、`lib/with-auth.ts`、`lib/access.ts`、`lib/prisma.ts`、`lib/crud*.ts`、`lib/schemas.ts`、`lib/company-server.ts`、`lib/validation.ts` 已删除；`lib` 仍有少量兼容 re-export 和领域旧 helper | 未完成；迁移后 `lib` 只保留 re-export 或 Next 必需工具 |
