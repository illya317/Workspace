...
| 平台认证、权限、Prisma 入口、CRUD 工厂、历史、FK 解析 | `packages/platform/server` | `lib/permissions.ts`, `lib/prisma.ts`, `lib/crud-factory.ts`, `lib/history.ts`, `lib/resolve-fk.ts`；低层 token 仍在 `lib/auth/token.ts` | `packages/platform/server/*`, `packages/platform/resources.ts` | 部分完成：`lib/auth.ts` 聚合 hub 已删除；`lib` 仍承载其他兼容与少量真实逻辑 | 未完成；迁移后 `lib` 只保留 re-export 或 Next 必需工具 |
