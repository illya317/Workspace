...
- **Apps** 放业务能力：HR、Finance、Production、Administration、Contracts、Library 等各自拥有 `ui / server / types / constants / import / module`。
- Project 相关暂时不迁移，预留 `packages/project` 接入，不在本轮改。
...
| 平台壳、模块首页、导航、用户菜单、Portal、审计 UI | `packages/platform` | `app/components/AppShell.tsx`, `ModuleHome.tsx`, `NavLink.tsx`, `UserMenu.tsx`, `AuditLog*`, `app/portal/*` | `packages/platform/ui/*`, `packages/platform/modules.tsx`, `packages/platform/audit/*` | 部分完成：app 层仍大量通过兼容壳引用 | 未完成；应逐页改为 `@workspace/platform/ui` |
...
| Settings 设置 | `packages/platform` | `app/settings/*` | `packages/platform/ui/settings/*`, `packages/platform/server/settings/*` | 未完成 | 未完成 |
...
| Contracts 合同模块 | `packages/contracts` | `app/contracts/*`, `server/services/contracts.ts` | `packages/contracts/ui/*`, `packages/contracts/server/*`, `packages/contracts/types/*`, `packages/contracts/module.ts` | 未完成 | 未完成 |
| Library 资料库模块 | `packages/library` | `app/library/*`, `app/api/library/*`, `server/services/library/*` | `packages/library/ui/*`, `packages/library/server/*`, `packages/library/types/*`, `packages/library/module.ts` | 未完成 | 未完成 |
| Administration 行政模块 | `packages/administration` | `app/administration/*` | `packages/administration/ui/*`, `packages/administration/server/*`, `packages/administration/module.ts` | 未完成 | 未完成 |
...
5. **硬约束升级**：`arch:check` 继续扩大检查范围，禁止新真实逻辑进入 `app/components`、`app/hooks`、`lib`、`server/services` 的旧位置。
