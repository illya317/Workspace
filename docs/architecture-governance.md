...
| 线程 | 负责范围 | 避让规则 |
|---|---|---|
| Architecture | `AGENTS.md`、`docs/*` 架构治理、`scripts/arch/*`、`scripts/check/*`、`packages/platform/module-registry.ts`、`packages/platform/api-registry.ts`、Platform/API/service 收敛和 baseline ratchet | 不实现业务功能，不改 Work 体验细节，不改 Production/QC 数据生成内容 |
...
| 层 | 典型问题 | 应放位置 |
|---|---|---|
| 业务领域 | HR、Finance、Production、Work、Administration、Library、库存、绩效、采购等 | `packages/<domain>/`, `app/<domain>/`, `app/api/<domain>/` |
...
未来绩效、采购等也必须各自有独立领域入口，不要塞进 HR、Finance、Platform 或通用 `lib`。
...
- 每个任务必须包含：目标文件、动作类型（move/delete/refactor/rewrite）、目标归属层、禁止触碰范围、验证命令和风险。
