...
项目里已有一些旧 API 兼容入口，例如 `/api/hr`、`/api/finance`、`/api/work`、`/api/employees`、`/api/positions`、`/api/departments`。这些入口只能代理或兼容旧调用，迁移顺序是先补 `/api/modules/*` 新路径，再改前端 fetch，最后删除旧路径和 baseline。
...
范围: finance
文件: app/api/finance/example/route.ts, packages/finance/server/example.ts
动作: refactor
...
- `/api/hr`、`/api/finance`、`/api/work` 和部分旧 API 入口仍与 `/api/modules/*` 并存，旧入口应逐步降级为兼容代理并在无引用后删除。
