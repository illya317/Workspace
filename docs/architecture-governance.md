...
- `settings.account.access`、`work.access`、`docs.access` 是登录用户默认有效的入口权限；`work.access` 和 `docs.access` 按普通父资源授权继承到 L2，但不授予 capability。个人 API Key 使用能力使用 `settings.account.apiAccess.access`。`settings.api` 是 Open API 控制台资源，读取按 `settings.api.access`，Client 创建、secret 轮换和 scope 授权按 capability `settings.api.manage.write`。
