...
| 层 | 典型问题 | 应放位置 |
|---|---|---|
| 业务领域 | HR、Finance、Production、Work、Administration、Library、库存、绩效、采购等 | `packages/<domain>/`, `app/<domain>/`, `app/api/modules/<domain>/` |
...
项目里已有一些旧 API 兼容入口，例如 `/api/modules/hr`、`/api/modules/finance`、`/api/modules/work`、`/api/modules/hr/roster`、`/api/modules/hr/positions`、`/api/modules/hr/departments`。这些入口只能代理或兼容旧调用，迁移顺序是先补 `/api/modules/*` 新路径，再改前端 fetch，最后删除旧路径和 baseline。
...
- `app/api/*/route.ts` 只做认证、权限、参数校验、调用 package service 或 Platform 通用 factory、返回 DTO；业务模块 route 必须位于 `app/api/modules/<module>/*`，旧一级业务目录只作为兼容入口保留。
...
- `/api/modules/hr`、`/api/modules/finance`、`/api/modules/work` 和部分旧 API 入口仍与 `/api/modules/*` 并存，旧入口应逐步降级为兼容代理并在无引用后删除。
