...
import { buildContractRows } from "@/server/services/contracts";
...
export async function GET(request: Request, { params }: Props) {
  ...
  const [employments, edps, employeeProjects] = await Promise.all([
    ...
    prisma.eDP.findMany({
      where: { employeeId },
      include: {
        department: { select: { id: true, code: true, name: true, parent: { select: { code: true, name: true, parent: { select: { code: true, name: true } } } } } },
