...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  return updateCompanyRelationField(request, Promise.resolve({ id: String(parsedParams.data.id) }));
}
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const parsedParams = routeIdParamsSchema.safeParse(await params);
  if (!parsedParams.success) return jsonErrorResponse("ID 无效", 400);
  return deleteCompanyRelation(request, Promise.resolve({ id: String(parsedParams.data.id) }));
}
