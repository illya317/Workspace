import { deleteEmployee, updateEmployeeField, buildHrRouteCommand, idParams, replayJsonRequest } from "@workspace/hr/server";
...
export const PUT = createCommandRoute({
  ...
  action: ({ request, id, body }) => updateEmployeeField(replayJsonRequest(request, body), idParams(id)),
});
...
export const DELETE = createCommandRoute({
  ...
  action: ({ request, id }) => deleteEmployee(request, idParams(id)),
});
