...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  return updateDepartmentField(request, Promise.resolve({ id: String(parsedParams.data.id) }));
}
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const parsedParams = routeIdParamsSchema.safeParse(await params);
  if (!parsedParams.success) return NextResponse.json({ error: "ID 无效" }, { status: 400 });
  return deleteDepartmentByParams(request, Promise.resolve({ id: String(parsedParams.data.id) }));
}
