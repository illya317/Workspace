...
export async function GET(request: Request, context: { params: Promise<{ artifactId: string }> }) {
  ...
  if (!user) {
    const nextPath = `${BASE_PATH}/api/modules/library/integrations/wecom/download/${encodeURIComponent(params.data.artifactId)}?token=${encodeURIComponent(query.data.token)}`;
