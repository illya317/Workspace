...
import {
  canManageWorkProjectSpace,
  listWorkProjectSpacePermissions,
  normalizeWorkProjectSpaceTargetType,
  workProjectSpaceScopeId,
} from "@workspace/work/server";

const paramsSchema = z.object({
  targetType: z.enum(["personal", "user", "company", "committee", "department"]),
...
export const GET = createCommandRoute({
  ...
  buildCommand: ({ params, user }) => okCommand({
    userId: user.userId,
    targetType: normalizeWorkProjectSpaceTargetType(params.targetType),
    targetId: params.targetId,
  }),
  action: async (command) => {
    if (!await canManageWorkProjectSpace(command.userId, command.targetType, command.targetId)) {
      return serviceError("无权限管理该项目空间", 403);
    }
    const scopeId = workProjectSpaceScopeId(command.targetType, command.targetId);
    const [grantData, spacePermissions] = await Promise.all([
      getPermissionGrantData("user", "work.projects", scopeId),
      listWorkProjectSpacePermissions({
...
export const PUT = createCommandRoute({
  ...
  action: async (command) => {
    if (!await canManageWorkProjectSpace(command.actorUserId, command.targetType, command.targetId)) {
      return serviceError("无权限管理该项目空间", 403);
    }
    ...
    await setSubjectPermissionActionGrant(
      ...
      command.value,
      { actorUserId: command.actorUserId, scopeId: workProjectSpaceScopeId(command.targetType, command.targetId), storage: "action" },
    );
