...
export async function GET(request: Request) {
  ...
  return NextResponse.json(await listUserNotifications(auth.user.userId, parsed.data));
}
...
export async function DELETE(request: Request) {
  ...
  const parsed = querySchema.safeParse(Object.fromEntries(searchParams.entries()));
  if (!parsed.success) return jsonErrorResponse("参数错误", 400);
  return NextResponse.json(await clearReadUserNotifications(auth.user.userId, parsed.data));
}
...
export async function PATCH(request: Request) {
  ...
  return NextResponse.json(await markAllUserNotificationsRead(auth.user.userId, parsedQuery.data));
}
