...
const columns: DataTableColumn<DisplayItem>[] = [
  ...
  },
  {
    ...
    render: (row) => fmt(row.closingDebit),
  },
  {
    key: "closingCredit",
    label: "期末贷方",
    ...
    headerClassName: "text-right",
    cellClassName: "text-right text-slate-600",
    render: (row) => fmt(row.closingCredit),
  },
  {
    key: "net",
    label: "净值",
...
function UnmappedStatus({ item }: { item: DisplayItem }) {
  if (item.status === "excluded") {
    return <span className="rounded bg-gray-200 px-1.5 py-0.5 text-xs font-medium text-gray-600">已排除</span>;
  }
  if (item.status === "subtractOnly") {
    return (
      <span className="rounded bg-amber-100 px-1.5 py-0.5 text-xs font-medium text-amber-700">
        仅减项 → {item.subtractSourceLine ?? "?"}
      </span>
    );
  }
  return <span className="rounded bg-red-100 px-1.5 py-0.5 text-xs font-medium text-red-700">未映射</span>;
}
