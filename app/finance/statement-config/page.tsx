...
export default function StatementConfigPage() {
  ...
  return (
    <div className="space-y-4 p-4 max-w-6xl mx-auto">
        <h1 className="text-lg font-semibold text-gray-800">报表项目配置</h1>
        <div className="flex items-center gap-3">
          ...
          <button onClick={save} disabled={saving} className="rounded bg-emerald-600 px-3 py-1 text-sm text-white hover:bg-emerald-700">{saving ? "保存中..." : "保存"}</button>
                ...
                {lines.filter((l) => !l.isHeader && !l.isTotal && !l.isGrandTotal).map((l) => (
                  <tr key={l.lineCode} className="border-b">
                    ...
                    <td className="px-2 py-1">
                      <input value={l.prefixes?.join(", ") || ""} onChange={(e) => updateLine(l.lineCode, "prefixes", e.target.value.split(/[, ]+/).filter(Boolean))}
                        className="w-full rounded border border-gray-200 px-1.5 py-0.5 text-xs focus:border-emerald-400 focus:outline-none" />
                    </td>
                    <td className="px-2 py-1">
                      <input value={l.subtractPrefixes?.join(", ") || ""} onChange={(e) => updateLine(l.lineCode, "subtractPrefixes", e.target.value.split(/[, ]+/).filter(Boolean))}
                        className="w-full rounded border border-gray-200 px-1.5 py-0.5 text-xs focus:border-emerald-400 focus:outline-none" />
                    </td>
