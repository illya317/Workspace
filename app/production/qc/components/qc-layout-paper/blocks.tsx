  ...
  return (
    <PostSection block={block} title="原始数据">
      ...
      {context.advancedMode ? <Part part={{ type: "line", fieldKey: key, underline: true }} context={context} /> : (
        <>
          <button type="button" className="ml-4 rounded border border-slate-400 px-3 py-1 text-sm" onClick={() => context.onFieldChange(key, context.values[key] || "待上传")}>{block.buttonText || "上传"}</button>
          <input type="hidden" data-field-key={key} value={context.values[key] || ""} readOnly />
        </>
      )}
    </PostSection>
  );
