...
function RenderBlock({ block, context }: {
  ...
  if (block.type === "conclusion") {
    ...
    return <PostSection block={block} title="结论"><span className="whitespace-nowrap">批号<QcPaperLineInput part={{ type: "line", fieldKey: "batch_number", width: "8rem" }} value={values.batch_number} onChange={(value) => onFieldChange("batch_number", value)} />{test?.name || "本品"}（{block.conclusionName || test?.conclusionName || test?.name || "结论"}）检测过程<QcPaperSelectInput part={{ type: "select", fieldKey: processKey, underline: false }} options={["符合", "不符合"]} value={values[processKey]} onChange={(value) => onFieldChange(processKey, value)} />各项规定，结果{block.hasValue || test?.hasNumericConclusion ? <>为<Part part={{ type: "field", field: "结论-结果", readonlyDisplay: true }} context={context} />{valueUnit}，</> : null}<QcPaperSelectInput part={{ type: "select", fieldKey: resultKey, underline: false }} options={["符合", "不符合"]} value={values[resultKey]} onChange={(value) => onFieldChange(resultKey, value)} />标准规定。</span></PostSection>;
  }
