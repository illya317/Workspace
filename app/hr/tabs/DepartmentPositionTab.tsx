...
import OptionPicker, { type PickerOption } from "@/app/hr/components/OptionPicker";
import RankPicker from "@/app/hr/components/RankPicker";
              ...
              <div className="flex items-center gap-3">
                ...
                <label className="flex items-center gap-2 text-xs font-medium text-slate-500">
                  ...
                  <select
                    ...
                    onChange={(event) => handlePositionDescriptionTemplateChange(event.target.value)}
                    className="rounded-md border border-slate-300 bg-white px-2 py-1 text-xs text-slate-700 shadow-sm focus:border-emerald-500 focus:outline-none focus:ring-1 focus:ring-emerald-500"
                  >
                    {positionDescriptionTemplates.map((template) => (
                      <option key={template.id} value={template.id}>{template.label}</option>
                    ))}
                    <option value={NEW_POSITION_DESCRIPTION_TEMPLATE_OPTION}>新建模板...</option>
                  </select>
                </label>
                <button
                  type="button"
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-1 gap-3 lg:grid-cols-[110px_180px_180px_190px_auto] lg:items-start">
                <label className="flex flex-col gap-1">
                  ...
                  <select
                    value={newDepartmentDraft.level}
                    onChange={(event) => {
                    ...
                    className="h-10 w-full rounded-md border border-slate-300 bg-white px-3 py-0 text-sm focus:border-emerald-500 focus:outline-none focus:ring-1 focus:ring-emerald-500"
                  >
                ...
                <label className="flex flex-col gap-1">
                  ...
                  <select
                    value={newDepartmentDraft.parentId ?? ""}
                    disabled={newDepartmentDraft.level === 1}
                    onChange={(event) => setNewDepartmentDraft((prev) => ({ ...prev, parentId: event.target.value ? Number(event.target.value) : null, code: "" }))}
                    className="h-10 w-full rounded-md border border-slate-300 bg-white px-3 py-0 text-sm focus:border-emerald-500 focus:outline-none focus:ring-1 focus:ring-emerald-500 disabled:bg-slate-100 disabled:text-slate-400"
                  >
                    <option value="">{newDepartmentDraft.level === 1 ? "无" : `选择 L${newDepartmentDraft.level - 1} 部门`}</option>
                    {createDepartmentParentOptions.map((department) => (
      ...
      {treeDrawerOpen && (
        <div className="fixed inset-0 z-40 xl:hidden">
          ...
          <div className="absolute inset-y-0 left-0 w-[min(380px,calc(100vw-32px))] bg-white p-3 shadow-2xl">
            {renderTreePanel("drawer")}
          </div>
        </div>
      )}

      <div className={`grid grid-cols-1 gap-5 ${treeOpen ? "xl:grid-cols-[3fr_7fr]" : ""}`}>
        {treeOpen && <div className="hidden xl:block">{renderTreePanel("desktop")}</div>}
