...
父资源 `finance` 的 `entry/read/delete` 可按继承规则覆盖成本资源。成本模块当前不是通用 CRUD：发货、成本构成、成本分析、车间工分和工资均为导入后的查询模型；写入入口收口在导入脚本和导入历史清理。
...
| Tab | 数据表 | API |
|-----|--------|-----|
| 总览 | 全部（service 汇总计算） | `/api/modules/finance/cost/summary` |
| 发货与回款 | `FinanceShipment` | `/api/modules/finance/cost/shipments` |
...
| 车间工分 | `FinanceWorkshopReport` | `/api/modules/finance/cost/workshop` |
| 业务员工资 | `FinanceSalesSalary` | `/api/modules/finance/cost/sales-salary` |
| 导入记录 | `FinanceDataImport` | `/api/modules/finance/cost/imports` |
