...
export default async function FinanceStatementReviewPage() {
  ...
  const [canCreate, canUpdate, canApprove] = await Promise.all([
    evaluatePermissionAction(user.id, "finance.statementReview", "create"),
    evaluatePermissionAction(user.id, "finance.statementReview", "update"),
  ...
  return renderAppShellPage({
    ...
    backHref: "/finance",
    user,
    children: <StatementReviewClient canCreate={canCreate} canUpdate={canUpdate} canApprove={canApprove} />,
  });
}
