...
- `finance.ledger.write` — 编辑已有科目、凭证和期间。
...
│   PUT /api/modules/finance/ledger/reclass-rules → FinanceReclassRule     │
│   DELETE /api/modules/finance/ledger/reclass-rules/[id]                  │
└──────────────────────────────────────────────────────────┘
                    ↓
┌─ 生成结果 ──────────────────────────────────────────────┐
│ 凭证明细 → 生成重分类结果 按钮                            │
│   POST /api/modules/finance/ledger/reclass-results { periodId }          │
...
┌─ 审核 ──────────────────────────────────────────────────┐
│ 凭证明细 → 查看系统结果 / 人工调整例外                    │
│   PATCH /api/modules/finance/ledger/reclass-results/[id]                │
...
- `GET ?companyCode=&year=&month=&reportType=` — `finance.statementReview.access`，返回底稿或空 draft
- `PUT { companyCode, year, month, reportType, lines[] }` — `finance.statementReview.write`，**全量保存**（payload 的 lines 完全替换现有行，不在 payload 中的 lineCode 会被删除）
...
新增 API：
- `GET /api/modules/finance/statement-review/reviews?workpaperId=` — `finance.statementReview.access`
- `GET /api/modules/finance/statement-review/reviews?companyCode=&year=&month=&reportType=` — `finance.statementReview.access`
- `POST /api/modules/finance/statement-review/reviews { workpaperId }` — `finance.statementReview.create`
- `PUT /api/modules/finance/statement-review/reviews/[id] { lines[], note? }` — `finance.statementReview.write`，局部更新
...
各资源支持的动作以 `packages/platform/permission-resource-policy.ts` 为准。成本管理当前是导入后的查询模型，写入不开放通用 `write/create`，只保留导入批次清理和导入脚本语义。
...
| API | Wrapper | 说明 |
|-----|---------|------|
| `/api/modules/finance/ledger/accounts*` | `finance.ledger.access/write/delete` | 科目管理 |
| `/api/modules/finance/ledger/vouchers*` | `finance.ledger.access/write/delete` | 凭证管理 |
| `/api/modules/finance/ledger/balances` | `finance.ledger.access/revise` | 余额查询/按年度基准重新计算 |
| `/api/modules/finance/ledger/balances/reconcile` | `finance.ledger.import` | 上传会计软件年度余额表进行校准核对 |
| `/api/modules/finance/ledger/periods*` | `finance.ledger.access/write/delete` | 会计期间 |
| `/api/modules/finance/ledger/init` | `finance.ledger.write` | 财务初始化 |
| `/api/modules/finance/statements/reports*` | `finance.statements.access` | 报表生成/取数明细 |
| `/api/modules/finance/analysis/budget` | `finance.analysis.access` | 预算分析 |
| `/api/modules/finance/budget` | `finance.budget.access/import` | 预算查询/导入 |
| `/api/modules/finance/import/preview` | `finance.import.access` | 导入预览（非变更操作，用 access） |
| `/api/modules/finance/import/confirm` | `finance.import.import` | 导入确认（写入数据库，用 import） |
| `/api/modules/finance/ledger/reclass-rules` | `finance.ledger.access/revise` | 重分类规则查询/写入/删除 |
| `/api/modules/finance/ledger/reclass-results` | `finance.ledger.access/revise` | 重分类结果列表/生成/审核 |
| `/api/modules/finance/statement-review/workpapers` | `finance.statementReview.access/write` | 底稿查询/保存（P3 Batch 2） |
| `/api/modules/finance/statement-review/reviews` | `finance.statementReview.access/create/write/approve` | 校对查询/生成/更新/确认（P3 Batch 3） |
