...
个别三表冻结是显式批次动作，不是页面读取副作用：打开或切换“合并准备”只读取已有冻结来源，工具栏刷新与“生成合并工作底稿”才允许刷新草稿批次。目标期间已关账且批次已有完整可用来源时直接复用冻结快照；期间重开后才重新生成。开放期间或首次创建批次确需刷新时，利润表和现金流量表按公司、年度批量加载来源并在内存按月分组，禁止逐月调用完整报表生成器，也禁止用无界 `Promise.all` 把所有主体、报表和月份同时压入数据库连接池。来源内容指纹只比较已经生成的候选与冻结内容；正式 SHA-256 继续用于锁定输出的不可变完整性校验，不承担页面打开时的变化探测。
...
对比证据与金额解释 API 全部挂在 `/api/modules/finance/statements/**`，route 只做 auth/RBAC/Zod/一次 service 调用（命令归一与错误映射在 `comparison/route-commands.ts`）：上传 `POST /comparisons` 要求显式 `import` + `create`（网关在 `permission-api-action-policy` 用精确 pathPattern 同时强制，宽 POST=create 默认绝不放行上传）；`PUT /comparisons/:id/mapping` 用 `update`（mapping revision CAS，冲突 409）；`POST /comparisons/:id/runs`（:id=mappingId）用 `create`；`POST /comparisons/:id/archive` 用 `update`（归档而非删除）；三个 GET 与 `POST /amount-explanations/query` 用 `read`——后者是注册的显式 read-only POST exception（复杂只读查询，零持久化）。`finance.statements.import` 是 explicit-only 动作，不经 `finance` 容器继承；页面 shell 传 `canImport`，无权限时上传入口隐藏/禁用而只读解释保持可用。run 执行接线（`comparison/run-execution.ts`）同步完成生命周期：单体目标按 StatementTargetRef 的确定性报表输入重算指纹、合并目标取绑定的输出快照指纹，目标指纹漂移先 CAS 失效映射再拒绝建 run；逐行调 `explainAmountOrigin`（同一有界合同），落 immutable run + lines 并冻结 orchestrator/formula/solver 版本与 config/input/output 指纹；失败落 `failed` + failureCode。整个导入与 run 链路只写 comparison 四表，对凭证/报表/合并/重分类/FX 事实零写。Agent 只经 API discovery 注册的 `amount-explanations/query` 使用该能力，叙述必须携带 residual/ambiguity/truncation 与 `accountingTreatment: "not_evaluated"`，不得声称对账完成。
