...
当前总账前端只暴露 `revise`、`import`、`export` 相关入口：重分类规则/结果调整使用 `revise` 图标语义，余额核对使用 `import` 上传语义，重分类表导出使用 `export` 下载语义。`create/update/delete` 保留为 API contract 和后端 guard，未出现前端新增/编辑/删除按钮时不要提前读取或传递对应 UI 权限。
