...
| 端点 | 方法 | 说明 | 权限 |
|------|------|------|------|
| `/api/modules/finance/budget` | GET | 查询预算数据，支持 `?versionId=` | `finance.budget.access` |
| `/api/modules/finance/budget` | POST | 导入 Excel 预算，创建 draft 版本 | `finance.budget.write` |
| `/api/modules/finance/budget/versions` | GET | 版本列表，支持 `?year=` | `finance.budget.access` |
| `/api/modules/finance/budget/versions` | POST | 手动创建空版本 | `finance.budget.write` |
| `/api/modules/finance/budget/versions/{id}/activate` | POST | 激活版本，自动归档旧版本 | `finance.budget.write` |
