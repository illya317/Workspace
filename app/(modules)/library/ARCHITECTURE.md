...
packages/library/server/
  ...
  generators/                 # 文档生成器（Phase 6）
    ...
    authoritative-source.ts     # Platform 权威资料源客户端适配
    finance-report.ts           # 最新已验证单体 + 合并财务报表生成器
    ownership-structure.ts      # Workspace 股权结构生成器
    organization-chart.ts       # Workspace 组织架构生成器
...
packages/platform/server/onlyoffice-viewer.ts # Library 与公司文档共用的 ONLYOFFICE 配置签名、只读宿主和短时 token primitive
...
处理以不可变 `LibraryDocumentVersion.versionUid + checksumSha256` 为输入事实。`LibraryProcessingJob.idempotencyKey` 由版本 UID、输入 checksum、任务 kind 和 pipeline version 共同计算；同一输入重复提交只复用任务或增加受控 attempt，不创建另一套资料身份。当前抽取 pipeline 为 `v1.0.4`：PDF 使用页码 locator，XLSX 直接按原始工作表非空行切成 chunk 并保存唯一的 `sheet + cellRange`，不能依赖转换器内部表格编号或只生成 Markdown。
...
- 企业微信私聊的“发送/打包”请求由 Agent 先调用 Library 发送规划工具，读取按完整业务主题、当前权限和相关性收窄后的候选，再判断是否需要确认：唯一明确命中，或用户已明确要求全部发送、指定编号、确认上一轮清单时才调用发送工具；多候选且范围不明确时必须先列清单追问，路由不得在 Agent 回答后按关键词强制发送。企业微信只展示安全进度和工具执行后的最终结果，不逐字展示可能与真实发送结果冲突的模型草稿；检索结果提供带资料 ID 的精确 `viewPath`，由 worker 补齐站点 origin 和 `/workspace` basePath。发送工具会重新检索并校验所选不可变版本，最多 5 份时逐个发送原始文件，更多时生成临时 ZIP；JPG/PNG 且单份不超过 10 MiB 时进入最终流的图文 `msg_item`，其他图片走 image media，普通文件走 file media。每个文件 45 MiB 以内可直传，超限或上传失败时返回30分钟、绑定请求用户的受控绝对下载链接。临时 ZIP 与链接同为 30 分钟生命周期，后台每 5 分钟清理过期目录并把任务标记为 `expired`，数据库仅保留选择、数量、大小、checksum 和时间等审计记录。文件流和浏览器下载都会再次校验版本/导出任务 owner、当前权限与密级；群聊不得生成或发送业务资料。
...
资料抽取、PDF 预览和 OCR 共用 `LIBRARY_WORKER_PYTHON`；未配置时固定使用 `~/.cache/workspace-library/venv/bin/python`。通过 `npm run library:deps:install` 安装固定版本运行时，`npm run library:deps:check -- --quick-check` 是启动/发布前最低检查，禁止回退到未锁定依赖的系统 Python。
...
Library owner 登记资料、当前版本、完整历史版本、资料标签、业务分类、逻辑目录和目录子节点共 7 个版本化 source。公开 DTO 中稳定的标量元数据均可分析，包括内部标识、版本、路径、哈希和 Git 追踪字段；这些字段可标记为 `restricted` 或禁止导出，但只要当前账号通过原 `library.basicInfo.read` 并在资料服务中可见，轻代码读取不会再次拦截。

资料库没有可信的个人、部门或项目归属外键，三类空间均明确展示当前账号可见的全公司资料元数据，而不伪称空间自有资料。历史版本 source 先通过原资料列表服务确定当前账号可见的资料父集，再批量读取其版本公开 DTO；资料父集或历史版本超过 5,000 行时直接失败并要求增加筛选，不会截断后冒充完整数据。文件正文、存储定位、下载/预览流、Office 宿主、处理任务运行态、搜索候选和生成控制面不进入分析源。
...
- `key`: 生成器标识，如 `bp-html`、`finance-report`、`ownership-structure`、`organization-chart`、`roster-due-diligence`、`contract-ledger`
- `name`: 展示名称
- `outputCategory`: 默认输出分类
- `defaultConfidentialityLevel`: 默认保密等级
- `enabled`: 是否启用

生成流程：前端选来源 → 对自定义来源填标题/简介，对权威来源使用固定标题 → POST `/api/modules/library/basic-info/generated-sources/:key/generate` → 调注册表中的 `GeneratorFn` → 每个 artifact 分别由 `upsertGeneratedDocument` 写入 `LIBRARY_ROOT/generated/` → upsert `LibraryDocument` → 内容变化时创建 `LibraryDocumentVersion`。一个权威来源可以返回多份独立 artifact，但每份都必须有自己的稳定身份。

普通生成器按标题派生 `stableKey`；权威来源必须提供跨期不变的 `identityKey`，使同一来源始终更新同一份资料。输出内容不变时 checksum 不变，不会创建新版本。

### Workspace 权威资料源接口

Library 只依赖 Platform 的权威资料源 Interface，不直接 import Finance、HR、Capital 或 Administration 实现。各 owner 模块在自己的内部路由实现 Adapter，Library 通过部署单元间 HMAC 签名 RPC 读取已冻结的 XLSX artifact；浏览器不能直接调用这些内部端点。

当前权威来源语义：

- `finance-report`：只选最新 `locked/published` 合并批次；同时校验合并输出快照内容指纹及批次中全部单体来源的 `submitted` 状态。每家公司单体三表各自生成一份 XLSX，集团合并三表另生成一份 XLSX，不把单体与合并工作表拼入同一文件。
- `ownership-structure`：从 Capital 当前有效股权图生成集团结构与股东阵营工作表。
- `organization-chart`：从 HR 当前未归档部门树生成组织架构工作表。
- `roster-due-diligence`：复用 HR 尽调版花名册口径，不在 Library 复制员工筛选、任职和合同逻辑。
- `contract-ledger`：复用 Administration 合同导出口径完整生成当前台账，在结构化证据中披露有状态与状态为空的数量，不推断或改写历史状态。

权威来源输出必须同时提供固定 `identityKey`、`asOfDate`、`verifiedAt` 和结构化证据。证据不拼入用户可见简介，简介只保留一句中文摘要；XLSX 入库前统一按内容扩展列宽并设置合理上下限。入库后自动写 `reviewStatus=approved`、`reviewedAt=verifiedAt`；不能提供这些事实的来源不得冒充已验证资料。生成目录的内部路径继续使用稳定英文 key，页面名称由生成器写成中文。
...
- Workspace 权威快照执行双重读取授权：用户必须同时具备 `library.basicInfo:read` 与来源资源的 `read`。财务报表对应 `finance.statements`，股权结构对应 `capitalSecurities.investors`，组织架构对应 `hr.roster`，尽调版花名册对应 `hr.roster.generated`，合同台账对应 `administration.contracts`。生成入库还需 `library.basicInfo:import`，且不能绕过来源 `read`。
- 双重授权由 Library document access policy 统一注入列表、详情、目录/分类计数、预览、Office viewer、当前及历史版本下载、检索/RAG、Agent 发送和资料包导出；生成来源列表也只返回当前用户可读取来源。不得在各 route 复制一份映射。
...
- 检索在召回前按 Library read、来源 read 和 `confidentialityLevel` 过滤；RAG 不允许先检索无来源权限或高密级正文再在回答阶段删减。
...
- 下载前以 `documentId + versionId`（历史版本）或 `currentVersionId`（当前版本）定位文件，先确认 `export`、`active` 状态、来源 read 和用户可访问的保密等级，再返回文件内容
...
本项目使用 PostgreSQL。活动 migration 历史从 schema-only sanitized baseline 开始；pre-genesis 与旧 SQLite 历史只读归档在私有 `WORKSPACE_CONFIG_DIR/audit`，不得提交到源码或对 PostgreSQL 直接回放。
...
**Seed 脚本**：
- 资料库生成来源：`npm run db:seed:library-generated`（upsert `bp-html` 及五个 Workspace 权威来源）
