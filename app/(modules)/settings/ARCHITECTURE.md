...
- Managed WeCom groups are discovered only from the Bot runtime, then claimed, named and verified in the “企业微信群” tab. The UI and Agent contracts expose stable `groupKey` / `policyId` only and never accept or return raw `chatId`, provider conversation references or webhook URLs. Unclaimed, unverified or suspended groups and disabled policies remain fail-closed; weekly-report bindings use the fixed `work.weekly-report` adapter. Each bound policy owns its editable full message template in `NotificationGroupPolicy.messageTemplate`; the first line is the title and the remaining lines are the body, so the UI reads and writes the same single content that the WeCom worker ultimately formats. The UI exposes only the adapter's registered flat variables. The selected published definition supplies the governed publication protocol and must accept exactly the single `message` variable; it must not conceal another title or business-message body from the policy editor.
