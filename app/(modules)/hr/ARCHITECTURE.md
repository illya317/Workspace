...
编码表的层级详情使用 DataSurface 展开行：岗位点击后在原行下展示关联部门，窄屏在对应卡片下展开；不可达的人员列表弹窗不再保留。编辑历史仍属于审计场景，继续使用受控弹窗。
