...
| 考勤绩效 | `/hr/performance` | `app/(modules)/hr/performance/page.tsx` → `@workspace/hr/ui` 的 `HRPerformanceClient` |
...
部门负责人分两层：`Department.managerPositionId`（FK 到 `Position`）是唯一可编辑事实源；“部门负责人”是该负责人岗位下当前在职员工的派生名单，允许多人。组织架构、部门岗位、模板空间权限等场景只能读取这条链；`Department.managerUserId`、部门说明书 JSON 和岗位汇报关系不得再承载或编辑部门负责人。
...
- `hr.roster.create` — 保留给新增动作拆分；当前新增仍跟随 `write` guard
- `hr.roster.write` — 控制普通新增、编辑（POST / PUT / PATCH）
- `hr.roster.delete` — 控制删除（DELETE）
- `hr.roster.archive` — 控制部门/岗位归档与恢复
- `hr.roster.revise` — 控制审计快照恢复
