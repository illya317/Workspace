...
- 工作计划负责承载执行全集：个人、部门、项目、公司空间里的承诺事项、跟踪、汇报和闭环。
...
| 工作计划 `work.tasks` | 个人/部门/项目/公司空间的执行事项、来源、负责人、状态、周期和汇报 | 项目结构建模、会议决议正文 |
...
`/work/tasks`、`/work/tasks/personal`、`/work/tasks/companies/:id`、`/work/tasks/departments/:id`、`/work/tasks/projects/:id` 是同一个工作计划客户端工作台的不同空间视图。空间切换必须保留客户端状态，用 `window.history.pushState/replaceState` 同步 URL，不使用整页 `router.push/replace`、`redirect` 或 `<Link>`。
...
- `WorkItem`：执行事项事实表，承载个人/部门/项目/公司空间中的任务、目标和关键结果。
...
- 工作计划空间权限按个人、部门、项目、公司空间分别判定；空间切换不代表权限继承。
