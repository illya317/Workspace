...
- 会议管理使用 `work.meetings.create` 控制会议创建，`work.meetings.write/delete` 控制会议编辑和删除，`work.meetings.submit` 控制参会投票提交，`work.meetings.approve` 控制关闭表决；会议对象可见、可编辑、可管理仍由会议参与角色继续收窄。
- 项目管理使用 `work.projects.create` 控制项目、项目阶段、项目任务、项目成员和计划基线创建；`work.projects.revise` 控制计划基线激活这类修订动作。
- 工作计划使用 `work.tasks.create` 控制工作项和工作计划创建；空间权限读写仍由目标空间 `manager` 角色收窄。
