...
- GET：`production.qc.access`
- POST `/api/modules/production/qc`：`production.qc.create`
- PATCH `/api/modules/production/qc/[batchId]`：`production.qc.write`
- DELETE `/api/modules/production/qc/[batchId]`：`production.qc.delete`
- POST `/submit`：`production.qc.submit`
- POST `/approve-review`：`production.qc.approve`
